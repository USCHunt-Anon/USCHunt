...
for chain in chain_names:
    ...
    path = f"../../data/proxies_with_uschunt_results/{chain}/{net}/proxies"
    for root, d_names, f_names in os.walk(path):
        for file_name in f_names:
            file_path = os.path.join(root, file_name)
            if file_name.endswith(".json") and os.path.exists(file_path.replace(".json", ".sol")):
                version = file_path.split("/proxies/")[1].split("/")[0]
    ...
    o = open(f"../../data/proxies_with_uschunt_results/{chain}/{net}/upgradeable_addresses.txt", "w")
    o.write(str(list_upgradeable).replace(",", "\n"))
    o.close()
out = open("../../artifacts/proxy_counts.json", "w")
