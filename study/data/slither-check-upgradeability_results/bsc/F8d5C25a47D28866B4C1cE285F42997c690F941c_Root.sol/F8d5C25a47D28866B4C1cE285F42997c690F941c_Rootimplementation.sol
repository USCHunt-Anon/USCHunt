...
contract Constants {
    // Initializer keys
    bytes32 public constant INITIALIZED = keccak256("storage.longdrink.initialized");
...
library SafeERC20 {
    ...
    function _callOptionalReturn(IERC20 token, bytes memory data) private {
        ...
        bytes memory returndata = address(token).functionCall(data, string(abi.encode("SafeERC20: low-level call failed", token, data)));
...
contract Logic is Storage, Constants, IO {
    ...
    /// @notice Initializes the market maker and market maker admin
    /// @param _marketMaker initial market maker and market maker admin
    function initializeMarketMaker (address _marketMaker) public authorized2(GOVERNANCE, TIMELOCK) {
        _setupRole(MARKET_MAKER_ADMIN, _marketMaker);
        _setupRole(MARKET_MAKER, _marketMaker);
    }
    ...
    /// @notice Gets the assets and their balances within the longdrink
    /// @return (the addresses of the assets,
    ///          the amount held by the longdrink of each asset)
    ...
    /// @notice Gets the amount of assets backing each Longdrink token
    /// @return (the addresses of the assets,
    ///          the amount of backing 1 Longdrink token)
    ...
    /// @notice Sets the list of assets backing the Longdrink
    ...
    /// @notice Mints a new Longdrink token
    /// @param  _amountOut  Amount of Longdrink tokens to mint
    ...
    /// @notice Previews the corresponding assets and amount required to mint `_amountOut` Longdrink tokens
    /// @param  _amountOut  Amount of Longdrink tokens to mint
    ...
    /// @notice Burns the Longdrink token and retrieves
    /// @param  _amount  Amount of Longdrink tokens to burn
