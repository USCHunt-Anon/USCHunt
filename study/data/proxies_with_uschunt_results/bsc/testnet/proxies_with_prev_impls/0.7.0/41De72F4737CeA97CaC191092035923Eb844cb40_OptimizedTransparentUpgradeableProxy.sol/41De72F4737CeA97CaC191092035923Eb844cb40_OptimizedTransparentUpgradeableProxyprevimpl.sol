...
contract Airdrop is ReentrancyGuard, Pausable, RedKiteWhitelist {
     ...
     * @param _stakePool Stake pool address
     */
    function initialize(
        address _token,
        address _signer,
        ...
        paused = false;
        signer = _signer;
        stakePool = ILinearPool(_stakePool);
    }
    ...
    function changeStakePool(address _stakePool) external onlyOwner {
        stakePool = ILinearPool(_stakePool);
    }
    ...
    /**
     * @notice Claim token
     * For recovery dApp only
     * to prevent users claim tokens many times with the same amount
     */
    function claimTokens(
        ...
        require(
            ...
            "POOL::MUST_AMOUNT_GREATER_THAN_CLAIMED"
        );
    ...
    function claimAndStakeTokens(
        ...
        uint256 claimAmount = _amount.sub(userClaimed[_candidate]);
        require(claimAmount <= _amount, "POOL::INVALID_STAKE_AMOUNT");

        userClaimed[_candidate] = userClaimed[_candidate].add(claimAmount);

        _stakeToken(_poolId, uint128(_amountStake), msg.sender);

        if (claimAmount - _amountStake > 0) {
            _deliverTokens(msg.sender, claimAmount - _amountStake);
        }
    ...
    function _stakeToken(
        ...
        _approveIfNeeded(address(stakePool), _amount);
        stakePool.linearDepositSpecifyReceiver(_poolId, _amount, _receiver);
    }

    function _approveIfNeeded(address _spender, uint256 _minAmount) private {
        if (_minAmount > token.allowance(address(this), _spender)) {
...
interface ILinearPool {
    function linearDeposit(uint256 _poolId, uint128 _amount) external;
    ...
    function linearWithdraw(uint256 _poolId, uint128 _amount) external;

    function linearClaimPendingWithdraw(uint256 _poolId) external;

    function linearClaimReward(uint256 _poolId) external;
}
