...
library PawnLib {
    ...
    function safeTransfer(address asset, address from, address to, uint256 amount) internal {
        if (asset == address(0)) {
            ...
            require(IERC20(asset).balanceOf(from) >= amount, '3'); //not-enough-balance
            if (from == address(this)) {
                // transfer direct to to
                IERC20(asset).safeTransfer(to, amount);
            } else {
                require(IERC20(asset).allowance(from, address(this)) >= amount, '4'); //not-allowance
                IERC20(asset).safeTransferFrom(from, to, amount);
            }
            require(IERC20(asset).balanceOf(to) - amount == prebalance, '5'); //not-trans-enough
        }
    }
