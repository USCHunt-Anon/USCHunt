...
contract PlanetDiscountDelegate is PlanetStoragev1,Exponential{
    ...
    function totalReservesAfterDiscount(address market) external view returns(uint res){
        (,res) = subUInt(GToken(market).totalReserves(),totalDiscountGiven[market]);
    }
    ...
    function changeUserBorrowDiscount(address borrower) external returns(uint,uint,uint){
        ...
        if( _dis.exist && discount > 0) {
            ...
            (,vars.interest) = subUInt(currentBorrowBal,_dis.lastBorrowAmountDiscountGiven);
            ...
            (,vars.newDiscount) = mulUInt(discount,valueOfGivenInterestGoToReserves);
            
            (,vars.newDiscount) = divUInt(vars.newDiscount,10000);
            
            //update the borrow amount of borrower
            _dis.lastBorrowAmountDiscountGiven = currentBorrowBal;
            
            (, vars.accountBorrowsNew) = subUInt(currentBorrowBal, vars.newDiscount);
            
            (,totalDiscountGiven[_market]) = addUInt(totalDiscountGiven[_market],vars.newDiscount);
