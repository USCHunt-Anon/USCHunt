...
contract DinoLandNFTUpgradeable is ERC721Upgradeable, DinosAccessControl {
     ...
     function initialize() initializer public {
        ...
        //Create some genesis dinos
        _createDino(1111, msg.sender, block.timestamp, 1, 0); // Normal Novis
        _createDino(1212, msg.sender, block.timestamp, 2, 0); // Rare Aquis
        _createDino(1313, msg.sender, block.timestamp, 1, 0); // Super Rare Terrot
        _createDino(1313, msg.sender, block.timestamp, 2, 0); // Super Rare Terrot
        _createDino(1314, msg.sender, block.timestamp, 1, 0); // Lengendary Terrot
        _createDino(1315, msg.sender, block.timestamp, 1, 0); // Mystic Terrot
     }
    ...
    string dnlStorageExtenstion = "png";
    
    event DinoSpawned(uint256 indexed _dinoId, address indexed _owner, uint256 _genes, uint128 _gender, uint128 _generation);
    event DinoRetired(uint256 indexed _dinoId);
    event DinoEvolved(uint256 indexed _dinoId, uint256 _oldGenes, uint256 _newGenes);
    event DinoTransfered(uint256 indexed _dinoId, address _oldOwner, address _newOwner);
    ...
    function _removeDinoFromArray(uint256 _dinoId, uint256[] memory _array) internal pure returns (uint256[] memory){
    ...
    function getDino(uint _dinoId) external view returns(uint256 genes, uint256 bornAt, uint256 cooldownEndAt, uint128 gender, uint128 generation) {
        Dino memory dino = dinos[_dinoId];
        return (dino.genes, dino.bornAt, dino.coolDownEndAt, dino.gender, dino.generation);
    }
    ...
    function tokenURI(uint256 _dinoId) public view virtual override returns (string memory) {
        require(_exists(_dinoId), "ERC721Metadata: URI query for nonexistent token");
        string memory baseURI = _baseURI();
        return bytes(baseURI).length > 0 ? string(abi.encodePacked(_baseURI(), "dino_",_dinoId, ".", dnlStorageExtenstion)) : "";
    }
    ...
    function setDNLStorageURI(string memory _dnlStorageURI) external onlyCLevel {
        dnlStorageURI = _dnlStorageURI;
    }
    
    function setDNLStorageExtension(string memory _dnlStorageExtension) external onlyCLevel {
    ...
    function transferFrom(
        ...
        //solhint-disable-next-line max-line-length
        require(_isApprovedOrOwner(_msgSender(), _dinoId), "transfer caller is not owner nor approved");
        _transfer(_from, _to, _dinoId);
        userOwnedDinos[_to].push(_dinoId);
        userOwnedDinos[_from] = _removeDinoFromArray(_dinoId, userOwnedDinos[_from]);
    ...
    function safeTransferFrom(
        ...
        require(_isApprovedOrOwner(_msgSender(), _dinoId), "ERC721: transfer caller is not owner nor approved");
    ...
    function createDino(uint256 _genes, address _owner, uint128 _gender, uint128 _generation)
        onlySpawner
        external
        returns (uint256)
    {
        require(_genes >= 1111 , "Dino genes is not valid");
        uint256 dinoId = _createDino(_genes, _owner, block.timestamp, _gender, _generation);
    ...
    function _createDino(uint256 _genes, address _owner, uint256 _coolDownEndAt, uint128 _gender, uint128 _generation)
        private
        returns (uint256)
    {
        Dino memory dino = Dino(_genes, block.timestamp, _coolDownEndAt, _gender, _generation);
        dinos.push(dino);
