...
contract DinoMarketplaceUpgradeable is Pausable, Ownable, Initializable {
  ...
  function disableEggAndCreateDino(uint256 _eggId, uint256 _dinoGenes, address _ownerAddress, uint128 _gender, uint128 _generation) external onlyAdminOrOwner returns(uint256) {
      require(_eggId <= eggs.length -1, "egg_not_exist");
      eggs[_eggId].isAvailable = false;
      uint256 newDinoId = IDinoLandNFT(nftAddress).createDino(_dinoGenes, _ownerAddress, _gender, _generation);
  ...
  function cancelAllAuction() external onlyAdminOrOwner {
