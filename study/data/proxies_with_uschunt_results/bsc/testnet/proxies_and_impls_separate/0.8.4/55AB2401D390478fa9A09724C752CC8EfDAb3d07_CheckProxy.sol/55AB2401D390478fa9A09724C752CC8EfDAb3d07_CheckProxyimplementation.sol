...
contract Token is Context, IERC20, Ownable {
	...
	uint256 public ALL_PERCENT = 100;
	uint256 public maxLiqFee = 100;
	uint256 public maxTaxFee = 100;
	uint256 public maxBurnFee = 100;
	uint256 public maxWalletFee = 100;
	uint256 public maxBuybackFee = 100;
	uint256 public minMxTxPercentage = 1;
	uint256 public minMxWalletPercentage = 1;
	...
	) payable
	{
		...
		_maxTxAmount = _tTotal*(setMxTxPer)/(ALL_PERCENT);
		_maxWalletAmount = _tTotal*(setMxWalletPer)/(ALL_PERCENT);
	...
	function checkOverAllFeePercent() private view {
		uint256 all = _taxFee + _burnFee + _walletFee + _buybackFee + _liquidityFee;
		require(all <= ALL_PERCENT, "More than 100 percent is impossible");
	}
	...
	function setMaxTxPercent(uint256 maxTxPercent) external onlyOwner() {
		...
		_maxTxAmount = _tTotal * maxTxPercent / (ALL_PERCENT);
	}
	...
	function setMaxWalletPercent(uint256 maxWalletPercent) external onlyOwner() {
		...
		_maxWalletAmount = _tTotal * maxWalletPercent / (ALL_PERCENT);
	}
	...
	function calculateTaxFee(uint256 _amount) private view returns (uint256) {
		return (_amount * _taxFee) / (ALL_PERCENT);
	}

	function calculateLiquidityFee(uint256 _amount) private view returns (uint256) {
		return _amount * (_liquidityFee + _burnFee + _walletFee + _buybackFee) / (ALL_PERCENT);
	}
