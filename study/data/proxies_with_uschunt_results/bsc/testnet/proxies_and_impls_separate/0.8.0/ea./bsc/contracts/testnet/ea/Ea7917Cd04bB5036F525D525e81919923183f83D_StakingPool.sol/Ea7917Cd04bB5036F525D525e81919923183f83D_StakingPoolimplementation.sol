...
contract ImplAndTermsV6 is Storage, Ownable, DoTransfer, ERC20Init {
    ...
    // stakeId => bool
    mapping(uint => bool) public approvedRewards;
    ...
    function _approveReward(uint stakeId, bool isApprove_) public onlyOwner returns (bool) {
        approvedRewards[stakeId] = isApprove_;
    ...
    function unstake(uint[] calldata userStakeIds) external {
        ...
        for (uint i = 0; i < userStakeIds.length; i++) {
            ...
            if (amountOut > maxDefaultApprovedReward) {
                require(approvedRewards[userStakeIds[i]], "ImplAndTerms::unstake: unstake reward is not approve");
            }
