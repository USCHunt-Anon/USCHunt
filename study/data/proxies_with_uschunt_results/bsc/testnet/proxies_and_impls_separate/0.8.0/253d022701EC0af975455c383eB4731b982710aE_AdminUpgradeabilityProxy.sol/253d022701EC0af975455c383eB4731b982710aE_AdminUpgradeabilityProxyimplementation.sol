...
interface IFyb3rBundleMarketplace {
...
//import"./interfaces/IFyb3rBundleMarketplace.sol";
//import"./interfaces/IFyb3rAuction.sol";
//import"./interfaces/IFyb3rNFTFactory.sol";
//import"./interfaces/IFyb3rTokenRegistry.sol";
//import"./interfaces/IFyb3rPriceFeed.sol";

contract Fyb3rMarketplace is OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function _buyItem(
        ...
        IFyb3rBundleMarketplace(addressRegistry.bundleMarketplace())
    ...
    function createOffer(
        ...
        IFyb3rAuction auction = IFyb3rAuction(addressRegistry.auction());
    ...
    function registerRoyalty(
        ...
        require(_isFyb3rNFT(_nftAddress), "invalid nft address");
    ...
    function _isFyb3rNFT(address _nftAddress) internal view returns (bool) {
        return
            addressRegistry.artion() == _nftAddress ||
            IFyb3rNFTFactory(addressRegistry.factory()).exists(_nftAddress) ||
            IFyb3rNFTFactory(addressRegistry.privateFactory()).exists(
                _nftAddress
            ) ||
            IFyb3rNFTFactory(addressRegistry.artFactory()).exists(
                _nftAddress
            ) ||
            IFyb3rNFTFactory(addressRegistry.privateArtFactory()).exists(
                _nftAddress
            );
    }
    ...
    function getPrice(address _payToken) public view returns (int256) {
        int256 unitPrice;
        uint8 decimals;
        IFyb3rPriceFeed priceFeed = IFyb3rPriceFeed(
            addressRegistry.priceFeed()
        );

        if (_payToken == address(0)) {
            (unitPrice, decimals) = priceFeed.getPrice(priceFeed.wBNB());
        } else {
        ...
        require(
            _payToken == address(0) ||
                (addressRegistry.tokenRegistry() != address(0) &&
                    IFyb3rTokenRegistry(addressRegistry.tokenRegistry())
                        .enabled(_payToken)),
            "invalid pay token"
        );
    }
