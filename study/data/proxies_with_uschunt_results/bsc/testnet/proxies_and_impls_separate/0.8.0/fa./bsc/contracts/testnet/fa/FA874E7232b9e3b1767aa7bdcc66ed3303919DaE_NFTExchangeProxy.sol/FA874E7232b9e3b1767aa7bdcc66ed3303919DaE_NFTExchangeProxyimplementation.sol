...
contract API is Upgradable {
    ...
    * set the address of the super admin
    ...
    function setCreateHandlerAddress(address _createHandler) public onlySuperAdmin {
        createHandlerAddress = _createHandler;
    }

    function setEditHandlerAddress(address _editHandler) public onlySuperAdmin {
        editHandlerAddress = _editHandler;
    }

    function setPutOnSaleHandlerAddress(address _putOnSaleHandler) public onlySuperAdmin {
        putOnSaleHandlerAddress = _putOnSaleHandler;
    }
    ...
    function getNFTItem(uint256 _tokenId) public view returns (NFTItem memory item) {
        return mapIdToNft[_tokenId];
    }

    function getSaleOrdersAdmin() public view returns (SaleOrder[] memory) {
        return saleOrdersAdmin;
    }

    function getSaleOrdersUser() public view returns (SaleOrder[] memory) {
        return saleOrdersUser;
    }

    function getSaleOrderBySignature(bytes memory _signature) public view returns (SaleOrder memory) {
        return saleOrderBySignature[_signature];
    }

    function getSaleOrdersBySeller(address _addr) public view returns (SaleOrder[] memory) {
        return saleOrdersBySeller[_addr];
    }

    function getPayoutGroupBySignature(bytes memory _signature) public view returns (PayoutGroup memory) {
        return payoutGroupBySignature[_signature];
    }
}
...
//import{ PEM1155 } from "../Token1155.sol";
//import{ PEM721 } from "../Token721.sol";
//import"./Structs.sol";
...
contract Upgradable is ReentrancyGuarded {
    ...
    address public createHandlerAddress;
    address public editHandlerAddress;
    address public putOnSaleHandlerAddress;
    
    address public superAdmin;

    NFTItem[] nftItems;
    SaleOrder[] saleOrdersAdmin;
    SaleOrder[] saleOrdersUser;
    // mapping (uint256 => NFTItem) nftItem;               // token ID => nft item
    ...
    mapping (uint256 => NFTItem) mapIdToNft;            // tokenId -> nft

    mapping (address => NFTItem[]) listOwnedItemByAddress;   // address -> owned nft

    mapping (bytes => SaleOrder) saleOrderBySignature;      // signature -> sale order
    mapping (address => SaleOrder[]) saleOrdersBySeller;    // address -> sale orders

    mapping (bytes => PayoutGroup) payoutGroupBySignature;  // signature -> payout group
...
struct NFTItem {
    ...
    uint256 amount;
    uint256 createdAt;
	uint256 tokenType; // 0: dort, 1: item
	uint256 rarity; // 0: Common, 1:UnCommon, 2:Rare, 3: Ultra rare 
    uint256 royalty; // 1->100 %
	string name;
	string description;
    address owner; // address create nft
	uint256[] traits; //[levels, Scientist, Warrior, Builders, Magicians, Grunt, Farmers, Transporters, Ultra Rare Royal] element between [0,10]
}
...
struct SaleOrder {
    ...
    uint256 totalCopies;   // quantity of token has been created
...
//import{API} from "../common/Api.sol";

contract NFTExchange is API {
    ...
    function createNFT(
        ...
        require(
            createHandlerAddress != address(0),
            "NFTExchange: buyHandlerAddress is zero"
        );
        address impl = createHandlerAddress;
    ...
    function editNFT(
        ...
        require(
            editHandlerAddress != address(0),
            "NFTExchange: buyHandlerAddress is zero"
        );
        address impl = editHandlerAddress;
    ...
    function putOnSale() public {
        require(
            putOnSaleHandlerAddress != address(0),
            "NFTExchange: buyHandlerAddress is zero"
        );
        address impl = putOnSaleHandlerAddress;
