...
contract UpgradableV2 is ERC1155Holder{
    ...
    /**
     * @param amount amount of stake/unstake of token or nft 
    ...
    mapping(address => uint256) totalAmountStakedPerUser;  // userAddr => totalAmountStaked 
    mapping(address => uint256) totalPoolStakedPerUser;    // userAddr => totalPoolStaked
    mapping(address => uint256) totalRewardClaimedPerUser; // userAddr => totalRewardClaimed

    mapping(string => mapping(address =>uint256)) public totalStakedInPoolByUser; // poolId => user => totalStaked
...
contract StakingPool1155 is UpgradableV2{
     ...
     * @param _signatureUtils: address of SignatureUtils contract, for staking data verification
     * @param _collection1155: address of NFT1155 Marketplace contract, stake 1155 tokens from this contract
     * @param _rewardToken: address of reward token contract, the token used for reward distribution
    */
    ...
    function stake1155(
        ...
        if (isSignatureEnable){
            require(signatureUtils.verify1155(poolId_, tokenId_, amount_, scarcityId_, signature_) == signer, "Stake 1155: Signature not valid");
        }
        ...
        totalAmountStakedPerUser[msg.sender] += amount_;
        ...
        emit StakingEvent(msg.sender, poolId_,amount_, 0, strs[1]);
    }
    ...
    function unstake1155(string[] memory strs, uint256 tokenId_, uint256 amount_) external notBlocked() poolEnable(strs[0]){
        ...
        emit StakingEvent(msg.sender, poolId_, amount_, reward, strs[1]);
    }
    ...
    function claim1155(string[] memory strs, uint256 tokenId_) external notBlocked() poolEnable(strs[0]){
        ...
        emit StakingEvent(msg.sender, poolId_,0,reward, strs[1]);
    }
     ...
     */
    function calculateClaimableReward1155(string memory poolId_, address user_, uint256 tokenId_, string memory stakingId_) external view returns(uint256){
        StakingData1155 storage data =  stakingDatas[poolId_][user_][tokenId_][stakingId_];
     ...
     * @dev enable pool
     */
    function setPoolStatus(string memory poolId_, bool status_) external onlyAdmins{
        poolInfo[poolId_].isActive = status_;

        emit PoolStatus(poolId_, status_);
    }
    ...
    */
    function setSignatureUtilsAddress(address _signatureUtils) external {
        signatureUtils = SignatureUtils1155(_signatureUtils);
        emit RoleEvent(_signatureUtils, true);
    }
    ...
    */
    ...
    function setCollection(address _collection) external {
        collection1155 = IERC1155(_collection);
        emit RoleEvent(_collection, true);
    }
    
    /**
     * @dev disable checking signature when stake token
     */
    function setSignatureStatus(bool status_) external onlyAdmins{
        isSignatureEnable = status_;

        emit RoleEvent(address(this), status_);
    }

    function setSigner(address newSigner_) external onlyAdmins{
        signer = newSigner_;
    }

    /**
     * @dev Withdraw the remaining reward of the pool
     */
    function emergencyWithdrawFromPool(address account_, string memory poolId_) external {
        ...
        emit WithdrawalEvent(address(rewardToken), address(this), account_, withdrawableAmount);
    }
    ...
    */
     ...
     */
    function getStakingIds(string memory poolId_, address user_, uint256 tokenId_) external view returns(string[] memory){
    ...
    *  @dev Get total staked balacne of pool
    ...
    /**
     * @return 0: total pool staked, 1: total edition locked, 2: total reward claiemd
     */
    function getUserStakingData(address[] memory users_) public view returns(uint256[] memory, uint256[] memory, uint256[] memory){
        uint256[] memory totalPoolStaked = new uint256[](users_.length);
        uint256[] memory totalEditionLocked = new uint256[](users_.length);
        uint256[] memory totalRewardClaimed = new uint256[](users_.length);

        for (uint16 i=0; i< users_.length; i++){
            totalPoolStaked[i] =  totalPoolStakedPerUser[users_[i]];
            totalEditionLocked[i] = totalAmountStakedPerUser[users_[i]];
            totalRewardClaimed[i] = totalRewardClaimedPerUser[users_[i]];
        }

        return(totalPoolStaked, totalEditionLocked, totalRewardClaimed);
    }
