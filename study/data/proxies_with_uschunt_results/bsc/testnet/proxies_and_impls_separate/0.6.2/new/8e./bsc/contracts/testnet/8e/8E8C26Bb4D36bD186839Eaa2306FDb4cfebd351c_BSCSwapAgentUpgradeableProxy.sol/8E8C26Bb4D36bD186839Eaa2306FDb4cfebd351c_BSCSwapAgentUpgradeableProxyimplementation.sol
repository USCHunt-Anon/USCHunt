// File: contracts\interfaces\IBEP20Query.sol

pragma solidity 0.6.4;

interface IBEP20Query {
    /**
...
contract BSCSwapAgentImpl is Context, Initializable {
    ...
    function registerSwapPairToETH(address bep20Addr) external returns (bool) {
        require(!registeredBEP20[bep20Addr], "already registered");

        string memory name = IBEP20Query(bep20Addr).name();
        string memory symbol = IBEP20Query(bep20Addr).symbol();
        uint8 decimals = IBEP20Query(bep20Addr).decimals();
    ...
    function fillETH2BSCSwap(bytes32 ethTxHash, address bep20Addr, address toAddress, uint256 amount) onlyOwner external returns (bool) {
        ...
        emit SwapFilled(bep20Addr, ethTxHash, toAddress, amount);
        return true;
    }
