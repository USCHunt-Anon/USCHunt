...
abstract contract IDivision {
    ...
    function setDivision(Division[] memory _divisions) external virtual;

    function getDivision(address _user) public view virtual returns (Division memory);
}
...
contract Farming is  ReentrancyGuardUpgradeable,OwnableUpgradeable {
    ...
    address public ownerW = address(0x000000000000000000000000000000000000dEaD);
    ...
    function initialize(address _rewardsToken,address _stakingToken,address _division) public initializer{
         rewardsToken = IERC20Upgradeable(_rewardsToken);
         stakingToken = IERC20Upgradeable(_stakingToken);
         division = IDivision(_division);
         __Ownable_init();
         __ReentrancyGuard_init();

    }

    function setDivision(address _division) external onlyOwner{
        division = IDivision(_division);
	}
    ...
    function updateWhiteList(address[] memory accounts, bool excluded) external onlyOwner{
         for (uint256 i = 0; i < accounts.length; i++) {
             isWhiteListed[accounts[i]] = excluded;
         }
    }
    function updateWhiteListOnly(bool state) external onlyOwner{
        whitelistOnly = state;
    }
    ...
    function stake(uint256 amount) external  nonReentrant updateReward(msg.sender) {
        ...
        require(isWhiteListed[msg.sender] && whitelistOnly,"User Whitelisted");
    ...
    function getReward() public  nonReentrant updateReward(msg.sender) {
        ...
        if (reward > 0) {
            uint256 rewardDiv = earnedDiv(msg.sender);

            rewards[msg.sender] = 0;
            rewardsToken.safeTransfer(msg.sender, rewardDiv);
            rewardsToken.safeTransfer(ownerW,reward.sub(rewardDiv));
            emit RewardPaid(msg.sender, rewardDiv);
        }
    }
    ...
    function earnedDiv(address account) public view  returns (uint256) {
        uint256 total = division.getTotal();
        return earned(account).mul(division.getDivision(account).divId.add(1)).div(total);
    }
