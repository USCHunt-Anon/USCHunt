...
contract LotteryNFT is ERC721, Ownable {
    ...
    constructor() public ERC721("miracles Lottery Ticket", "PLT") {}
...
contract Lottery is LotteryOwnable, Initializable {
    ...
    function buy(uint256 _price, uint8[4] memory _numbers) external {
       ...
       // uint amount = _price.mul(feePercent).div(100);
       // _price = _price.sub(amount);
        ...
        totalCount[msg.sender] = userInfo[msg.sender].length; 
        totalAmount = totalAmount.add(_price);
        lastTimestamp = block.timestamp;
        ...
        miracles.safeTransferFrom(address(msg.sender), address(this), _price);
       // miracles.safeTransfer(feeAddress,amount);
    ...
    function  multiBuy(uint256 _price, uint8[4][] memory _numbers) external {
        ...
        miracles.safeTransferFrom(address(msg.sender), address(this), totalPrice);
    ...
    function claimReward(uint256 _tokenId) external {
        ...
        if(reward>0) {
            miracles.safeTransfer(address(msg.sender), reward);
        }
    ...
    function  multiClaim(uint256[] memory _tickets) external {
        ...
        if(totalReward>0) {
            miracles.safeTransfer(address(msg.sender), totalReward);
        }
    ...
    function updateAddress(address _fee,uint _percent)public onlyOwner{
        feeAddress = _fee;
        feePercent = _percent;
    }
    ...
    function adminWithdraw(uint256 _amount) public onlyAdmin {
        miracles.safeTransfer(address(msg.sender), _amount);
