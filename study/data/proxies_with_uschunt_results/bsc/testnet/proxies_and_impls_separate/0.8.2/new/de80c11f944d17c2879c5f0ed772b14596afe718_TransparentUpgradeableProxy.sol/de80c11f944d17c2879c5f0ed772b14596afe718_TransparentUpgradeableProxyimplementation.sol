...
interface IDivMarketplaceFeeDistributor {
    function distributeTSHIBTokens()
    external
    returns (bool);
    event DistributedTSHIBTokens(uint256 amount);
}
...
contract TST is IBEP20, Context {
    using SafeMath for uint256;
    ...
    uint256 _totalSupply = 1 * 10**15 * (10**_decimals); // 1 Billion
    ...
    function swapBack() internal swapping {
       ...
       _basicTransfer(address(this),address(DivMarket), amountSwapBackBefore.div(2));
        ...
        try DivMarket.distributeTSHIBTokens() {} catch {}
