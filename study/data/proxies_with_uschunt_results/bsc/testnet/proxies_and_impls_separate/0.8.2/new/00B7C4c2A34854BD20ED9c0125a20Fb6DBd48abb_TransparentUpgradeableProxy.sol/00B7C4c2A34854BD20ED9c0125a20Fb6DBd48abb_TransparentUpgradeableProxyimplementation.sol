...
// File Contracts/interfaces/IAutoMBABY.sol


pragma solidity ^0.8.0;

interface IAutoMBABY {
    ...
    struct UserInfo {
        ...
        uint MBABYAtLastUserAction; // keeps track of MBABY deposited at the last user action
...
contract FixedStaking is Initializable, AccessControlUpgradeable, ReentrancyGuardUpgradeable, PausableUpgradeable {
    ...
    function initialize(address _treasury, address _treasuryAdmin, IAutoMBABY _autoMBABY) public initializer {
        require(_treasury != address(0) && address(_autoMBABY) != address(0) && _treasuryAdmin != address(0), "Address cant be zero");
    ...
    function setAutoMBABY(IAutoMBABY _autoMBABY) external onlyRole(DEFAULT_ADMIN_ROLE){
        require(address(_autoMBABY) != address(0), "Cant be zero address");
        autoMBABY = _autoMBABY;
    }
    ...
    function getHolderPoolAmount(address _user) public view returns(uint holderPoolAmount){
        holderPoolAmount = autoMBABY.balanceOf() * autoMBABY.userInfo(_user).shares / autoMBABY.totalShares();
    }
