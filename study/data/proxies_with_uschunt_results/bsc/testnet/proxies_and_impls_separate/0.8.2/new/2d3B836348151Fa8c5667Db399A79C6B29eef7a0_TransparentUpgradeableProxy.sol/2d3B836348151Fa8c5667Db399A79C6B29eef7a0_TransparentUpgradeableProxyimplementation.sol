...
测试网：
第一次部署： 0xE7976D53B32648cFE9Ef6CB3e8Cd81af4804cf9E
...
再引用
//import"../proxy/utils/Initializable.sol"; 上面已经有了
...
contract AS_Params is Initializable,OwnableUpgradeable {
    mapping(address => uint256) private add_uint;
    ...
    event Uint256ParamSetted(address _addr,uint256 _value);

    //实现设置地址绑定一个信息
    function SetUint256Param(address _addr,uint256 _value) external onlyOwner{
        add_uint[_addr] = _value;
        emit Uint256ParamSetted(_addr,_value);
    }
    function GetUint256Param(address _addr)public view returns(uint256){
        return add_uint[_addr];
    }

    /*/为升级测试添加的
    function GetID() public  pure returns (bytes4){
        //return keccak256(abi.encodePacked(pass));
        return bytes4(keccak256("initialize()"));
    }*/
