...
interface IMasterChef {
    ...
    function getBoostMultiplier(address _user, uint256 _pid) external returns (uint256);
    struct PoolInfo {
    ...
    function totalRegularAllocPoint() external returns (uint256);
...
contract StakingSetStorage is Ownable, Context, ReentrancyGuard, ERC165 {    
    ...
    uint256 public constant CAKE_PID = 4;
...
contract StakingSet is StakingSetStorage, IBEP721, IBEP721Metadata {
    ...
    function buyStakingSet(uint256 amount, uint256 tokenId) payable external {
      ...
      (uint256 nbuAmount,uint256 gnbuAmount,uint256 cakeAmount,uint256 cakeLPamount) = makeSwaps(amountBNB); 
   ...
   // uint RewardDebt;
      ...
      uint oldRewardDebt = user.rewardDebt;
      ...
      userSupply.RewardDebt = user.rewardDebt - oldRewardDebt;
    ...
    function makeSwaps(uint256 amount) private returns(uint256,uint256,uint256,uint256) {
      uint NbuAmount = amount / 100 * 35;
    ...
    (, uint amountBnbCake, uint liquidityBnbCake) = pancakeRouter.addLiquidityETH{value: amount - (CakeEAmount / 2)}(address(cakeToken), amountsBnbCakeSwap[1], 0, 0, address(this), block.timestamp);
      ...
      return (amountsBnbNbuStaking[1], amountsBnbGnbuStaking[1], amountsBnbCakeSwap[1], liquidityBnbCake);
...
// withdraw Cake
    ...
    function getTokenRewardsAmounts(uint tokenId) public returns (uint256 NbuUserRewards, uint256 GnbuUserRewards, uint256 CakeUserRewards) {
        UserSupply memory userSupply = tikSupplies[tokenId];
        require(userSupply.IsActive, "StakingSet: Not active");
    ...
    function getTotalAmountsOfRewards(uint tokenId) public returns (uint256, uint256) {
    ...
    function getUserCakeRewards(uint256 tokenId) public returns (uint256) {
        UserSupply memory userSupply = tikSupplies[tokenId];
        require(userSupply.IsActive, "StakingSet: Not active");
        ...
        return boostedAmount * accCakePerShare / ACC_CAKE_PRECISION - userSupply.RewardDebt;
    }
    ...
    function _withdrawUserRewards(uint256 tokenId, address tokenOwner, uint256 totalNbuReward, uint256 totalCakeReward) private {
        ...
        if (cakeToken.balanceOf(address(this)) < totalCakeReward) {
            CakeStaking.deposit(CAKE_PID, 0);
            tikSupplies[tokenId].RewardDebt += totalCakeReward;
        }
        ...
        */
        ...
        TransferHelper.safeTransfer(address(cakeToken), tokenOwner, totalNbuReward);
