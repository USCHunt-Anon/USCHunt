...
contract Comptroller is ComptrollerV5Storage, ComptrollerInterface, ComptrollerErrorReporter, ExponentialNoError {
    ...
    /// @notice Emitted when a new borrow-side ANN speed is calculated for a market
    ...
    function _supportMarket(AToken aToken) public returns (uint) {
    ...
    function _initializeMarket(AToken  aToken) internal {
        uint blockNum = getBlockNumber();
        uint32 blockNumber = safe32(uint(blockNum), "block number exceeds 32 bits");
