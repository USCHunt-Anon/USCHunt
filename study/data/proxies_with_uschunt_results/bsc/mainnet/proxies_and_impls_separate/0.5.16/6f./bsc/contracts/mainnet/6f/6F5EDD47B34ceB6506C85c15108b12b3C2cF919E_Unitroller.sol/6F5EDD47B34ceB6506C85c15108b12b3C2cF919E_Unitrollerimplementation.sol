...
contract Ecoptroller is EcoptrollerV5Storage, EcoptrollerInterface, EcoptrollerErrorReporter, ExponentialNoError {
     ...
     * @param ecopSpeed New COMP speed for market
     */
     ...
     */
    function claimEcop(address holder) public {
     ...
     */
    function claimEcop(address holder, EToken[] memory eTokens) public {
     ...
     */
    function claimEcop(address[] memory holders, EToken[] memory eTokens, bool borrowers, bool suppliers) public {
     ...
     * @param amount The amount of COMP to (possibly) transfer
     */
    function _grantEcop(address recipient, uint amount) public {
     ...
     * @param ecopSpeed New COMP speed for market
     */
    function _setEcopSpeed(EToken eToken, uint ecopSpeed) public {
     ...
     * @param ecopSpeed New COMP speed for contributor
     */
    function _setContributorEcopSpeed(address contributor, uint ecopSpeed) public {
        require(adminOrInitializing(), "only admin can set ecop speed");
     ...
     */
    function getEcopAddress() public view returns (address) {
...
contract Ecop {
    /// @notice EIP-20 token name for this token
    string public constant name = "ECOP";
    ...
    /// @notice Total number of tokens in circulation
    uint public constant totalSupply = 100000000e18; // 100 million Ecop
    ...
    function transfer(address dst, uint rawAmount) external returns (bool) {
        uint96 amount = safe96(rawAmount, "Ecop::transfer: amount exceeds 96 bits");
    ...
    function transferFrom(address src, address dst, uint rawAmount) external returns (bool) {
        ...
        uint96 amount = safe96(rawAmount, "Ecop::approve: amount exceeds 96 bits");
    ...
    function getPriorVotes(address account, uint blockNumber) public view returns (uint96) {
        require(blockNumber < block.number, "Ecop::getPriorVotes: not yet determined");
    ...
    function _transferTokens(address src, address dst, uint96 amount) internal {
        require(src != address(0), "Ecop::_transferTokens: cannot transfer from the zero address");
        require(dst != address(0), "Ecop::_transferTokens: cannot transfer to the zero address");

        balances[src] = sub96(balances[src], amount, "Ecop::_transferTokens: transfer amount exceeds balance");
        balances[dst] = add96(balances[dst], amount, "Ecop::_transferTokens: transfer amount overflows");
    ...
    function _moveDelegates(address srcRep, address dstRep, uint96 amount) internal {
        if (srcRep != dstRep && amount > 0) {
            if (srcRep != address(0)) {
                ...
                uint96 srcRepNew = sub96(srcRepOld, amount, "Ecop::_moveVotes: vote amount underflows");
            ...
            if (dstRep != address(0)) {
                ...
                uint96 dstRepNew = add96(dstRepOld, amount, "Ecop::_moveVotes: vote amount overflows");
    ...
    function _writeCheckpoint(address delegatee, uint32 nCheckpoints, uint96 oldVotes, uint96 newVotes) internal {
      uint32 blockNumber = safe32(block.number, "Ecop::_writeCheckpoint: block number exceeds 32 bits");
