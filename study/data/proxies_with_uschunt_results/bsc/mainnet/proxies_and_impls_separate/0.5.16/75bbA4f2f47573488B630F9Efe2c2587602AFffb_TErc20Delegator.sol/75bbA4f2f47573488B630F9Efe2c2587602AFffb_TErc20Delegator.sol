...
contract TErc20Delegator is TTokenInterface, TErc20Interface, CDelegatorInterface {
   ...
   /**
    ...
    function showBorrowList() external view returns(address[] memory){
       bytes memory data = delegateToViewImplementation(abi.encodeWithSignature("showBorrowList()"));
    ...
    /**
      ...
      */
    function _setTENTroller(TENTrollerInterface newTENTroller) public returns (uint) {
        bytes memory data = delegateToImplementation(abi.encodeWithSignature("_setTENTroller(address)", newTENTroller));
        return abi.decode(data, (uint));
    }
