// File: JNTRb/contracts/DumperShield/Splitter.sol
...
library Splitter {
    ...
    function claim(SplitterSet storage set, address user) internal returns (uint256 valueA, uint256 valueB) {
        ...
        if (set.tokenARatesShares[set.index] <= rateA) {  // withdraw both tokens and reduce shares
            ...
            //require(rateA >= set.tokenARate, "rateA");

            valueA = (rateA - set.tokenARate)*shares / 1 ether;

            //require(set.tokenBRate >= d.tokenBRate, "rateB");

            valueB = (set.tokenBRate - d.tokenBRate)*shares / 1 ether;

            //require(set.totalShares >= shares, "shares");

            set.totalShares -= shares;

            //require(set.sharesOrRateB[rateA] >= shares, "sharesOrRateB<shares");

            set.sharesOrRateB[rateA] -= shares;  // reduce shares for this rate

        } else {    // all tokens A where sold

            //require(set.sharesOrRateB[rateA] >= d.tokenBRate, "sharesOrRateB<tokenBRate");
        ...
        //require(set.totalTokenA >= valueA, "totalTokenA");
    ...
    function _transfer(SplitterSet storage set, address user, uint256 valueB) internal {
        ...
        //require(set.totalTokenB >= valueB, "_transfer");
    ...
    function swap(SplitterSet storage set, uint256 valueA, uint256 valueB) internal {
        ...
        while (valueA > 0) {
            ...
            if (valueA >= amount) {
                ...
                _tokenARate = _tokenARate + (valueA * 1 ether / _totalShares) + 1;
...
// File: JNTRb/contracts/DumperShield/Voting.sol
...
// File: JNTRb/contracts/DumperShield/DumperShieldToken.sol
...
// File: JNTRb/contracts/DumperShield/DumperShield.sol
...
// end debug
    ...
    function cancelOrder(uint256 orderId) external {
        ...
        restrictedBalance[o.tokenSell][o.seller] += o.sellAmount;    // transfer to restricted account
    ...
    function _transfer(address token, address from, address to, uint256 value) internal returns (bool) {
        ...
        if (from == address(this)) {
            ...
            (,uint256 available) = balanceOf(token, from);
