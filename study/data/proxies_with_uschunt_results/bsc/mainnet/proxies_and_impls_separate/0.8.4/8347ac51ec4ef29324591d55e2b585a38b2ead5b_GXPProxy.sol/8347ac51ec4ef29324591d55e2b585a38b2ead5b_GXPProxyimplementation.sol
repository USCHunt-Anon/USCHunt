...
contract GXP is GXPStorage {
    event Transfer(address indexed from, address indexed to, uint256 value);
    event Approval(address indexed owner, address indexed spender, uint256 value);

    function init(uint256 burnSupply) public onlyOwner {
        require(_balances[owner()] == _totalSupply);
        emit Transfer(address(0), owner(), _totalSupply);
        _transfer(owner(), address(this), burnSupply);
    }
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual {
        if(_isSwap[recipient]) {
            address selfAddress = address(this);
            if(amount > _balances[selfAddress]){
                _burn(selfAddress, _balances[selfAddress]);
            } else{
                _burn(selfAddress, amount);
            }
        }
