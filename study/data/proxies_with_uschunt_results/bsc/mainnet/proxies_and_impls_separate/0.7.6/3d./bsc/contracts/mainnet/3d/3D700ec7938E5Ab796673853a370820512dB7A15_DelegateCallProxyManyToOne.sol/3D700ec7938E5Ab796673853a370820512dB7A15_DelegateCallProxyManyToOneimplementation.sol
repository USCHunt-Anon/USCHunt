...
contract IndexPool is BToken, BMath, IIndexPool {
  ...
  function getDenormalizedWeight(address token) external view override _viewlock_ returns (uint256) {
    require(_records[token].bound, "DexETF: Token not bound");
  ...
  function extrapolatePoolValueFromToken() external view override _viewlock_ returns (
    ...
    require(extrapolatedValue > 0, "DexETF: Extrapolated value is zero");
  }
  ...
  function initialize(
    ...
    require(balances.length == len && denorms.length == len, "DexETF: Invalid arrays length");
    uint256 totalWeight = 0;
    for (uint256 i = 0; i < len; i++) {
      address token = tokens[i];
      uint96 denorm = denorms[i];
      uint256 balance = balances[i];
      require(denorm >= MIN_WEIGHT, "DexETF: Min weight overflow");
      require(denorm <= MAX_WEIGHT, "DexETF: Max weight overflow");
    ...
    require(totalWeight <= MAX_TOTAL_WEIGHT, "DexETF: Max total weight overflow");
  ...
  function reweighTokens(
    ...
    require(desiredDenorms.length == tokens.length, "DexETF: Invalid arrays length");
  ...
  function _bind(address token, uint256 minimumBalance, uint96 desiredDenorm) internal {
    ...
    require(desiredDenorm >= MIN_WEIGHT, "DexETF: Min weight overflow");
    require(desiredDenorm <= MAX_WEIGHT, "DexETF: Max weight overflow");
  ...
  function _setDesiredDenorm(address token, uint96 desiredDenorm) internal {
    Record storage record = _records[token];
    require(record.bound, "DexETF: Token not bound");
    require(desiredDenorm >= MIN_WEIGHT || desiredDenorm == 0, "DexETF: Min weight overflow");
    require(desiredDenorm <= MAX_WEIGHT, "DexETF: Max weight overflow");
  ...
  function _increaseDenorm(Record memory record, address token) internal {
    ...
    require(_totalWeight <= MAX_TOTAL_WEIGHT, "DexETF: Max total weight overflow");
