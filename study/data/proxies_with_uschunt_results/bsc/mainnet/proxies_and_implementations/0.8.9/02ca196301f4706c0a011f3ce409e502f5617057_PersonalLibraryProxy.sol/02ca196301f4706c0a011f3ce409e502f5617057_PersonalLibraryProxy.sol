...
contract PersonalLibrary {
    ...
    function provideLiquidityAndStake(
        ...
        uint256 balance = _pl.amount;
            ...
            if(_pl.exchange > 0){
                ...
                ? convertTokenToToken(address(this), _pl.tokenAddress, _pl.pairAddress, _pl.amount, _pl.minPoolTokens)
                : convertETHToToken(address(this), _pl.pairAddress, _pl.amount, _pl.minPoolTokens);
            }
        ...
        if(_wl.exchange > 0){
            ...
            if(_balance > 0) {
                if(_wl.toTokenAddress != address(0)){
                    convertTokenToToken(_wl.toWhomToIssue, _wl.fromTokenAddress, _wl.toTokenAddress, _balance, _wl.minTokensRec);
                }else{
                    convertTokenToETH(_wl.toWhomToIssue, _wl.fromTokenAddress, _balance, _wl.minTokensRec);
                }
            }
    ...
    */
    function claimRewardNativeTokens(address[] memory _tokens) strategistOrInvestor public {
        ...
        for (uint256 i; i < _tokens.length; i++){
            ...
            ethToDevelopment = convertTokenToETH(_factory.developmentFund(), _tokens[i], ethToDevelopment, 1);
            yieldBurn = convertTokenToToken(address(this), _tokens[i], yieldToken, yieldBurn, 1);
    ...
    @param _tokens array of reward token addresses to claim
    */
    function claimRewardYIELDTokens(address[] memory _tokens, uint256 _minTokensRec) strategistOrInvestor public {
     
        //TODO: add min tokens variable check here. We can keep 1 in convertTokenToToken(), cause we we'll have one common check
        //this is medium priority, search key mintokn1024

        uint256 rewardAmount = convertRewardTokensToYIELD(address(this), _tokens, _minTokensRec);
        ...
        /*uint256 ethToDevelopmentFund = */convertTokenToETH(_factory.developmentFund(), yieldToken, yieldToDevelopmentFund, 1);
    ...
    function stopLossExit(
        ...
        if(_tokens.length > 0){
            for (uint256 i; i < _tokens.length; i++){
                ...
                if( _wl.toTokenAddress == address(0)){
                    convertTokenToETH(investor, _tokens[i], _balance, 1);//1 is ok here
                }else{
                    convertTokenToToken(investor, _tokens[i], _wl.toTokenAddress,  _balance, 1);//1 is ok here
                }
            }      
    ...
    */
    ...
    @return Amount of LP bought
    */
    function stakeReward(address[] memory _tokens, uint256 _minTokensRec) strategistOrInvestor public returns (uint256) {
        ...
        uint256 amountOfTokens = convertRewardTokensToYIELD(address(this), _tokens, _minTokensRec);
        //No revert, due to function unstakeAndWithdrawLiquidityAndClaimReward
    ...
    */
    function convertTokenToToken(address _toWhomToIssue, address _tokenToExchange, address _tokenToConvertTo, uint256 _amount, uint256 _minOutputAmount) inFunction internal returns (uint256) {       
    ...
    @param _amount how much will be converted
    */
    function convertTokenToETH(address _toWhomToIssue, address _tokenToExchange, uint256 _amount, uint256 _minOutputAmount) inFunction internal returns (uint256)  {

        //TokenConversionLibrary == ITokenConversionLibrary(IFactory(factory).tokenConversionLibrary())
        //return TokenConversionLibrary.convertTokenToETH(factory, _toWhomToIssue, _tokenToExchange, _amount, _minOutputAmount);
        ...
        /*if(_tokenToExchange == networkNativeToken){
            ...
            _amount = convertTokenToToken(address(this), _tokenToExchange, yieldToken,  _amount, 1);//min tokens doesn't matter here, we'll check in the last conversion
    ...
    */
    function convertETHToToken(address _toWhomToIssue, address _tokenToExchange, uint256 _amount, uint256 _minOutputAmount) inFunction internal returns (uint256)  {

        //ITokenConversionLibrary(IFactory(factory).tokenConversionLibrary())
        //return TokenConversionLibrary.convertETHToToken(factory, _toWhomToIssue, _tokenToExchange, _amount, _minOutputAmount);

        bool status;
        bytes memory result;
        (status, result) = IFactory(factory).tokenConversionLibrary().delegatecall(abi.encodeWithSignature(
            "convertETHToToken(address,address,address,uint256,uint256)",  
            factory,
            _toWhomToIssue, 
            _tokenToExchange, 
            _amount, 
        ...
        require(status, 'convertETHToToken call failed');
    ...
    */
    function convertRewardTokensToYIELD(address _toWhomToIssue, address[] memory _tokens, uint256 _minTokensRec) internal returns (uint256) {
        ...
        for (uint256 i; i < _tokens.length; i++){
            ...
            if(_tokens[i] != yieldToken){
                ...
                if(b > 0){
                    amount += convertTokenToToken(_toWhomToIssue, _tokens[i], yieldToken, b, 1);
                }
            }
        }
