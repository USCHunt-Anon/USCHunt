...
contract IDOV3 is IIDOV3, UUPSUpgradeable, ReentrancyGuardUpgradeable, ERC165 {
        ...
        updateStakingPower(_buyParams.idsToUpdate)
    {
        ...
        uint amountInToken = _buyParams.amountInBuyToken * exclusiveRound.priceTokenPerBuyTokenInUQ / UQ112x112.Q112;
        ...
        maxAllocation += _getAllocation(_buyParams, isProofValid, amountInToken);
    ...
    function _getAllocation(BuyParams calldata _buyParams, bool _isProofValid, uint _amountInToken) private returns (uint _maxAllocation) {
        IStaking.InfoAccountDetails memory stakingDetails = IStaking(staking).info(msg.sender);
        ...
        if (block.timestamp < exclusiveRound.maxEndTimestamp) { // if exclusive round
            ...
            require(stakingPower >= requiredPower || _isProofValid, "IDOV3: INVALID_STAKING_POWER");
            uint lotteryTickets = victoryTicketsCount(msg.sender);
            if (lotteryTickets > 0) {
                _maxAllocation = (lotteryTickets * allocationPerTicketInBuyToken) * exclusiveRound.priceTokenPerBuyTokenInUQ / UQ112x112.Q112;
                IStaking(staking).setLastParticipationDate(msg.sender, block.timestamp);
            }
            emit PrivateRoundBuy(msg.sender, _buyParams.amountInBuyToken, _amountInToken);
        } else {
            bool isEnoughStakingPower = stakingPower >= openRoundMinStakingPower;
            require(isEnoughStakingPower || _isProofValid, "IDOV3: INVALID_STAKING_POWER");
            if (isEnoughStakingPower) {
                _maxAllocation = maxAllocationInToken;
            }
            emit PublicRoundBuy(msg.sender, _buyParams.amountInBuyToken, _amountInToken);
        }
    }
