...
contract SpaceNFT is ERC721Full, ERC721MetadataMintable, Ownable {
  ...
  address payable public cardSell;
  ...
  constructor() ERC721Full("SpaceNFT", "SpaceNFT") public {
    ...
    function initialize2() public{
     platform =  0x4711788bd1Bd1c7E25da8b4Bc37D8b8ac6407B3F;
     cardSell = 0x3848b74c1366Ee5aC2bF85Bac01DC2e60E230F73;
  }
 ...
 function placeBid(uint256 _tokenId) public payable returns (bool) {
  ...
  function buyNFT(address nft_a,uint256 tokenId, address buyer) public payable {
    ...
    function buyCard() public payable {
		...
		require(spaceCard <= 2900, "Sorry we have sold out"); 
		buy(msg.sender);
		cardSell.transfer(msg.value);
	}
	...
	function giveAway(address[] memory user,uint256 quantity) public onlyOwner {
		require(quantity <= 10, "Max 10 cards at once");
		for (uint256 i = 0; i < quantity; i++) {
			buy(user[i]);
		}
	...
	function buy(address user) private {
    ...
    _burn(ownerOf(tokenId), tokenId);
    if(_saleTokens[tokenId].seller != address(0)){
        _saleTokens[tokenId].seller = address(0);
     } else{
         require(_auctionTokens[tokenId].bidder == address(0), "111");
        _auctionTokens[tokenId].seller = address(0);
      }
    ...
    if(_saleTokens[tokenId].seller != address(0)){
        ...
        require(_auctionTokens[tokenId].bidAmount == 0, "112");
