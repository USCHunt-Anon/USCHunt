...
contract Comptroller is ComptrollerV5Storage, ComptrollerInterface, ComptrollerErrorReporter, ExponentialNoError {
 ...
 /**
     ...
     * @param lpVaultReleaseStartBlock_ The start block of release to LP Vault
     * @param lpVaultMinReleaseAmount_ The minimum release amount to LP Vault
     */
    function _setLPVaultInfo(address vault_, uint256 lpVaultReleaseStartBlock_, uint256 lpVaultMinReleaseAmount_) public {
        require(msg.sender == admin, "only admin can set LP vault info");

        lpVaultAddress = vault_;
        lpVaultReleaseStartBlock = lpVaultReleaseStartBlock_;
        lpVaultMinReleaseAmount = lpVaultMinReleaseAmount_;
        emit NewLPVaultInfo(vault_, lpVaultReleaseStartBlock_, lpVaultMinReleaseAmount_);
    }
    ...
    function releaseToVault() public {
        if(lpVaultReleaseStartBlock == 0 || getBlockNumber() < lpVaultReleaseStartBlock) {
            return;
        }
        ...
        uint256 deltaBlocks = sub_(getBlockNumber(), lpVaultReleaseStartBlock);
        // releaseAmount = atlantisLPVaultRate * deltaBlocks
        uint256 _releaseAmount = mul_(atlantisLPVaultRate, deltaBlocks);

        if (_releaseAmount < lpVaultMinReleaseAmount) {
            return;
        }
        ...
        lpVaultReleaseStartBlock = getBlockNumber();
    ...
    function getAtlantisAddress() public view returns (address) {
        return 0xb6e19314E10eBc90173A155e228d17545187B05B;
    }
}
