...
//import "../../../LendingNetwork/Delegators/OMasterApeStakerDelegator.sol";

contract OMasterApeStakerDelegatorDeployer {
    function deployODelegator(
        ...
        OMasterApeStakerDelegator oMasterApeStakerDelegator = new OMasterApeStakerDelegator(
                underlying_,
                stakingTarget_,
                comptroller_,
        ...
        address oMasterApeStakerDelegatorAddress = address(oMasterApeStakerDelegator);

        return oMasterApeStakerDelegatorAddress;
    }
}
...
/**
 * @title Ola's OMasterApeStakerDelegator Contract
 * @notice OTokens which wrap an EIP-20 underlying and delegate to an implementation of a 'MasterApeStaker'
 * @author Ola
 */
contract OMasterApeStakerDelegator is ODelegator, CTokenInterface, CErc20Interface {
    // OLA_ADDITIONS : This contract name hash
    bytes32 constant public OMasterApeStakerDelegatorContractHash = keccak256("OMasterApeStakerDelegator");
    ...
    constructor(address underlying_,
        ...
        contractNameHash = OMasterApeStakerDelegatorContractHash;

        address oMasterApeStakerImplementation = RegistryForODelegator(comptroller_.getRegistry()).getImplementationForLn(address(comptroller_), OMasterApeStakerDelegatorContractHash);

        // First delegate gets to initialize the delegator (i.e. storage contract)
        delegateTo(oMasterApeStakerImplementation, abi.encodeWithSignature("initialize(address,address,address,uint256,string,string,uint8)",
                                                            underlying_,
                                                            stakingTarget_,
                                                            comptroller_,
                                                            initialExchangeRateMantissa_,
        ...
        // New implementations always get set via the setter (post-initialize)
        _setImplementation(oMasterApeStakerImplementation, false, becomeImplementationData);
   ...
   /**
     ...
     */
    function () external payable {
        require(msg.value == 0,"OMasterApeStakerDelegator:fallback: cannot send value to fallback");
