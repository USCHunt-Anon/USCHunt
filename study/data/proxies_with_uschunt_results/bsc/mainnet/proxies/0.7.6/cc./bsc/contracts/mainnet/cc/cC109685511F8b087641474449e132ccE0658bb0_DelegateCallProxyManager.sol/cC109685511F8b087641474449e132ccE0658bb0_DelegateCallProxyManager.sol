...
contract DelegateCallProxyManager is Ownable, IDelegateCallProxyManager {
  ...
  function isImplementationLocked(bytes32 implementationID) external override view returns (bool) {
    ...
    require(implementationHolder != address(0), "DexETF: Invalid implementation id");
  ...
  function setImplementationAddressManyToOne(
    ...
    address implementationHolder = _implementationHolders[implementationID];
    require(implementationHolder != address(0), "DexETF: Invalid implementation id");
    require(!_lockedImplementations[implementationHolder], "DexETF: Implementation is locked");
    _setImplementation(implementationHolder, implementation);
  ...
  function setImplementationAddressOneToOne(
    ...
    require(!_lockedImplementations[proxyAddress], "DexETF: Implementation is locked");
  ...
  function deployProxyManyToOne(
    ...
    address implementationHolder = _implementationHolders[implementationID];
    require(implementationHolder != zero, "DexETF: Invalid implementation id");
    bytes32 salt = SaltyLib.deriveManyToOneSalt(
      _msgSender(),
      implementationID,
      suppliedSalt
  ...
  function _setImplementation(
    ...
    require(Address.isContract(implementation), "DexETF: Implementation is not contract");
    // solium-disable-next-line security/no-low-level-calls
    (bool success,) = proxyOrHolder.call(abi.encode(implementation));
    require(success, "DexETF: Address setting fail");
  }

  modifier onlyApprovedDeployer {
    address sender = _msgSender();
    require(_approvedDeployers[sender] || sender == owner(), "DexETF: Deployer no approved");
    _;
  }
}
