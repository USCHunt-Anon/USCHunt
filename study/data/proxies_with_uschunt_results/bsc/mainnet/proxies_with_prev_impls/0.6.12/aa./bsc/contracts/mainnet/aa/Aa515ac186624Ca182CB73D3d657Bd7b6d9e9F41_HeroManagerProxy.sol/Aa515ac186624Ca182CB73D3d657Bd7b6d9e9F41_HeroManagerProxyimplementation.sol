...
/////////////////////////////// UpdateHero
    ...
    function updateHero(
        uint256 heroId, 
        uint256 amountToHero
        ) external onlyHeroOwner(heroId) heroIsDungeon(heroId){
        ...
        require(
            currencyToHero <= allowedHeroTeams[currencyId].maxAmount,
            "HeroManager::createHero: amountToHero so big"
        );
...
/////////////////////////////// CorectHero
    function corectHeroes(uint256[] memory heroesId, uint256[] memory heroesBody) public onlyOwner{
        require(heroesId.length == heroesBody.length);
        for (uint256 index = 0; index < heroesId.length; index++) { 
            uint256 currencyToHero =  IStorage(_HeroStorage).get(heroesId[index], AMOUNT_CYRRENCY_TO_HERO);
            __subTokenBalance(_getHeroTeamAddress(IStorage(_HeroStorage).get(heroesId[index], HERO_TYPE)), heroesId[index], currencyToHero);
            __addTokenBalance(_getHeroTeamAddress(IStorage(_HeroStorage).get(heroesId[index], HERO_TYPE)), heroesId[index], heroesBody[index]);
            IStorage(_HeroStorage).set(heroesId[index], AMOUNT_CYRRENCY_TO_HERO, heroesBody[index]);
        }
    }

/////////////////////////////// IncentivesPool
    function setAddressIncentivesPool(address polladdress) public onlyOwner{
        _incentivesPool  = polladdress;
    }
...
contract Treasury {
    ...
    function __addTokenBalance(address erc20Address, uint256 tokenId, uint256 amount) internal {
        getTokenBalance[erc20Address][tokenId] = getTokenBalance[erc20Address][tokenId].add(amount);
    }

    function __subTokenBalance(address erc20Address, uint256 tokenId, uint256 amount) internal {
...
interface IStorage is IBalancesLikeERC1155, IERC721, IERC721Metadata, IERC721Enumerable{
   ...
   function issoul(uint256 tokenId) external view returns(bool);

   function getDungeonAddress(uint256 tokenId) external view returns(address);
