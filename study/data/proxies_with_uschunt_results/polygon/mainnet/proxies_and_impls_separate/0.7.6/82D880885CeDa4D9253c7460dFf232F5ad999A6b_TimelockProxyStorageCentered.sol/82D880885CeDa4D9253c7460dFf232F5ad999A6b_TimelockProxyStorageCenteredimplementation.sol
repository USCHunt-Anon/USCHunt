...
interface ISwapCenter {

    function setReferalAddress(address _newReferral) external;
    function isETH(address token) external pure returns (bool);
    function getExchangeOrder(address tokenIn, address tokenOut) external view returns (uint256[] memory);
    function getPath(address tokenIn, address tokenOut) external view returns (address[][] memory);
    function setRoute(address input, address output, uint256[] memory exchangeOrder, address[][] memory path) external;
    function setRouteBatch(address[] memory input, address[] memory output, uint256[][] memory exchangeOrder, address[][][] memory path) external;
...
interface IMiniChefV2 {
  function userInfo(uint256 _pid, address _user) external view returns (uint256 amount, uint256 rewardDebt); // amount: balance of LP, rewardDebt: amount of SUSHI entitled to the user
  function rewarder(uint256 _pid) external view returns (address _rewarder); // wmatic rewarder address
  function pendingSushi(uint256 _pid, address _user) external view returns (uint256 pending);
  function deposit(uint256 pid, uint256 amount, address to) external; // stake
  function withdraw(uint256 pid, uint256 amount, address to) external; // unstake
  function harvest(uint256 pid, address to) external; // claim rewards
  function withdrawAndHarvest(uint256 pid, uint256 amount, address to) external; // unstake + claim rewards
  function emergencyWithdraw(uint256 pid, address to) external; // withdraw w/o claiming rewards
...
pragma solidity 0.7.6;
interface IRewarder {
  function pendingToken(uint256 pid, address user) external view returns (uint256 pending);
  function pendingTokens(uint256 pid, address user, uint256) external view returns (address[] memory rewardTokens, uint256[] memory rewardAmounts);
}
...
//import"../../liquidate/interface/ISwapCenter.sol";
//import"../../liquidate/external/IUniswapV2Router02.sol";
//import"../../liquidate/external/IUniswapV2Pair.sol";
//import"../InvestmentVehicleSingleAssetBaseV1Upgradeable.sol";
//import"./interface/IMiniChefV2.sol";
//import"./interface/IRewarder.sol";

/**
    MiniChefV2V1BasePolygon is the IV implementation that targets SUSHI MiniChefV2.
    It takes the base asset and stakes into minichef
*/
contract MiniChefV2V1BasePolygon is InvestmentVehicleSingleAssetBaseV1Upgradeable {
    ...
    mapping(address => bool) public rewardTokenMapping;
    address [] public rewardTokenList;

    /// initialize the iv with sushi minichef and its respective base asset
    /// @param _store the address of system storage
    /// @param _baseAsset the address of base asset
    /// @param _pid the pid of the sushi LP pair
    function initialize(
        address _store,
        address _baseAsset,
        uint256 _pid
        ...
        pid = _pid;
        token0 = IUniswapV2Pair(_baseAsset).token0();
        token1 = IUniswapV2Pair(_baseAsset).token1();

        require(IMiniChefV2(minichef).lpToken(_pid) == _baseAsset, "Unmatched pid and lpToken");
        _addRewardToken(wmatic);
        _addRewardToken(sushi);
    }

    function addRewardToken(address _token) public adminPriviledged {
        _addRewardToken(_token);
    }

    function _addRewardToken(address _token) internal {
        require(rewardTokenMapping[_token] == false, "token already exists");
        rewardTokenList.push(_token);
        rewardTokenMapping[_token] = true;
    }

    function removeRewardToken(uint256 _tokenId) public adminPriviledged {
        address rewardToken = rewardTokenList[_tokenId];
        rewardTokenList[_tokenId] = rewardTokenList[rewardTokenList.length - 1];
        rewardTokenList.pop();
        rewardTokenMapping[rewardToken] = false;
    }
    ...
    function _investAll() internal override {
        ...
        if(baseAssetAmountInVehicle > 0){
            // Approve minichef
            IERC20Upgradeable(baseAsset).safeApprove(minichef, 0);
            IERC20Upgradeable(baseAsset).safeApprove(minichef, baseAssetAmountInVehicle);
            // Deposit/Stake
            IMiniChefV2(minichef).deposit(pid, baseAssetAmountInVehicle, address(this));
        }
    }

    /** Interacting with underlying investment opportunities */
    function _pullFundsFromInvestment(uint256 _baseAmount) internal override{
        uint256 owned = totalStakedLPAmount();
        uint256 withdrawing = MathUpgradeable.min(owned, _baseAmount);

        IMiniChefV2(minichef).withdraw(pid, withdrawing, address(this));
    }

    function _collectProfitAsBaseAsset() internal override returns (uint256) {
        uint256 baseAssetAmountInVehicle = IERC20Upgradeable(baseAsset).balanceOf(address(this));

        // Collect SUSHI and WMATIC
        ...
        for(uint256 i = 0 ; i < rewardTokenList.length; i++) {
            ...
            // We swap all rewards into WETH
            if(rewardAmount > 0) {
                IERC20Upgradeable(rewardToken).approve(swapCenter(), 0);
                IERC20Upgradeable(rewardToken).approve(swapCenter(), rewardAmount);
                ISwapCenter(swapCenter()).swapExactTokenIn(rewardToken, weth, rewardAmount, 0);
            }
        }

        // Swap half WETH for token0
        uint256 _wethAmount = IERC20Upgradeable(weth).balanceOf(address(this));
        uint256 _wethAmountHalf = _wethAmount.div(2);

        IERC20Upgradeable(weth).approve(swapCenter(), 0);
        IERC20Upgradeable(weth).approve(swapCenter(), _wethAmount);

        if (_wethAmount > 0 && token0 != weth) {   
            ISwapCenter(swapCenter()).swapExactTokenIn(weth, token0, _wethAmountHalf, 0);
        }
        // Swap half WETH for token1
        if (_wethAmount > 0 && token1 != weth) {
            ISwapCenter(swapCenter()).swapExactTokenIn(weth, token1, _wethAmountHalf, 0);
        }

        // Adds in liquidity for token0/token1
        uint256 _token0Amount = IERC20Upgradeable(token0).balanceOf(address(this));
        uint256 _token1Amount = IERC20Upgradeable(token1).balanceOf(address(this));
        console.log(_token0Amount);
        console.log(_token1Amount);
        if (_token0Amount > 0 && _token1Amount > 0) {
            IERC20Upgradeable(token0).safeApprove(sushiRouter, 0);
            IERC20Upgradeable(token0).safeApprove(sushiRouter, _token0Amount);
            IERC20Upgradeable(token1).safeApprove(sushiRouter, 0);
            IERC20Upgradeable(token1).safeApprove(sushiRouter, _token1Amount);
            ...
            IUniswapV2Router02(sushiRouter).addLiquidity(
                ...
                _token0Amount,
                _token1Amount,
                0, // we allow any token0 or token1 to be added as liquidity, thus no requirement on min
                0,
                address(this),
                block.timestamp
            );
            // Caveat: there might be some dust of token0 or token1, for next time use
        }

        return IERC20Upgradeable(baseAsset).balanceOf(address(this)).sub(baseAssetAmountInVehicle);      
    }

    /** View functions */

    /// exposes the amount of staked LP amount owned by this IV
    /// @return totalAmount of the staked LP tokens that is owned by this IV
    function totalStakedLPAmount() public view returns (uint256 totalAmount) {
        (totalAmount, ) = IMiniChefV2(minichef).userInfo(pid, address(this));
    }

    /// calculates the amount of base asset that is deposited into minichef
    /// @return amount of base asset in minichef
    function invested() public view override returns (uint256){
        return totalStakedLPAmount();
    }
    ...
    /// @return profitInETH the amount of profit that has not been accounted yet
    function profitsPendingInEth() public view returns (uint256 profitInETH) {
        uint256 _pendingSUSHI = IMiniChefV2(minichef).pendingSushi(pid, address(this));
        address _rewarder = IMiniChefV2(minichef).rewarder(pid);
        uint256 _pendingWMATIC = IRewarder(_rewarder).pendingToken(pid, address(this));

        // denominated in ETH for gas calculation
        address[] memory path = new address[](2);
        path[0] = sushi;
        path[1] = weth;

        // swapResult[0] is the input token amount
        // swapResult[1] is the output of the first swap along the path and so on.
        // swapResult.length-1 refers to the last token out => output token amount
        uint256[] memory swapResult = IUniswapV2Router02(sushiRouter).getAmountsOut(_pendingSUSHI, path);
        profitInETH = profitInETH.add(swapResult[swapResult.length-1]);

        path[0] = wmatic; // path is [wmatic, weth] so no need to change path[1]
        swapResult = IUniswapV2Router02(sushiRouter).getAmountsOut(_pendingWMATIC, path);
        profitInETH = profitInETH.add(swapResult[swapResult.length-1]);
    }

    function profitsPending() public override pure returns (uint256) {
        // Denominating in baseAsset is troublesome when dealing with LPToken
        // thus in this case we will return an invalid number
        // bot and external viewers should use other methods such as
        // profitsPendingInEth()
