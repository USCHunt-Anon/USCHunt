...
abstract contract CToken is CTokenInterface, Exponential, TokenErrorReporter {
    ...
    ) external override nonMultiActivity(msg.sender) returns (uint256) {
        uint256 error = accrueInterest();
        if (error != uint256(Error.NO_ERROR)) {
            // accrueInterest emits logs on errors, but on top of that we want to log the fact that an attempted reduce reserves failed.
...
contract Comptroller is ComptrollerV8Storage, ComptrollerInterface, ComptrollerErrorReporter, Exponential {
    ...
    function flashloanAllowed(
        ...
        return flashloanGuardianActivated[cToken];
    }
    ...
    function _setFlashloanActivated(CToken cToken, bool state) public returns (bool) {
        require(isMarketListed(address(cToken)), "cannot pause a market that is not listed");
        require(msg.sender == pauseGuardian || msg.sender == admin, "only pause guardian and admin can pause");
        require(msg.sender == admin || state == true, "only admin can unpause");

        flashloanGuardianActivated[address(cToken)] = state;
