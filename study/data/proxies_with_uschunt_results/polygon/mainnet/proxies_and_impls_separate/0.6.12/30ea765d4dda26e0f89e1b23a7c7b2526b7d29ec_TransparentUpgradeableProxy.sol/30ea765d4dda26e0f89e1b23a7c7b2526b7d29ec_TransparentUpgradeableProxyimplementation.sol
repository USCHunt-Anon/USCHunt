...
contract POLYPAD is IERC20, OwnableUpgradeSafe, LGEWhitelisted {
	...
	uint256 public _feeRewardsPct;
	address public _feeRewardsAddress;
	
	uint256 public _feeRewardsLTPct;
	address public _feeRewardsLTAddress;
	...
	event Distributed(uint256 feeRewardsAmount, uint256 feeRewardsLTAmount);
    
    function initialize(
                address router,
                uint256 feeBurnPct, 
                uint256 feeRewardsPct, 
                address feeRewardsAddress, 
                uint256 feeRewardsLTPct, 
                address feeRewardsLTAddress
        )
        public
        initializer
    {
    ...
    function setFees(
		...
		require((feeBurnPct + feeRewardsPct + feeRewardsLTPct) <= 2000, "Fees must not total more than 20%");
		
		require(feeRewardsAddress != address(0), "Fund address must not be zero address");
		require(feeRewardsLTAddress != address(0), "Reward address must not be zero address");
		
		_feeBurnPct = feeBurnPct;
		
		_feeRewardsPct = feeRewardsPct;
		_feeRewardsAddress = feeRewardsAddress;
		
		_feeRewardsLTPct = feeRewardsLTPct;
		_feeRewardsLTAddress = feeRewardsLTAddress;
    ...
    function collectedFees() public view returns (uint256 totalAmount, uint256 feeRewardsAmount, uint256 feeRewardsLTAmount) {
            ...
            if(totalAmount > 0) {
                ...
                uint256 totalPct = _feeRewardsPct + _feeRewardsLTPct;
            
                feeRewardsAmount = totalAmount.mul(_feeRewardsPct).div(totalPct);                
                feeRewardsLTAmount = totalAmount.sub(feeRewardsAmount);
    ...
    function distributeFees() public onlyOwner {
        ...
        (uint256 totalAmount, uint256 feeRewardsAmount, uint256 feeRewardsLTAmount) = collectedFees();
        ...
        if(totalAmount > 0) {
            ...
            if(_reserveSwapPath[_reserveSwapPath.length-1] == address(weth)) {
                ...
                if(feeRewardsAmount > 0)
                    TransferHelper.safeTransferETH(_feeRewardsAddress, feeRewardsAmount);
                if(feeRewardsLTAmount > 0)    
                    TransferHelper.safeTransferETH(_feeRewardsLTAddress, feeRewardsLTAmount);
            
            } else {
                
                if(feeRewardsAmount > 0)
                    _reserveTokenPool.transfer(_feeRewardsAddress, feeRewardsAmount);
                if(feeRewardsLTAmount > 0)
                    _reserveTokenPool.transfer(_feeRewardsLTAddress, feeRewardsLTAmount);
                
            }
            
            emit Distributed(feeRewardsAmount, feeRewardsLTAmount);
	...
	function _transfer(address sender, address recipient, uint256 amount) internal {
		...
		if(_pair[recipient] && !_feeExcluded[sender]) {
			...
			uint256 feeToReservePct = _feeRewardsPct + _feeRewardsLTPct;
			...
			if(feeToReservePct > 0)  {
				...
				if(_router != address(0)) {
				    ...
				    _balances[_feeRewardsAddress] = _balances[_feeRewardsAddress].add(feeToReserveAmount);
				    emit Transfer(sender, _feeRewardsAddress, feeToReserveAmount);
				}
