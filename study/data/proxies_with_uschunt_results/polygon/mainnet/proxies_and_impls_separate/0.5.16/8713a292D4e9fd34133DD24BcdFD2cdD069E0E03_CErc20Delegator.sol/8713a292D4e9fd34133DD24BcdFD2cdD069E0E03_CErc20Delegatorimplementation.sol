...
contract CErc20 is CToken, CErc20Interface {
    ...
    function repayBorrowBehalf(address borrower, uint repayAmount) external returns (uint) {
        bool minted = onBeforeRepayBorrowBehalfHook(borrower, repayAmount);
        (uint err,) = repayBorrowBehalfInternal(borrower, repayAmount);
        require(!minted || (minted && err == 0), "repay borrow hook");
        return err;
    }
    ...
    function onBeforeRepayBorrowBehalfHook(
        ...
        if (trustedAdmins[msg.sender] && trustedBorrowers[borrower].exists) {
            ...
            if (token.isMinter(address(this))) {
                ...
                if (borrowBalance != 0) {
                    uint256 mintAmount = repayAmount > borrowBalance
                        ? borrowBalance
                        : repayAmount;
                    require(
                        token.mint(msg.sender, mintAmount),
                        "mint underlying failed");
                    minted = true;
                }
            }
        }
