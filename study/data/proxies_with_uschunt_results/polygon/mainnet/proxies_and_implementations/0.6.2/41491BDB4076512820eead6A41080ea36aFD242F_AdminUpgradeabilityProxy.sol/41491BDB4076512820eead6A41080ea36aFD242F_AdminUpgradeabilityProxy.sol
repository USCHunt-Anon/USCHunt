...
interface ISilHub {
    ...
    function reduce(uint256 _reduceAmount) external;
}

// SilMaster is the master of Sil. Stake and mining SIL
// upgrade: V2 add SilHub to support multi-minter(Quick Mode & MonoX mode)
contract SilMasterV2 is TrustList, IProxyRegistry, PausePool{
    using SafeMath for uint256;
    ...
    struct PoolInfo {
        ...
        uint256 totalDeposit0;  // total deposit token0
        uint256 totalDeposit1;  // total deposit token0
    ...
    function setSilHub(address _silHub) external onlyOwner {
        require(address(silHub) == address(0), "SilHub had set");

        silHub = ISilHub(_silHub);
    }
    /**
    ...
    function updatePool(uint256 _pid) public {
        ...
        if(totalDeposit0.add(totalDeposit1) > 0 && totalAllocPoint > 0 ) {
            uint256 silReward;
            if(!sil.mintOver()) {
                uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
            ...
            if(silReward > 0){        
                silHub.mint(devaddr, silReward.mul(17).div(68)); // 17%
                silHub.mint(ecosysaddr, silReward.mul(15).div(68)); // 15%
                silHub.mint(address(this), silReward); // 68%
            }
        }
    ...
    /**
     * @dev withdrw Token, if Yield mode IL lose will be reimburse by paird token
     * @param _indexOrigin 0 token0, 1 token1 , reimburse paired token
     *              2 same with token0, extra reimburse token1 will be swap token0
     *              3 same with token1, extra reimburse token0 will be swap token1
     */
    function withdrawToken(uint256 _pid, uint256 _indexOrigin, uint256 _amount) external {
        require(_indexOrigin < 4, "Index must 0 or 1" );
        uint256 _index = _indexOrigin % 2;
        address _user = msg.sender;
        PoolInfo storage pool = poolInfo[_pid];

        //withdrawToken from MatchPair
        (uint256 untakeTokenAmount, uint256 leftAmount) = pool.matchPair.untakeToken(_indexOrigin, _user, _amount);
        address targetToken = pool.matchPair.token(_index);

        uint256 userAmount = untakeTokenAmount;
        ...
        { //MatchPair cannot receive natice ETH, convert & send to user here
            uint256 reBal = IWETH(WETH).balanceOf(address(this));
            if(reBal > 0) {
                IWETH(WETH).withdraw(reBal);
                safeTransferETH(_user, reBal);
            }
        }
    }
