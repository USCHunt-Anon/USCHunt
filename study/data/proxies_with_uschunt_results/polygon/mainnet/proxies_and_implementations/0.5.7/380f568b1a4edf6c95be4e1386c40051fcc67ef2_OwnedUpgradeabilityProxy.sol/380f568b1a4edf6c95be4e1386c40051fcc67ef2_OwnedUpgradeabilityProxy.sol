...
contract SendFunds {
    ...
    function sendTokens(address _token, address[] memory _users, uint[] memory _amounts) public {
        ...
        for(uint i=0;i<_users.length;i++) {
            token.transferFrom(owner, _users[i], _amounts[i]);
        }
    }
    ...
    function sendbPLOTTokens(address _token, address[] memory _users, uint[] memory _amounts) public {
        require(msg.sender == owner);
        ERC20 token = ERC20(_token);
        for(uint i=0;i<_users.length;i++) {
