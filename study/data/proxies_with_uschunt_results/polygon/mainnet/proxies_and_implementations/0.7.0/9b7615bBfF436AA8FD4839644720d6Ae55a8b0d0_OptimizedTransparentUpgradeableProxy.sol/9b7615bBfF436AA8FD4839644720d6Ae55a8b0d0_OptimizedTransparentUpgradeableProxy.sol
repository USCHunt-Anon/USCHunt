...
/**
 *  SourceUnit: Protonauts-monorepo/contracts/node_modules/.pnpm/[email protected][email protected]/node_modules/hardhat-deploy/solc_0.7/proxy/OptimizedTransparentUpgradeableProxy.sol
 */
...
//import'../common/TokenDescriptor.sol';
//import'./interfaces/IEquipment.sol';
//import'./interfaces/IL2_SVG721.sol';

// //import'hardhat/console.s`ol';

contract L2TokenDescriptor is TokenDescriptor {
	IEquipment public Equipment;
	event SetEquipment(address);

	function setEquipment(address _Equipment) external onlyAdmin {
		Equipment = IEquipment(_Equipment);
		emit SetEquipment(_Equipment);
	}

	function imageURI(uint256 tokenId, uint256[2] memory indices)
		public
		view
		override
		returns (string memory image)
	{
		...
		uint256[] memory equipmentIds = IL2_SVG721(address(SVG721)).equipments(
			tokenId
		);
		for (uint256 index = 0; index < equipmentIds.length; index++) {
			b = abi.encodePacked(b, Equipment.pureImage(equipmentIds[index]));
		}
