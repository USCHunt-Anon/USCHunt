...
contract NFTProxy is UpgradeableProxy, Ownable {
     ...
     * @param _data - Encoded Function Call - delegates call to _logic | 0x8129fc1c [for initialize()]
     */
     ...
     *
     * @param _logic - Implementation/Logic Contract Address
     * @param _data - Encoded Function Call - delegates call to _logic
     */
    function upgradeTo(address _logic, bytes memory _data) external onlyOwner {
        _upgradeTo(_logic);
        if (_data.length > 0) {
            Address.functionDelegateCall(_logic, _data);
        }
