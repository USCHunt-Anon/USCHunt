...
contract Comptroller is ComptrollerStorage, ComptrollerInterface, ComptrollerErrorReporter, ExponentialNoError {
    ...
    function mintAllowed(address rToken, address minter, uint mintAmount) external returns (uint) {
        ...
        distributeSupplierComp(rToken, minter);
    ...
    function redeemAllowed(address rToken, address redeemer, uint redeemTokens) external returns (uint) {
        ...
        // Keep the flywheel moving
        updateCompSupplyIndex(rToken);
        distributeSupplierComp(rToken, redeemer);
    ...
    function borrowAllowed(address rToken, address borrower, uint borrowAmount) external returns (uint) {
        ...
        distributeBorrowerComp(rToken, borrower, borrowIndex);
    ...
    function seizeAllowed(
        ...
        distributeSupplierComp(rTokenCollateral, borrower);
        distributeSupplierComp(rTokenCollateral, liquidator);
    ...
    function transferAllowed(address rToken, address src, address dst, uint transfeRTokens) external returns (uint) {
        ...
        // Keep the flywheel moving
        updateCompSupplyIndex(rToken);
        distributeSupplierComp(rToken, src);
        distributeSupplierComp(rToken, dst);
     ...
     * @param rToken The market whose COMP speed to update
     * @param compSpeed New COMP speed for market
     */
    function setCompSpeedInternal(RToken rToken, uint compSpeed) internal {
        uint currentCompSpeed = compSpeeds[address(rToken)];
        if (currentCompSpeed != 0) {
            ...
            if (compSupplyState[address(rToken)].index == 0 && compSupplyState[address(rToken)].block == 0) {
                compSupplyState[address(rToken)] = CompMarketState({
                    index: compInitialIndex,
                    block: safe32(getBlockNumber(), "block number exceeds 32 bits")
                });
            }

            if (compBorrowState[address(rToken)].index == 0 && compBorrowState[address(rToken)].block == 0) {
                compBorrowState[address(rToken)] = CompMarketState({
                    index: compInitialIndex,
                    block: safe32(getBlockNumber(), "block number exceeds 32 bits")
                });
            }
        ...
        if (currentCompSpeed != compSpeed) {
            compSpeeds[address(rToken)] = compSpeed;
            emit CompSpeedUpdated(rToken, compSpeed);
        }
    }
    ...
    function updateCompSupplyIndex(address rToken) internal {
        ...
        uint supplySpeed = compSpeeds[rToken];
    ...
    function updateCompBorrowIndex(address rToken, Exp memory marketBorrowIndex) internal {
        ...
        uint borrowSpeed = compSpeeds[rToken];
        uint blockNumber = getBlockNumber();
     ...
     */
    function distributeSupplierComp(address rToken, address supplier) internal {
        CompMarketState storage supplyState = compSupplyState[rToken];
     ...
     */
    function distributeBorrowerComp(address rToken, address borrower, Exp memory marketBorrowIndex) internal {
        CompMarketState storage borrowState = compBorrowState[rToken];
        ...
        for (uint i = 0; i < rTokens.length; i++) {
            ...
            if (borrowers == true) {
                ...
                for (uint j = 0; j < holders.length; j++) {
                    distributeBorrowerComp(address(rToken), holders[j], borrowIndex);
            ...
            if (suppliers == true) {
                updateCompSupplyIndex(address(rToken));
                for (uint j = 0; j < holders.length; j++) {
                    distributeSupplierComp(address(rToken), holders[j]);
                    compAccrued[holders[j]] = grantCompInternal(holders[j], compAccrued[holders[j]]);
                }
            }
        }
    ...
    function grantCompInternal(address user, uint amount) internal returns (uint) {
        IShare comp = IShare(getCompAddress());
     ...
     * @param compSpeed New COMP speed for market
     */
    function _setCompSpeed(RToken rToken, uint compSpeed) public {
        require(adminOrInitializing(), "only admin can set comp speed");
        setCompSpeedInternal(rToken, compSpeed);
    }
    ...
    function getCompAddress() public pure returns (address) {
        return 0x7b4C66b45a4DdfE321Da8e5F6a0dAEd4269AF97C;
    }
}
...
contract IShare is EIP20Interface {
