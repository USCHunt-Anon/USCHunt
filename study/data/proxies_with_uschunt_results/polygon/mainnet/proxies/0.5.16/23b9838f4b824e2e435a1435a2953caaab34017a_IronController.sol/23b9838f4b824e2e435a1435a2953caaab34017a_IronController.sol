...
contract IronController is IronControllerStorage, IronControllerInterface, IronControllerErrorReporter, ExponentialNoError {
    ...
    function grantRewardInternal(address user, uint amount) internal returns (uint) {
        EIP20Interface reward = EIP20Interface(getRewardAddress());
        uint rewardRemaining = reward.balanceOf(address(this));
     ...
     * @param amount The amount of REWARD to (possibly) transfer
     */
    function _grantReward(address recipient, uint amount) public {
        require(adminOrInitializing(), "only admin can grant reward");
        uint amountLeft = grantRewardInternal(recipient, amount);
