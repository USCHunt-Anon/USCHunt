 ...
 * @author Iron
 */
contract IronController is
    IronControllerStorage,
    IronControllerInterface,
    IronControllerErrorReporter,
    ExponentialNoError
{
    ...
    event NewCollateralFactor(RToken rToken, uint256 oldCollateralFactorMantissa, uint256 newCollateralFactorMantissa);
    ...
    event NewBorrowCap(RToken indexed rToken, uint256 newBorrowCap);
    ...
    event NewIceCollateralRatio(uint256 oldRatio, uint256 newRatio);
    ...
    uint256 public iceCollateralRatio = 1e17;
     ...
     */
    function enterMarkets(address[] memory rTokens) public returns (uint256[] memory) {
        uint256 len = rTokens.length;

        uint256[] memory results = new uint256[](len);
        for (uint256 i = 0; i < len; i++) {
            RToken rToken = RToken(rTokens[i]);

            results[i] = uint256(addToMarketInternal(rToken, msg.sender));
        }
    ...
    function exitMarket(address rTokenAddress) external returns (uint256) {
        ...
        (uint256 oErr, uint256 tokensHeld, uint256 amountOwed, ) = rToken.getAccountSnapshot(msg.sender);
        ...
        uint256 allowed = redeemAllowedInternal(rTokenAddress, msg.sender, tokensHeld);
        if (allowed != 0) {
    ...
    function redeemAllowed(
        ...
        uint256 allowed = redeemAllowedInternal(rToken, redeemer, redeemTokens);
    ...
    function redeemAllowedInternal(
        ...
        if (getRIceAddress() == rToken) {
            (Error cErr, uint256 collateralRatio) = calculateIceCollateralRatioInternal(redeemer, redeemTokens, 0);
            if (cErr != Error.NO_ERROR) {
                return uint256(cErr);
            }

            if (collateralRatio < iceCollateralRatio) {
                return uint256(Error.INSUFFICIENT_ICE_COLLATERAL);
            }
        }
    ...
    function borrowAllowed(
        ...
        uint256 borrowCap = borrowCaps[rToken];
        // Borrow cap of 0 corresponds to unlimited borrowing
        if (borrowCap != 0) {
            uint256 totalBorrows = RToken(rToken).totalBorrows();
     ...
     * @return 0 if the borrow is allowed, otherwise a semi-opaque error code (See ErrorReporter.sol)
     */
    function borrowIronAllowed(address borrower, uint256 borrowAmount) internal view returns (Error) {
        (Error err, uint256 collateralRatio) = calculateIceCollateralRatioInternal(borrower, 0, borrowAmount);
        if (err != Error.NO_ERROR) {
            return err;
        }
    ...
    function calculateIceCollateralRatio(address account) external view returns (Error, uint256) {
    ...
    function calculateIceCollateralRatioInternal(
        address borrower,
        uint256 redeemTokens,
        uint256 borrowAmount
        ...
        (uint256 tokenError, uint256 rTokenBalance, , uint256 exchangeRateMantissa) = rICE.getAccountSnapshot(borrower);
        ...
        uint256 collateralValue = mul_(
            sub_(rTokenBalance, redeemTokens),
            mul_(mul_(vars.iceCollateralFactor, vars.iceExchangeRate), vars.icePrice)
        );

        RToken rIron = RToken(getRIronAddress());
        uint256 ironBorrowBalance = rIron.borrowBalanceStored(borrower);

        vars.ironPrice = Exp({mantissa: oracle.getUnderlyingPrice(rIron)});

        // borrowValue = (borrowBalance + borrowAmount) * price
        uint256 borrowValue = mul_ScalarTruncate(vars.ironPrice, add_(ironBorrowBalance, borrowAmount));

        if (borrowValue == 0) {
            // if no iron borrow, return max value
            return (Error.NO_ERROR, uint256(-1));
        }
    ...
    function liquidateBorrowAllowed(
        ...
        uint256 borrowBalance = RToken(rTokenBorrowed).borrowBalanceStored(borrower);
    ...
    function seizeAllowed(
        ...
        if (RToken(rTokenCollateral).ironController() != RToken(rTokenBorrowed).ironController()) {
            return uint256(Error.IRON_CONTROLLER_MISMATCH);
        }
    ...
    function transferAllowed(
        ...
        uint256 allowed = redeemAllowedInternal(rToken, src, transfeRTokens);
        if (allowed != uint256(Error.NO_ERROR)) {
            return allowed;
        }
        ...
        )
    {
        ...
        for (uint256 i = 0; i < assets.length; i++) {
            ...
            (oErr, vars.rTokenBalance, vars.borrowBalance, vars.exchangeRateMantissa) = asset.getAccountSnapshot(
                account
            );
            if (oErr != 0) {
    ...
    function liquidateCalculateSeizeTokens(
        ...
        uint256 priceBorrowedMantissa = oracle.getUnderlyingPrice(RToken(rTokenBorrowed));
        uint256 priceCollateralMantissa = oracle.getUnderlyingPrice(RToken(rTokenCollateral));
         ...
         */
        uint256 exchangeRateMantissa = RToken(rTokenCollateral).exchangeRateStored(); // Note: reverts on error
        uint256 seizeTokens;
     ...
     */
    function _setCollateralFactor(RToken rToken, uint256 newCollateralFactorMantissa) external returns (uint256) {
        // Check caller is admin
        if (msg.sender != admin) {
            return fail(Error.UNAUTHORIZED, FailureInfo.SET_COLLATERAL_FACTOR_OWNER_CHECK);
        }
     ...
     */
    function _supportMarket(RToken rToken) external returns (uint256) {
        if (msg.sender != admin) {
     ...
     */
    function _setMarketBorrowCaps(RToken[] calldata rTokens, uint256[] calldata newBorrowCaps) external {
        require(
            msg.sender == admin || msg.sender == borrowCapGuardian,
            "only admin or borrow cap guardian can set borrow caps"
        );

        uint256 numMarkets = rTokens.length;
     ...
     */
    function _setIceCollateralRatio(uint256 newRatio) public returns (uint256) {
        // Check caller is admin
        if (msg.sender != admin) {
            return fail(Error.UNAUTHORIZED, FailureInfo.SET_ICE_COLLATERAL_RATIO_OWNER_CHECK);
        }

        uint256 oldRatio = iceCollateralRatio;
     ...
     */
    function setRewardSpeedInternal(RToken rToken, uint256 rewardSpeed) internal {
        uint256 currentRewardSpeed = rewardSpeeds[address(rToken)];
    ...
    function updateRewardSupplyIndex(address rToken) internal {
        ...
        uint256 supplySpeed = rewardSpeeds[rToken];
        uint256 blockNumber = getBlockNumber();
        uint256 deltaBlocks = sub_(blockNumber, uint256(supplyState.block));
        if (deltaBlocks > 0 && supplySpeed > 0) {
            uint256 supplyTokens = RToken(rToken).totalSupply();
    ...
    function updateRewardBorrowIndex(address rToken, Exp memory marketBorrowIndex) internal {
        ...
        uint256 borrowSpeed = rewardSpeeds[rToken];
        uint256 blockNumber = getBlockNumber();
        uint256 deltaBlocks = sub_(blockNumber, uint256(borrowState.block));
        if (deltaBlocks > 0 && borrowSpeed > 0) {
            uint256 borrowAmount = div_(RToken(rToken).totalBorrows(), marketBorrowIndex);
    ...
    function distributeSupplierReward(address rToken, address supplier) internal {
        ...
        uint256 supplieRTokens = RToken(rToken).balanceOf(supplier);
    ...
    function distributeBorrowerReward(
        ...
        if (borrowerIndex.mantissa > 0) {
            ...
            uint256 borrowerAmount = div_(RToken(rToken).borrowBalanceStored(borrower), marketBorrowIndex);
     ...
     */
    function grantRewardInternal(address user, uint256 amount) internal returns (uint256) {
        if (amount == 0) {
            return 0;
        }
     ...
     * @param rewardSpeed New REWARD speed for market
     */
    function _setRewardSpeed(RToken rToken, uint256 rewardSpeed) public {
    ...
    function getRewardAddress() public pure returns (address) {
        return 0x1258f64c4b7355B22876Bb3253b37d0c49615D1E; // LOR
    }
}
