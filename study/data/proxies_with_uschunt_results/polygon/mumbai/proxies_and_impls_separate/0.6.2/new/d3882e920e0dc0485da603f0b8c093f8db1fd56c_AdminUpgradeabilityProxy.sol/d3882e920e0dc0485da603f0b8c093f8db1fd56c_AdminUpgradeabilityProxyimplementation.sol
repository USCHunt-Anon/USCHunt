...
interface StakingInterface {
    function votingPowerOf(address acc, uint256 until)
        external
        view
        returns (uint256);
}

contract Staking is StakingInterface, Initializable, OwnableUpgradeable {
...
function setMinDays(uint256 _days) public onlyOwner {
        minDays = _days;
    }
    ...
    function claim() public {
        ...
        require(
            block.timestamp > stakes[msg.sender].payday.sub(earlyExit),
            "too early"
        );

        uint256 owed = stakes[msg.sender].reward.add(
            stakes[msg.sender].initial
        );
    ...
    function ownerReclaim(uint256 _amount) public onlyOwner {
        require(
            _amount < token.balanceOf(address(this)).sub(totalOwedValue()),
