...
contract UniverselleGenesis is ERC1155Upgradeable, IERC2981 {
    using AddressUpgradeable for address;
    
    bytes32 public constant admin = keccak256("Genesis.admin");

    uint256 token_ID;

    event NewMint(address minter, uint256 tokenId, uint256 amount);

    function initialize(string memory _metadata) virtual public initializer {
        bytes32 position = admin;
    ...
    */
    function createToken(address _minter, uint256 _amount) virtual public onlyAdmin {
        require(_amount > 0);
        _mint(_minter, token_ID, _amount, "");
        emit URI(uri(token_ID), token_ID);
        emit NewMint(_minter, token_ID, _amount);
        token_ID++;
    }
    /**
    * @dev 2981 Royalties
    */
    function royaltyInfo(uint256 tokenId, uint256 _salePrice) virtual override external view returns (address _receiver, uint256 _royaltyAmount) {
        require(tokenId < token_ID, "invalid token");
    ...
    */
    ...
    /**
    * @dev Burn "_amount" token/s of token id "_id"
    */
    function burn(uint256 _id,uint256 _amount) virtual  external {
        require(balanceOf(_msgSender(), _id) >= _amount, "you can't burn the token/s");
        _burn(_msgSender(), _id, _amount);
    }
    /**
    * @dev "admin" getter
    */
    function owner() virtual public view returns (address _owner) {
        bytes32 position = admin;
        assembly {
    ...
    /**
     * @dev (it is unlikely that this function will ever be used) Sends ETH outside the contract
     */
    ...
    */
    modifier onlyAdmin {
        ...
        require(_admin == _msgSender(), "only admin can call this function");
        _;
    }
}
