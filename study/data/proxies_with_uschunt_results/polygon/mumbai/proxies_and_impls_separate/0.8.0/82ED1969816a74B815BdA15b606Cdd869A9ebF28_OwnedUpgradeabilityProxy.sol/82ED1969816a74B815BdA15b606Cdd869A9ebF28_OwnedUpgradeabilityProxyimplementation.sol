    ...
    MinterRole
{
    ...
    mapping(uint256 => uint256) internal _cardsToToken;
        ...
        returns (uint256)
    {
        require(
            owner != address(0),
            "Cards: balance query for the zero address"
        );
        return _balances[owner];
    }
        ...
        returns (address)
    {
        require(
            _exists(tokenId),
            "Cards: approved query for nonexistent token"
        );
    ...
    /**
     * @dev return tokenId corresponding to the cardId
     */
    function cardToToken(uint256 cardId) public view virtual returns (uint256) {
        require(cardId > 0, "Cards: Invalid cardId");
        return _cardsToToken[cardId];
    }

    /**
     * @dev mints next tokenId at 'to' address
     */
    function mint(uint256 cardId, string memory _tokenURI)
        public
        virtual
        onlyMinter
        nonReentrant
    ...
    function transferFrom(
        ...
        require(
            ...
            "Cards: transfer caller is not owner nor approved"
        );
     ...
     */
    function bundleMint(uint256[] memory cardIds, string[] memory _tokenURI)
        public
        onlyMinter
        nonReentrant
    {
        ...
        require(
            _tokenURI.length == cardIds.length,
            "Cards: TokenURIs and Cards amount won't match"
        );
    ...
    /**
     * @dev set baseURI for all the cards
     * @dev !!!Important this function can only be called by the one of the admins'
    ...
    function _safeTransfer(
        ...
        require(
            ...
            "Cards: transfer to non ERC721Receiver implementer"
        );
    }
        ...
        returns (bool)
    {
        require(
            _exists(tokenId),
            "Cards: operator query for nonexistent token"
        );
        address owner = this.ownerOf(tokenId);
    ...
    /**
     * @dev Transfers `tokenId` from `from` to `to`.
     *  As opposed to {transferFrom}, Cards imposes no restrictions on msg.sender.
     *
     * Requirements:
     *
     * - `to` cannot be the zero address.
    ...
    function _transfer(
        ...
        require(
            ...
            "Cards: transfer of token that is not own"
        );
