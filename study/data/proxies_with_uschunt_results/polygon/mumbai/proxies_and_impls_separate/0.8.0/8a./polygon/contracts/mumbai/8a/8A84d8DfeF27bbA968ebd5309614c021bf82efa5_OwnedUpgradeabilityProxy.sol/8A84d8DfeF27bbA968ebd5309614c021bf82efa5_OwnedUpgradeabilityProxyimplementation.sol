...
contract SuperCharge is AccessControl,Initializable{
    ...
    function addSupercharge(uint amount) external onlyRole(OPERATOR){
        superChargeRewards[++superChargeCount] = superChargeReward(amount,IStaking(admin.stakingContract()).totalStakedAmount());
    }
    ...
    function claimSuperCharge(address user) external{
        require(canClaim(user),"nothing to claim");
        uint end = (superChargeCount - userDetails[user].superChargeId) > 150 ? userDetails[user].superChargeId + 150 : superChargeCount;
        uint amount;
        (,, uint stakedAmount, ) = IStaking(admin.stakingContract()).getUserState(user);
        for(uint i = userDetails[user].superChargeId + 1; i <= end; i++){
            amount += (superChargeRewards[i].rewardAmount * stakedAmount)/superChargeRewards[i].totalStaked;
        }
    ...
    function canClaim(address user) view public returns(bool){
        return(userDetails[user].staked && userDetails[user].superChargeId <= superChargeCount);
    }
