    ...
    RevocableToAddress
{
    ...
    */
    function mint(address account, uint256 amount)
        ...
        returns (bool)
    {
        require(amount > 0, "Can not mint a nonpositive amount of tokens");
        ...
        ) = _reserveFeed.latestRoundData();

        require(
            uint256(answer) > amount.add(ERC20.totalSupply()), // uint256(answer) * 10 * 10 ** ERC20.decimals() > totalSupply * 9,
            "Can not mint without necessary reserve"
        );
        ...
        returns (bool, string memory)
    {
        try reserveFeed.latestRoundData() returns (
            ...
            return
                uint256(answer) > ERC20.totalSupply()
                    ? (true, "")
                    : (false, "Can not mint without necessary reserve");
