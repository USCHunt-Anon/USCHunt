...
// File contracts/CoreContractV5.sol
...
contract CoreContractV5 is ERC20Upgradeable, AccessControlUpgradeable {
    event NewPoolAddress(address newPoolAddress);
    ...
    function initialize() public {
        require(!_initializedV5, "Already initialized");
    ...
    function massAirDrop(address[] memory accounts, uint256[] memory amounts) public onlyRole(MINTER_ROLE) {
    ...
    function setPoolAddress(address poolAddress) public onlyRole(DEFAULT_ADMIN_ROLE) {
        ...
        emit NewPoolAddress(_poolAddress);
    }

    function getTokenPrice() public view returns (uint256) {
        IERC20 token = IERC20(_poolAddress);
        uint256 balance = token.balanceOf(address(this));
        return totalSupply() / balance;
    }

    function depositPool(address from, uint256 amount, bool mintToPool) public onlyRole(MINTER_ROLE) {
        IERC20 token = IERC20(_poolAddress);
        token.transferFrom(from, address(this), amount);

        uint256 balance = token.balanceOf(address(this));
        uint256 price = totalSupply() / balance;
