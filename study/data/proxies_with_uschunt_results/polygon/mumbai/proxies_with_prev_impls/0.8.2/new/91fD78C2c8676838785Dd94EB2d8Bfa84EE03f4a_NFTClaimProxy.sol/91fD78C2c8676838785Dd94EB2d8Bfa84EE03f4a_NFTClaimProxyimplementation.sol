...
contract NFTClaimV3 is Initializable, AccessProtectedUpgradeable, BaseRelayRecipient, ReentrancyGuardUpgradeable {
    ...
    /**
     * Set Claim NftTypes for a Given User (only increments)
     *
    ...
    function initializeV2(IERC1155Tradeable _token) external onlyAdmin {
        require(!_initializedV2, "NFTClaimV2: contract is already initialized");
    ...
    event SetManualUserClaimableNFTBatch(
        address indexed _admin,
        address[] _users,
        uint256[] _tokenIds,
        uint256[] amounts
    ...
    /* 
     * sets the user claimalbe NFTs to a specific number
     * (unlike setUserClaimableNFT which only increments)
     */
    function setManualUserClaimableNFTBatch(
        address[] memory users,
        uint256[] memory tokenIds,
        uint256[] memory amounts
    ) external onlyAdmin {
        ...
        for (uint256 i = 0; i < amounts.length; ++i) {
            ...
            userClaimableNFTs[user][tokenId] = amount;
        }

        emit SetManualUserClaimableNFTBatch(_msgSender(), users, tokenIds, amounts);
    }
    ...
    function initializeV3() external onlyAdmin {
        require(!_initializedV3, "NFTClaimV3: contract is already initialized");

        _initializedV3 = true;
    }

    function setERC1155Token(IERC1155Tradeable _token) public onlyAdmin {
        token = _token;
    }
