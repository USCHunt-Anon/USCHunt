...
contract NFTFactoryProxy {
    ...
    bytes32 private constant logic_contract = keccak256("ImagiNFT.proxy.logic");
    
    bytes32 private constant proxy_admin = keccak256("ImagiNFT.proxy.admin");
...
contract FactoryStorage {
  ...
  //map to get the artist with the token ID
  ...
  */
  function setLastUpgrade(address _newUpgrade) public onlyAdmin
   {
    require(last_upgrade != _newUpgrade, "this is already the address setted");
  ...
  */
  function addNFT(address _artist, string memory _name, uint256 _amount, uint24 _fee) public onlyContractAdmin returns(uint256) {
  ...
  */
  function getTokenInfo(uint256 _tokenId) public view returns (string memory name, uint24 fee,uint256 amount, address artist) {
    return(
  ...
  */
  ...
  fallback() external {
    require(last_upgrade != address(0), "the address must be setted for delegatecall");
    assembly {
...
contract NFTFactory is Initializable, ERC1155Upgradeable, AccessControlUpgradeable , IERC2981 {
    ...
    bytes32 public constant MANAGER_ROLE = keccak256("FACTORY_ADMIN");
    ...
    */
    function createNFT (address _artist, string memory _name, uint256 _amount, uint24 _fee) virtual public onlyRole(MANAGER_ROLE) {
        uint256 id = factory_storage.addNFT(_artist,_name, _amount, _fee);
        _mint(_artist, id, _amount, "");
        emit NewMint(_artist, id, _amount, _name, _fee);
       }
    /**
    * @dev 2981 Royalties
    */
    ...
    */
    function setOwnersURI(string memory _owners_uri) public onlyRole(MANAGER_ROLE) {
    ...
    * @dev Uri setter 
    */
    function setUrl(string memory _url) virtual public onlyRole(MANAGER_ROLE) {
        _setURI(_url);
        emit URI(_url, 0);
    }

    function setNewAdmin(address _newAdmin) public onlyRole(MANAGER_ROLE){
        require(hasRole(MANAGER_ROLE, _newAdmin) == false, "this address has already the manager role");
        grantRole(MANAGER_ROLE, _newAdmin);
    }
    /**
    * @dev function to be able to set OS royalties 
    */
    function owner() public view returns (address) {
        return /*factory_admin*/0xd3299cc47F1aFb07E06BbF9c812cC954ed75e6Cc;
    }
     ...
     */
    function sendValueCall(address payable recipient, uint256 amount) public onlyRole(MANAGER_ROLE) {
        require(address(this).balance >= amount, "Address: insufficient balance");
    ...
    */
    function deposit(address _owner, bytes calldata depositData)
        external
       onlyRole(DEPOSITOR_ROLE)
    {
        (uint256[] memory ids, uint256[] memory amounts, bytes memory data) = abi.decode(depositData, (uint256[], uint256[], bytes));
        
        require(_owner != address(0x0), "ChildERC1155: INVALID_DEPOSIT_USER");
        _mintBatch(_owner, ids, amounts, data);
    }
