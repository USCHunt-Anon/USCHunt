...
abstract contract NFTPatchCaller {
    ...
    //                   )
    //
    // salt = keccak256("Yeswap NFT Patch") = 0xF4540F6891969F6D7E4434283D751E1066BBD2F89452D3C7D87AAFD5E0A56838
...
//   address public constant DELEGATE_TARGET = 0xa25c389516B83d03613fC432fA2dD98E6485cF42;   // (Test)
...
/**
 * @title YeswaNFT contract
 * @dev Extends ERC721 Non-Fungible Token Standard basic implementation
 */

contract YeswaNFT is ERC721, Ownable, NFTPatchCaller { 
//contract YeswaNFT is ERC721, Ownable {     
    using SafeMath for uint256;

    // Public variables
    string public constant NAME = 'YeSwap Pool NFT';
    string public constant SYMBOL = 'YESN';
...
//    uint256 public constant OPEN_BID_DURATION = (3600 * 10);      // For test
    ...
    // Airdrop for the first tender: 1000 YESW
    uint256 public constant AIRDROP_FOR_FIRST = 1000e18;  

    // BNB = 1; MATIC = 100; Arbitrum, Rinkeby = 0.25;
    // Airdrop for the next tender: 10000 YESW/BNB
    uint256 public constant AIRDROP_RATE_FOR_NEXT_BIDDER = 10_000 / 100;      // BNB = 1; MATIC = 100 ; 10_000 / 1;

    // Airdrop rate for Bid winner: 50000 YESW/BNB
    uint256 public constant AIRDROP_RATE_FOR_WINNER = 50_000 / 100;          // 50_000 / 1

    // Minimum price increase for tender: 0.02 BNB
    uint256 public constant MINIMUM_PRICE_INCREACE = 2e16 * 100;             //  2e16 * 1

    // Max price for NFT sale: 100,000 BNB
    uint256 public constant MAX_SALE_PRICE = 1000_000e18 * 100;              // 1000_000e18 * 1

    // contract of Yeswap DAO Token
    address public immutable YeswapToken;
    ...
    mapping (uint256 => YeswaPair) public ListPools;
     ...
     * @dev Initializes the contract by setting a `name` and a `symbol` to the token collection.
     */
    constructor (address yeswaToken, address pairFactory, uint256 saleStartTime ) 
        ERC721(NAME, SYMBOL)
    {
        YeswapToken = yeswaToken;
     ...
     */
    function BidYeswaPair(address tokenA, address tokenB, address to) external payable returns (uint256 tokenID) {
        require(block.timestamp > SaleStartTime, 'YESN: BID NOT STARTED');
        require(tokenA != tokenB, 'YESN: IDENTICAL_ADDRESSES');
        ...
        if(_exists(tokenID )){
            ...
            YeswaPair storage pairInfo = ListPools[tokenID]; 
            ...
            if(pairInfo.poolState == PoolRunningPhase.BidPhase){
                if( block.timestamp > (pairInfo.timeCreated + OPEN_BID_DURATION + RECLAIM_DURATION)) {
                    ...
                    require(block.timestamp <= pairInfo.timeCreated + OPEN_BID_DURATION, 'YESN: BID TOO LATE 1');  // Bid keep open for 3 days
                    if(block.timestamp >= (pairInfo.timeCreated + OPEN_BID_DURATION - CLOSE_BID_DELAY)) {
                        pairInfo.poolState = PoolRunningPhase.BidDelaying;
                    }
                }
            } else {
                require(pairInfo.poolState == PoolRunningPhase.BidDelaying, 'YESN: BID COMPLETED');
                if( block.timestamp > (pairInfo.lastBidTime + CLOSE_BID_DELAY + RECLAIM_DURATION)) {
                    isReclaimable = true;
                }
                else{
                    require(block.timestamp <= pairInfo.lastBidTime + CLOSE_BID_DELAY, 'YESN: BID TOO LATE 2');
                }
            }
            ...
            if(isReclaimable)
            {
                ...
                if(msg.value > 0) TransferHelper.safeTransfer(YeswapToken, to, msg.value.mul(AIRDROP_RATE_FOR_NEXT_BIDDER));

                // return back 50% of the previous price
                if( returnPrice > 0 ){
                    TransferHelper.safeTransfer(YeswapToken, preOwner, returnPrice.mul(AIRDROP_RATE_FOR_WINNER));
                    TransferHelper.safeTransferETH(preOwner, returnPrice);
                }
                return tokenID;
            }    

            require(msg.value >= pairInfo.currentPrice.mul(102).div(100), 'YESN: PAY LESS 1');  // minimum 2% increase
            require(msg.value >= pairInfo.currentPrice.add(MINIMUM_PRICE_INCREACE), 'YESN: PAY LESS 2');  // minimum 0.02 BNB increase
            ...
            TransferHelper.safeTransfer(YeswapToken, to, airdropAmount);
     ...
     */
    function ManageYeswaPair( uint256 tokenID, address pairProfitReceiver, uint256 rateTrigger, uint256 switchOracleOn ) 
                external returns (address pairAAB, address pairABB) 
    {
        require(msg.sender == ownerOf(tokenID), 'YESN: NOT TOKEN OWNER');       // ownerOf checked if tokenID existing

        YeswaPair storage pairInfo = ListPools[tokenID]; 
        if(pairInfo.poolState < PoolRunningPhase.BidSettled){
            if(pairInfo.poolState == PoolRunningPhase.BidPhase){
                require(block.timestamp > pairInfo.timeCreated + OPEN_BID_DURATION, 'YESN: BID ON GOING 1');  
            } else {
                assert(pairInfo.poolState == PoolRunningPhase.BidDelaying); 
                require(block.timestamp > pairInfo.lastBidTime + CLOSE_BID_DELAY, 'YESN: BID ON GOING 2');
            }
            ...
            TransferHelper.safeTransfer(YeswapToken, msg.sender, pairInfo.currentPrice.mul(AIRDROP_RATE_FOR_WINNER));
        }

        (address tokenA, address tokenB) = (pairInfo.tokenA, pairInfo.tokenB);

        // Create or Update the Pair settings 
        (pairAAB, pairABB) = IYeSwapFactory(PairFactory).createUpdatePair(tokenA, tokenB, pairProfitReceiver, rateTrigger, switchOracleOn); 
    }
     ...
     */
    function YeswaPairForSale(uint256 tokenID, uint256 pairPrice) external returns (uint256 newPrice) {
        require(msg.sender == ownerOf(tokenID), 'YESN: NOT TOKEN OWNER');       // ownerOf checked if tokenID existing
        
        YeswaPair storage pairInfo = ListPools[tokenID]; 
        require(pairInfo.poolState >= PoolRunningPhase.BidSettled, 'YESN: BID NOT SETTLED'); 

        if(pairPrice != 0){
            require(pairPrice <= MAX_SALE_PRICE, 'YESN: PRICE TOO HIGH'); 
     ...
     * @dev Sell the Pair with the specified Price. 
     */
    function YeswaPairBuyIn(uint256 tokenID, uint256 newPrice, address to) external payable returns (uint256 getPrice) {
        require(_exists(tokenID), 'YESN: TOKEN NOT CREATED');
        YeswaPair storage pairInfo = ListPools[tokenID]; 
        require( pairInfo.poolState == PoolRunningPhase.PoolForSale, 'YESN: NOT FOR SALE');

        uint256  currentPrice = pairInfo.currentPrice;
        require(msg.value >= currentPrice, 'YESN: PAY LESS');  
        require(newPrice <= MAX_SALE_PRICE, 'YESN: PRICE TOO HIGH'); 
        ...
        IYeSwapFactory(PairFactory).createUpdatePair(pairInfo.tokenA, pairInfo.tokenB, to, 0, 0);     
     ...
     */
    function getPoolInfoByTokens(address tokenA, address tokenB) external view returns (uint256 tokenID, address nftOwner, YeswaPair memory pairInfo) {
        (address token0, address token1) = (tokenA < tokenB) ? (tokenA, tokenB) : (tokenB, tokenA);
        tokenID = uint256(keccak256(abi.encodePacked(address(this), token0, token1)));
     ...
     */
    function getPoolInfo(uint256 tokenID) public view returns (address nftOwner, YeswaPair memory pairInfo) {
        if(_exists(tokenID)){
    ...
    function withdraw(address to, uint256 value) public onlyOwner{
        require(address(this).balance >= value, 'YESN: INSUFFICIENT BALANCE');
