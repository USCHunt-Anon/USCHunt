...
/**
 * @title Ola's OAdapterStakerDelegator Contract
 * @notice OTokens which wrap an EIP-20 underlying and delegate to an implementation of a 'Adapter' for staking
 * @author Ola
 */
contract OAdapterStakerDelegator is ODelegator, CTokenInterface, CErc20Interface {
    // OLA_ADDITIONS : This contract name hash
    bytes32 constant public OAdapterStakerDelegatorContractHash = keccak256("OAdapterStakerDelegator");
    ...
    constructor(address underlying_,
        ...
        contractNameHash = OAdapterStakerDelegatorContractHash;

        address oAdapterStakerImplementation = RegistryForODelegator(comptroller_.getRegistry()).getImplementationForLn(address(comptroller_), OAdapterStakerDelegatorContractHash);

        // First delegate gets to initialize the delegator (i.e. storage contract)
        delegateTo(oAdapterStakerImplementation, abi.encodeWithSignature("initialize(address,address,address,uint256,string,string,uint8)",
            underlying_,
            stakingTarget_,
            comptroller_,
        ...
        // New implementations always get set via the setter (post-initialize)
        _setImplementation(oAdapterStakerImplementation, false, becomeImplementationData);
     ...
     */
    function () external payable {
        require(msg.value == 0,"OAdapterStakerDelegator:fallback: cannot send value to fallback");
...
//import"../../OTokens/CompoundingStakeableOToken.sol";

interface IStakerAdapterForV0_5 {
    function earn(uint amount) external;
 ...
 * @author Ola
 */
contract OAdadpterStakerV0_05 is CompoundingStakeableOToken, OMasterStakerStorageV0_05 {
    /**
     ...
     * @param symbol_ ERC-20 symbol of this token
     * @param decimals_ ERC-20 decimal precision of this token
     * @param underlying_ The address of the underlying (and interest) asset
     * @param stakingTarget_ The address of the staking target
     */
    function initialize(address underlying_,
                        ...
                        string memory symbol_,
                        uint8 decimals_) public {
        // CompoundingStakeableOToken initialize does the bulk of the work
        super.initialize(comptroller_, initialExchangeRateMantissa_, name_, symbol_, decimals_, underlying_, stakingTarget_);
    }
    ...
    /**
      ...
      */
    function borrow(uint borrowAmount) external returns (uint) {
        return uint(Error.UNBORROWABLE);
    }
     ...
     */
    function sweepToken(EIP20NonStandardInterface token) external {
        require(msg.sender == admin, "OMasterStaker::sweepToken: Only admin");
    	require(address(token) != underlying, "OMasterStaker::sweepToken: can not sweep underlying token");
     ...
     * @return The supply interest rate per block, scaled by 1e18
     */
    function supplyRatePerBlockInternal() internal view returns (uint) {
        return 0;
    }
     ...
     */
    function getCashPrior() internal view returns (uint) {
        // Note : As part of the normal logic, this contract should hold any of the deposited underlying.
        //        Anyone can send underlying to this contract, so we will just add them to the balance.
        EIP20Interface token = EIP20Interface(underlying);

        // TODD: SafeMath ?
        return token.balanceOf(address(this)) + IStakerAdapterForV0_5(stakingTarget).balanceOf();
    }

    /**
     * Override
     * @notice Gets balance of this contract in terms of the underlying staked in the staking contract
     * @dev This excludes the value of the current message, if any
     * @return The quantity of (staked) underlying owned by this contract
     */
    function getStakedCashPrior() internal view returns (uint) {
        return IStakerAdapterForV0_5(stakingTarget).balanceOf();
    }

    /**
     * @dev Performs a restake, reverting upon failure. Returns the amount actually restaked in the staking contract, in case of a fee.
     *
     */
    function doReStake() internal returns (uint) {
        uint selfBalance = EIP20Interface(underlying).balanceOf(address(this));
        uint stakedCashPrior = getStakedCashPrior();

        // Note : This will cause any 'index based growth' staking gains to not be taken into account in the reserves.
        IStakerAdapterForV0_5(stakingTarget).harvest();

        if (selfBalance > 0) {
            EIP20Interface(underlying).approve(stakingTarget, 0);
            EIP20Interface(underlying).approve(stakingTarget, selfBalance);
            IStakerAdapterForV0_5(stakingTarget).earn(selfBalance);
        }

        uint stakedCashAfter = getStakedCashPrior();

        // Note : We will not take into account
        require(stakedCashAfter >= stakedCashPrior, "STAKED_CASH_OVERFLOW");
        return stakedCashAfter - stakedCashPrior;
    }

    /**
     * @dev Performs a stake, reverting upon failure. Returns the amount actually staked in the staking contract, in case of a fee.
     *  This may revert due to insufficient balance or insufficient allowance.
     */
    function doStake(address from, uint amount) internal returns (uint) {
        ...
        uint safeAmount = balanceAfter - balanceBefore;   // underflow already checked above, just subtract

        EIP20Interface(underlying).approve(stakingTarget, 0);
        EIP20Interface(underlying).approve(stakingTarget, safeAmount);

        IStakerAdapterForV0_5(stakingTarget).earn(safeAmount);

        return safeAmount;
    }

    /**
     * @dev Performs an unstake, ideally returning an explanatory error code upon failure rather than reverting.
     * Unless a fatal error occurred the total staked cash should always be greater than amount.
     * In case of a withdrawal fee, 'to' will receive amount - fee.
     */
    function doUnstake(address payable to, uint amount) internal {
        uint balanceBefore = EIP20NonStandardInterface(underlying).balanceOf(address(this));
        IStakerAdapterForV0_5(stakingTarget).withdraw(amount);
        uint balanceAfter = EIP20NonStandardInterface(underlying).balanceOf(address(this));

        if (balanceAfter > balanceBefore) {
            doTransferOut(to, balanceAfter - balanceBefore);
        }
    }
...
//import"./UnborrowableStakeableOToken.sol";

/**
 * @title Ola's CompoundingStakeableOToken Contract
 * @notice Abstract base for OTokens whose underlying asset is being deposit in an external contract which generates interest for it.
 * @author Ola Finance
 */
contract CompoundingStakeableOToken is UnborrowableStakeableOToken, CompoundingStakeableOTokenStorage {
    /**
     * @notice Initialize the money market
     * @param comptroller_ The address of the Comptroller
     * @param initialExchangeRateMantissa_ The initial exchange rate, scaled by 1e18
    ...
    function initialize(ComptrollerInterface comptroller_,
                        ...
                        uint8 decimals_,
                        address underlying_,
                        address stakingTarget) public {
        // UnborrowableOToken initialize does the bulk of the work
        // this market has the same asset for both 'underlying' and 'interest'.
        super.initialize(comptroller_, initialExchangeRateMantissa_, name_, symbol_, decimals_, underlying_, stakingTarget, underlying_);
    }
     ...
     *   up to the current block and writes new checkpoint to storage.
     */
    function accrueInterestInternal() internal returns (uint) {
        /* Remember the initial block number */
        uint currentBlockNumber = getBlockNumber();
        uint accrualBlockNumberPrior = accrualBlockNumber;
        uint currentBlockTimestamp = getBlockTimestamp();
        ...
        /*
         *  rewardsGained = Rewards gained from staking
         *  totalReservesNew = rewardsGained * reserveFactor + totalReserves
         */
        uint rewardsGained = doReStake();
        uint totalReservesNew;
        MathError mathErr;

        (mathErr, totalReservesNew) = mulScalarTruncateAddUInt(Exp({mantissa: reserveFactorMantissa}), rewardsGained, reservesPrior);
        if (mathErr != MathError.NO_ERROR) {
            return failOpaque(Error.MATH_ERROR, FailureInfo.ACCRUE_INTEREST_NEW_TOTAL_RESERVES_CALCULATION_FAILED, uint(mathErr));
        }
        ...
        /* We emit an AccrueInterest event */
        // emit AccrueInterest(cashPrior, interestAccumulated, borrowIndexNew, totalBorrowsNew);
        emit AccrueInterest(cashPrior, rewardsGained, 0, 0);
    ...
    function _reduceReservesFresh(uint reduceAmount) internal returns (uint) {
        ...
        // OLA_ADDITIONS : Transfer reserves to both admin and Ola bank addresses
        // doUnstake reverts if anything goes wrong, since we can't be sure if side effects occurred.
        doUnstake(adminBankAddress, adminPart);
        doUnstake(olaBankAddress, olaPart);
    ...
    function exchangeRateStoredInternal() internal view returns (MathError, uint) {
        ...
        if (_totalSupply == 0) {
             ...
             * Otherwise:
             *  exchangeRate = (totalCash + totalBorrows - totalReserves) / totalSupply
             * totalCash = staked and held underlying
             */
            uint totalCash = getCashPrior();
            uint cashPlusBorrowsMinusReserves;
            Exp memory exchangeRate;
            ...
            (mathErr, cashPlusBorrowsMinusReserves) = addThenSubUInt(totalCash, 0, totalReserves);
            if (mathErr != MathError.NO_ERROR) {
                return (mathErr, 0);
            }
...
/**
 * @title Ola's UnborrowableOToken Contract
 * @notice Abstract base for OTokens that offer staking of assets with no borrowing capabilities.
 * @author Ola Finance
 */
contract UnborrowableStakeableOToken is CTokenStorage, UnborrowableStakeableOTokenStorage, CTokenInterface, CTokenViewInterface, Exponential, TokenErrorReporter {
     ...
     * @param symbol_ EIP-20 symbol of this token
     * @param decimals_ EIP-20 decimal precision of this token
     * @param underlying_ EIP-20 asset of this market.
     * @param stakingTarget_ The contract where assets will be deposited.
     * @param interestAsset_ EIP-20 The asset that is earned by staking.
     */
    function initialize(ComptrollerInterface comptroller_,
                        uint initialExchangeRateMantissa_,
                        string memory name_,
        ...
        // Set underlying and interest assets + sanity check them
        underlying = underlying_;
        stakingTarget = stakingTarget_;
        interestAsset = interestAsset_;

        EIP20Interface(underlying).totalSupply();
        EIP20Interface(interestAsset).totalSupply();
    ...
    /**
     * @return The interest asset gained from staking at the 'staking target'
     */
    function getInterestAsset() external returns (address) {
        return interestAsset;
    }

    /**
     * @return The contract on which the staking is happening
     */
    function getStakingTarget() external returns (address) {
        return stakingTarget;
    }

    /**
     * @notice Get staked cash balance of this cToken in the underlying asset
     * @return The quantity of staked underlying asset owned by this contract
     */
    function getStakedCash() external view returns (uint) {
        return getStakedCashPrior();
    }
     ...
     */
    function supplyRatePerBlock() external view returns (uint) {
        return supplyRatePerBlockInternal();
    }
    ...
    function mintFresh(address minter, uint mintAmount) internal returns (uint, uint) {
        ...
        /*
         *  We call `doStake` for the minter and the mintAmount.
         *  Note: The oToken must handle variations between ERC-20 and ETH underlying.
         *  `doStake` reverts if anything goes wrong, since we can't be sure if
         *  side-effects occurred. The function returns the amount actually staked,
         *  in case of a fee. On success, the cToken is entitled to an additional `actualMintAmount`
         *  of staked cash.
         */
        vars.actualMintAmount = doStake(minter, mintAmount);
    ...
    function redeemFresh(address payable redeemer, uint redeemTokensIn, uint redeemAmountIn) internal returns (uint) {
        ...
        /*
         * Note : This part was moved to after the state changes.
         * We invoke doUnstake for the redeemer and the redeemAmount.
         *  Note: The oToken must handle variations between ERC-20 and ETH underlying.
         *  On success, the oToken has redeemAmount less of staked cash and the redeemer has redeemAmount
         *  more cash (minus any staking contract withdrawal fee).
         *  doUnstake reverts if anything goes wrong, since we can't be sure if side effects occurred.
         */
        doUnstake(redeemer, vars.redeemAmount);
    ...
    function _setInterestRateModel(InterestRateModel newInterestRateModel) public returns (uint) {
        ...
        revert("No IRM for deposited market");
    }


    /*** Safe Token abstracts ***/
...
//import"../OAdapterStakerV0_05.sol";

/**
 * @title Ola's OAdapterStakerDelegateDelegateV0_05 Contract
 * @notice OToken which wrap an EIP-20 underlying and are delegated to + which stakes all deposited assets
 * @author Ola
 */
contract OAdapterStakerDelegateV0_05 is OAdadpterStakerV0_05, CDelegateInterface {
    /**
