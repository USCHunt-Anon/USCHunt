...
// File: TransparentUpgradeableProxySafeBoxImpl.sol

contract TransparentUpgradeableProxySafeBoxImpl is TransparentUpgradeableProxy {
  // safebox has function weth
  ...
  receive() external payable override {
    require(msg.sender == wrapNative, '!native');
  }
}
...
contract SafeBoxFTM is Governable, ERC20Upgradeable, ReentrancyGuardUpgradeable {
  event Claim(address user, uint amount);

  ICErc20 public cToken;
  IWETH public weth; // WFTM address
  ...
  function initialize(
    ...
    weth = _weth;
    relayer = msg.sender;
    _weth.approve(address(_cToken), type(uint).max);
  }
  ...
  function withdraw(uint amount) public nonReentrant {
    ...
    uint wethAmount = wethBalanceAfter - wethBalanceBefore;
  ...
  receive() external payable {
    require(msg.sender == address(weth), '!wftm');
  }
}