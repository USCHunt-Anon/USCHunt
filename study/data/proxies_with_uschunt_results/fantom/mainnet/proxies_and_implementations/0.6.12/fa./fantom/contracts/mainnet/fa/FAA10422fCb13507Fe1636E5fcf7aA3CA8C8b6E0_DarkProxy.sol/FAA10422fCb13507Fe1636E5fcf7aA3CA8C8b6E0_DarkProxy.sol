...
contract DarkProxy is UpgradableProxy {
...
contract DarkBridgeFantom is OwnableUpgradeable, ReentrancyGuard {
    ...
    function initialize(address _token, uint256 _mininumOutAmount, uint256 _mininumInAmount, address _minter, address _verifyAuthority) public initializer {
        OwnableUpgradeable.__Ownable_init();
        token = _token;
        mininumOutAmount = _mininumOutAmount;
        mininumInAmount = _mininumInAmount;
        verifyAuthority = _verifyAuthority;
        minterCaps[_minter] = 1000000 ether;
        paused = false;
    }
    ...
    function inNetwork(
        ...
        bytes32 _nHash,
        bytes memory _sig
    ) public whenNotPaused checkMinterCap(_amount) notContract nonReentrant {
        require(txStatus[_txHash] == false, "Gateway: txhash already processed");
        ...
        require(hashStatus[signedMessage] == false, "Gateway: nonce hash already spent");
        ...
        hashStatus[signedMessage] = true;
        txStatus[_txHash] = true;
    ...
    function rescueStuckErc20(address _token, address _to) external onlyOwner {
        require(_token != token, "not rescue bridge token");
        IERC20(_token).transfer(_to, IERC20(_token).balanceOf(address(this)));
    }

    function rescueStuckEthers(address payable _to) external onlyOwner {
