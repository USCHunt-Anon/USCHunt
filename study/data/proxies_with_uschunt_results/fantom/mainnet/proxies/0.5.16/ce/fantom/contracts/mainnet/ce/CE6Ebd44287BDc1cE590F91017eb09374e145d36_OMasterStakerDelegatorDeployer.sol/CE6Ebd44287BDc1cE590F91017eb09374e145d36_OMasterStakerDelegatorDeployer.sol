...
//import "../../../LendingNetwork/Delegators/OMasterStakerDelegator.sol";

contract OMasterStakerDelegatorDeployer {
    function deployODelegator(
        ...
        OMasterStakerDelegator oMasterStakerDelegator = new OMasterStakerDelegator(
                underlying_,
                stakingTarget_,
                comptroller_,
        ...
        address oMasterStakerDelegatorAddress = address(oMasterStakerDelegator);

        return oMasterStakerDelegatorAddress;
    }
}
...
/**
 * @title Ola's OMasterStakerDelegator Contract
 * @notice OTokens which wrap an EIP-20 underlying and delegate to an implementation of a 'MasterStaker'
 * @author Ola
 */
contract OMasterStakerDelegator is ODelegator, CTokenInterface, CErc20Interface {
    // OLA_ADDITIONS : This contract name hash
    bytes32 constant public OMasterStakerDelegatorContractHash = keccak256("OMasterStakerDelegator");
    ...
    constructor(address underlying_,
        ...
        contractNameHash = OMasterStakerDelegatorContractHash;

        address oMasterStakerImplementation = RegistryForODelegator(comptroller_.getRegistry()).getImplementationForLn(address(comptroller_), OMasterStakerDelegatorContractHash);

        // First delegate gets to initialize the delegator (i.e. storage contract)
        delegateTo(oMasterStakerImplementation, abi.encodeWithSignature("initialize(address,address,address,uint256,string,string,uint8)",
                                                            underlying_,
                                                            stakingTarget_,
                                                            comptroller_,
                                                            initialExchangeRateMantissa_,
        ...
        // New implementations always get set via the setter (post-initialize)
        _setImplementation(oMasterStakerImplementation, false, becomeImplementationData);
   ...
   /**
     ...
     */
    function () external payable {
        require(msg.value == 0,"OMasterStakerDelegator:fallback: cannot send value to fallback");
