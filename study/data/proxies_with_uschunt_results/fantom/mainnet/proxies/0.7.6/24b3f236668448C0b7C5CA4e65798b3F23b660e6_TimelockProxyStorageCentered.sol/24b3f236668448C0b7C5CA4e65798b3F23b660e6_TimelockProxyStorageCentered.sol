...
// File contracts/utilities/UnstructuredStorageWithTimelock.sol
...
// File contracts/interface/IStorageV1.sol
...
// File contracts/TimelockProxyStorageCentered.sol
...
// File contracts/liquidate/external/IUniswapV2Router02.sol
...
// File contracts/StorageV1Upgradeable.sol
...
// File contracts/inheritance/StorageV1ConsumerUpgradeable.sol
...
// File contracts/interface/IDebtor.sol
...
// File contracts/interface/ICreditor.sol
...
// File contracts/interface/IInsuranceProvider.sol
...
// File contracts/investmentVehicle/InvestmentVehicleSingleAssetBaseV1Upgradeable.sol
    ...
    StorageV1ConsumerUpgradeable, IDebtor
{
    ...
    bytes32 private constant __storage = keccak256(
        "InvestmentVehicleSingleAssetBaseV1Upgradeable"
    );

    struct IVBaseStorage{
        uint256 _autoControl;
    }

    uint256 constant AUTO_INVEST_AT_DEPOSIT = 1 << 0;

    function _getIVBaseStorage() internal pure returns(
        IVBaseStorage storage s
    ){
        bytes32 ptr = __storage;
        assembly {s.slot := ptr}
    }
    ...
    function askToInvestAsCreditor(uint256 _amount) external onlyCreditor override returns(uint256) {
        ...
        IVBaseStorage storage _s = _getIVBaseStorage();
        if((_s._autoControl & AUTO_INVEST_AT_DEPOSIT) == AUTO_INVEST_AT_DEPOSIT) {
            _investAll();
        }
    ...
    function setAutoControl(uint256 _autoControl) adminPriviledged external{
        IVBaseStorage storage _s = _getIVBaseStorage();
        _s._autoControl = _autoControl;
    }

    function autoControl() external returns (uint256) {
        IVBaseStorage storage _s = _getIVBaseStorage();
        return _s._autoControl;
    }

}


// File contracts/investmentVehicle/yearn/interface/IYearnVaultV4.sol



pragma solidity 0.7.6;

interface IYearnVaultV4 {
...
// File contracts/investmentVehicle/yearn/YearnV4VaultV1Base.sol
...
/**
    YearnV4VaultV1Base is the IV implementation that targets Yearn V4 vaults.
    It takes the base asset and deposits into the external Yearn vaults
*/
contract YearnV4VaultV1Base is InvestmentVehicleSingleAssetBaseV1Upgradeable {
    ...
    function initialize(
        ...
        yearnVault = _yVault;
        YEARN_SHARE_UNIT = 10 ** (IYearnVaultV4(yearnVault).decimals());
    }
    ...
    function baseAssetToYVaultShares(uint256 baseAssetAmount) public view returns(uint256) {
        return baseAssetAmount
            ...
            .div(IYearnVaultV4(yearnVault).pricePerShare());
    }
    ...
    function yVaultSharesToBaseAsset(uint256 shares) public view returns(uint256) {
        return shares
            .mul(IYearnVaultV4(yearnVault).pricePerShare())
    ...
    function _investAll() internal override {
        ...
        if(baseAssetAmountInVehicle > 0){
            ...
            IYearnVaultV4(yearnVault).deposit(baseAssetAmountInVehicle);
    ...
    function _pullFundsFromInvestment(uint256 _baseAmount) internal override{
        ...
        uint256 ownedShare = IYearnVaultV4(yearnVault).balanceOf(address(this));
        uint256 withdrawingShare = MathUpgradeable.min(ownedShare, respectiveShare);

        IYearnVaultV4(yearnVault).withdraw(withdrawingShare);
    }
