/** 
 *  SourceUnit: /Users/oker/dev_code/okex/newNFTMarket/contracts/MyTokenERC1155.sol
*/
...
contract HasContractURI is ERC165 {
    ...
    function __initHasContractURI(string memory contractURI) internal {
        _contractURI = contractURI;
        _registerInterface(_INTERFACE_ID_CONTRACT_URI);
    }
...
contract MyTokenERC1155 is Ownable, ERC1155Base {
    using SafeMath for uint256;
    ...
    // mint for testing. It should be deleted in prod env.
    function mint(uint256 amount) external payable {
        ...
        // 2. mint
        _mint(currentId, msg.sender, amount, "");

        // 3. set tokenURI
        _setTokenURI(
            currentId,
            "https://gateway.pinata.cloud/ipfs/QmNe2Jxm3aqVmByTMdud3z2pDiAYARBfLTEFg1Z7iiK3rj/143.json"
        );
    }

    function multiMint(uint256[] memory amounts) external payable {
        ...
        for (uint256 i = 0; i < length; i++) {
            ...
            // 2. mint
            _mint(currentId, msg.sender, amounts[i], "");
