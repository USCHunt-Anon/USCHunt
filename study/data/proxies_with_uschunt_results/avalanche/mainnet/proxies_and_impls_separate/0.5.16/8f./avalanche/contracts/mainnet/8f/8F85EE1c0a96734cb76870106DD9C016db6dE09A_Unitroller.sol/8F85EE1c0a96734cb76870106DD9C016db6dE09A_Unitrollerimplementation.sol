...
contract Comptroller is ComptrollerV1Storage, ComptrollerInterface, ComptrollerErrorReporter, ExponentialNoError {
     ...
     * @dev Note: If there is not enough Atlantis, we do not perform the transfer at all.
    ...
    function grantAtlantisInternal(address user, uint amount) internal returns (uint) {
        uint atlantisRemaining = IBEP20(getAtlantisAddress()).balanceOf(address(this));
        if (amount > 0 && amount <= atlantisRemaining) {
            IBEP20(getAtlantisAddress()).safeTransfer(user, amount);
            return 0;
        }
        return amount;
    }
    ...
    /**
     * @dev We use getBlockNumber function name instead of getTimestamp so that it's backwards compatible with the ABI of other chains
    */
    function getBlockNumber() internal view returns (uint256) {
        return block.timestamp;
    }
     ...
     * DEV: This is just for backwards compatibility with the ABI of other chains
    ...
    */
    ...
    /**
     * @notice Set the Atlantis token address or any other distribution address
     */
    function setAtlantisAddress(address newAtlantisAddress) public {
        require(msg.sender == admin, "only admin can set atlantis address");
        atlantisAddress = newAtlantisAddress;
    }
}
