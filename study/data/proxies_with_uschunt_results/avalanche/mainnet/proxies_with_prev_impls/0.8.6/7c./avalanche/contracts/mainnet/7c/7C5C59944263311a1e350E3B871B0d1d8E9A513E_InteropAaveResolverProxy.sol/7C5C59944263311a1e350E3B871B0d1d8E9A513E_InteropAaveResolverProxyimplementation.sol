...
abstract contract Helpers is DSMath, Variables {
    ...
    */
     ...
     * Checks the position to migrate should have a safe gap from liquidation 
    */
    function _checkAaveLiquidityRatio (
        address liquidity,
        Position memory position,
        uint256 safeRatioPercentage
    ) public view returns (
        ...
        AaveData memory data = sortData(position, false);
        uint256 extraBorrow;
        bool isBorrowOk;

        (extraBorrow, isBorrowOk) =
            checkBorrowToken(liquidity, data, false);

        positionData = _checkLiquidityRatio(liquidity, safeRatioPercentage, extraBorrow);
    }
    ...
    function _checkLiquidityRatio(
        ...
        (
            ...
            p.currentLiquidationThresholdBefore,
            ,
        ) = aave.getUserAccountData(liquidity);

        positionData.totalBorrow = add(p.totalBorrowBefore, extraBorrow);
        positionData.totalSupply = p.totalSupplyBefore;

        totalMaxBorrow = add(positionData.totalBorrow, p.totalBorrowAvailableBefore);
        totalMaxLiquidation = positionData.totalSupply * p.currentLiquidationThresholdBefore / 10000;
