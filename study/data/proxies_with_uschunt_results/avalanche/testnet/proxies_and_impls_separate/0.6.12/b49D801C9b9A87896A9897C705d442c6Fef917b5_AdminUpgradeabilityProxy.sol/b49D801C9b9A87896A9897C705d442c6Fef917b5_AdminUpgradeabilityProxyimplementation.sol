...
interface IRewarder {
    ...
    function onSmayReward(uint256 pid, address user, address recipient, uint256 smayAmount, uint256 newLpAmount) external;
    function pendingTokens(uint256 pid, address user, uint256 smayAmount) external view returns (IERC20[] memory, uint256[] memory);
}
...
/// @notice The (older) MasterChef contract gives out a constant number of SMAY tokens per block.
/// It is the only address with minting rights for SMAY.
...
contract MasterChefV2 is OwnableUpgradeable {
    ...
    /// `rewardDebt` The amount of SMAY entitled to the user.
    struct UserInfo {
        uint256 amount;
        int256 rewardDebt;
    }
    ...
    /// Also known as the amount of SMAY to distribute per block.
    struct PoolInfo {
        uint256 accSmayPerShare;
    ...
    /// @notice Address of SMAY contract.
    IERC20 public SMAY;
    ...
    uint256 public MASTERCHEF_SMAY_PER_BLOCK;
    uint256 public ACC_SMAY_PRECISION;
    ...
    event LogUpdatePool(uint256 indexed pid, uint256 lastRewardBlock, uint256 lpSupply, uint256 accSmayPerShare);
    event LogInit();
    ...
    function initialize(IERC20 _smay, address _feeAddress, address _treasury) public initializer {
        __Ownable_init();
        SMAY = _smay;
        feeAddress = _feeAddress;
        treasury = _treasury;
        ACC_SMAY_PRECISION = 1e18;
    }

    function setMasterChef(IMasterChef masterChef, uint256 masterPid, uint256 masterChefSmayPerBlock) external onlyOwner {
        MASTER_CHEF = masterChef;
        MASTER_PID = masterPid;
        MASTERCHEF_SMAY_PER_BLOCK = masterChefSmayPerBlock;
    }
    ...
    /// @notice Deposits a dummy token to `MASTER_CHEF` MCV1. This is required because MCV1 holds the minting rights for SMAY.
    ...
    /// @notice Update the given pool's SMAY allocation point and `IRewarder` contract. Can only be called by the owner.
    ...
    /// @notice View function to see pending SMAY on frontend.
    /// @param _pid The index of the pool. See `poolInfo`.
    /// @param _user Address of user.
    /// @return pending SMAY reward for a given user.
    function pendingSmay(uint256 _pid, address _user) external view returns (uint256 pending) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 smayReward = blocks.mul(smayPerBlock()).mul(pool.allocPoint) / totalAllocPoint;
            accSmayPerShare = accSmayPerShare.add(smayReward.mul(ACC_SMAY_PRECISION) / lpSupply);
        }
        pending = int256(user.amount.mul(accSmayPerShare) / ACC_SMAY_PRECISION).sub(user.rewardDebt).toUInt256();
    }
    ...
    /// @notice Calculates and returns the `amount` of SMAY per block.
    function smayPerBlock() public view returns (uint256 amount) {
        amount = uint256(MASTERCHEF_SMAY_PER_BLOCK)
    ...
    function updatePool(uint256 pid) public returns (PoolInfo memory pool) {
        ...
        if (block.number > pool.lastRewardBlock) {
            ...
            if (lpSupply > 0) {
                uint256 blocks = block.number.sub(pool.lastRewardBlock);
                uint256 smayReward = blocks.mul(smayPerBlock()).mul(pool.allocPoint) / totalAllocPoint;
                pool.accSmayPerShare = pool.accSmayPerShare.add(smayReward.mul(ACC_SMAY_PRECISION) / lpSupply);
            }
            pool.lastRewardBlock = block.number;
            poolInfo[pid] = pool;
            emit LogUpdatePool(pid, pool.lastRewardBlock, lpSupply, pool.accSmayPerShare);
        }
    }

    /// @notice Deposit LP tokens to MCV2 for SMAY allocation.
    /// @param pid The index of the pool. See `poolInfo`.
    ...
    function deposit(uint256 pid, uint256 amount, address to) public {
        ...
        user.rewardDebt = user.rewardDebt.add(int256(amount.mul(pool.accSmayPerShare) / ACC_SMAY_PRECISION));

        // Interactions
        IRewarder _rewarder = rewarder[pid];
        if (address(_rewarder) != address(0)) {
            _rewarder.onSmayReward(pid, to, to, 0, user.amount);
        }
    ...
    function withdraw(uint256 pid, uint256 amount, address to) public {
        ...
        // Effects
        user.rewardDebt = user.rewardDebt.sub(int256(amount.mul(pool.accSmayPerShare) / ACC_SMAY_PRECISION));
        user.amount = user.amount.sub(amount);

        // Interactions
        IRewarder _rewarder = rewarder[pid];
        if (address(_rewarder) != address(0)) {
            _rewarder.onSmayReward(pid, msg.sender, to, 0, user.amount);
        }
    ...
    /// @param pid The index of the pool. See `poolInfo`.
    /// @param to Receiver of SMAY rewards.
    function harvest(uint256 pid, address to) public {
        PoolInfo memory pool = updatePool(pid);
        UserInfo storage user = userInfo[pid][msg.sender];
        int256 accumulatedSmay = int256(user.amount.mul(pool.accSmayPerShare) / ACC_SMAY_PRECISION);
        uint256 _pendingSmay = accumulatedSmay.sub(user.rewardDebt).toUInt256();

        harvestFromMasterChef();

        // Effects
        user.rewardDebt = accumulatedSmay;

        // Interactions
        if (_pendingSmay != 0) {
            SMAY.safeTransfer(to, _pendingSmay);
        }
        
        IRewarder _rewarder = rewarder[pid];
        if (address(_rewarder) != address(0)) {
            _rewarder.onSmayReward( pid, msg.sender, to, _pendingSmay, user.amount);
        }

        emit Harvest(msg.sender, pid, _pendingSmay);
    }
    
    /// @notice Withdraw LP tokens from MCV2 and harvest proceeds for transaction sender to `to`.
    /// @param pid The index of the pool. See `poolInfo`.
    /// @param amount LP token amount to withdraw.
    /// @param to Receiver of the LP tokens and SMAY rewards.
    function withdrawAndHarvest(uint256 pid, uint256 amount, address to) public {
        ...
        // Effects
        user.rewardDebt = accumulatedSmay.sub(int256(amount.mul(pool.accSmayPerShare) / ACC_SMAY_PRECISION));
        user.amount = user.amount.sub(amount);
    ...
    /// @notice Harvests SMAY from `MASTER_CHEF` MCV1 and pool `MASTER_PID` to this MCV2 contract.
    ...
    function emergencyWithdraw(uint256 pid, address to) public {
        ...
        IRewarder _rewarder = rewarder[pid];
        if (address(_rewarder) != address(0)) {
            _rewarder.onSmayReward(pid, msg.sender, to, 0, 0);
        }
