...
/*
    Copyright 2021 Ethic Money Devs <devs@ethic.money> and Copyright 2021 Empty Set Squad <emptysetsquad@protonmail.com>
...
library Constants {
    /* Chain */
    uint256 private constant CHAIN_ID = 3; //  Ropsten
    
    /* Bootstrapping */
    uint256 private constant BOOTSTRAPPING_PERIOD = 1; // Remove bootstrapping after launch
    uint256 private constant BOOTSTRAPPING_PRICE = 11e17; // 1.10 DAI
    uint256 private constant BOOTSTRAPPING_SPEEDUP_FACTOR = 2; // 
    
    /* Oracle */                                   
    address private constant DAI_ADDRESS = address(0xaD6D458402F60fD3Bd25163575031ACDce07538D); //  Ropsten DAI addr.   Mainnet DAI addr = 0x6B175474E89094C44Da98b954EedeAC495271d0F
    uint256 private constant ORACLE_RESERVE_MINIMUM = 1e12; // 10 DAI    
    ...
    uint256 private constant INCENTIVE = 5e20; // 500 ETHC total incentive
    uint256 private constant ADVANCE_INCENTIVE = (INCENTIVE/100)*40; // 40% of incentive. 200 ETHC
    uint256 private constant TREASURY_INCENTIVE = (INCENTIVE/100)*60; // 60% of incentive. 300 ETHC
    ...
    /* Deployed */
    address private constant DAO_ADDRESS = address(0x086765A465EcCfb78722587932D1898182d2Aeb3);      // (Root) Mainnet: DAO_ADDRESS = address(0xb5C18e59Ac0Ee2fe1a91542981DD5eFD8b6eE5C0);
    address private constant ETHIC_ADDRESS = address(0x4872c14608f74684e22b4cbFC5451642A1eE8521);    // (ERC-20) Mainnet:  5C18e59Ac0Ee2fe1a91542981DD5eFD8b6eE5C0);
    address private constant PAIR_ADDRESS = address(0x5Ab7FF199ac6da5C1995E458698e88a4b11e56fa);     // (UniswapV2Pair) Mainnet:  PAIR_ADDRESS = address(0xdB312a0952705484F3C685074F547c877EaA244b)
...
contract Comptroller is Setters {
    ...
    function mintToAccount(address account, uint256 amount) internal {
        ...
        increaseDebt(amount);   // Removed code if (!bootstrappingAt(epoch())) {  }
    ...
    function increaseSupply(uint256 newSupply) internal returns (uint256, uint256) {
        // 0-a. Pay out to Pool   -- This is instead of mintToBonded()
...
contract Regulator is Comptroller {
    ...
    function oracleCapture() private returns (Decimal.D256 memory) {
        ...
        // remove unneeded code -- if (bootstrappingAt(epoch().sub(1))) {}
