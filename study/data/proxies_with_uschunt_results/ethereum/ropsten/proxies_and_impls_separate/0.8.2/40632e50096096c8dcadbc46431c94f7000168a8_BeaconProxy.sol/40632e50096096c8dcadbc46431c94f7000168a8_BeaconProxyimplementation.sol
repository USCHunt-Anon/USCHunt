...
interface IPartyEvents {
     ...
     * @param member Address of the user
     * @param soldAssets Array of addresses sold to redeem denomination assets
     * @param soldAmounts Array of amounts sold to redeem denomination assets
     * @param buyAmounts Array of denomination assets bought
     * @param amount Total denomination asset withdrawed
     * @param fee Collected fees (in denomination asset)
...
contract Party is PartyInitializable, PartyStorage, IParty {
    ...
    ***************/
     ...
     * @param tokenSymbol The Party Token symbol
    ...
    function initialize(
        ...
        // Init functions
        __ERC20_init(_partyInfo.name, tokenSymbol);
    ...
    ***************/
    ...
    function joinParty(
        ...
        // Emit Join event
        emit Join(msg.sender, mainToken, amount, fee, mintedPT);
    }
    ...
    function deposit(
        ...
        // Emit Deposit event
        emit Deposit(msg.sender, mainToken, amount, fee, mintedPT);
    }
    ...
    function withdraw(
        ...
        (
            address[] memory soldAssets,
            uint256[] memory soldAmounts,
    ...
    ***************/
        ...
        )
    {
        ...
        if (amountPT > 0) {
            // 4) Get the portion of denomination asset to withdraw (before allocation)
            uint256 daBalance = IERC20Upgradeable(mainToken).balanceOf(
                address(this)
            );
            uint256 withdrawDA = ((daBalance * amountPT) / totalSupply);

            // 5) Swap member's share of other assets into the denomination asset
            ...
            // 6) Iterate through allocation and accumulate pending withdrawal for the user
            ...
            // 7) Collect fees
            fee = getPlatformFee(withdrawDA);
            IERC20Upgradeable(mainToken).transfer(PLATFORM_ADDRESS, fee);

            // 8) Transfer DA funds to user
            amount = (withdrawDA - fee);
            IERC20Upgradeable(mainToken).transfer(_memberAddress, amount);
            return (
