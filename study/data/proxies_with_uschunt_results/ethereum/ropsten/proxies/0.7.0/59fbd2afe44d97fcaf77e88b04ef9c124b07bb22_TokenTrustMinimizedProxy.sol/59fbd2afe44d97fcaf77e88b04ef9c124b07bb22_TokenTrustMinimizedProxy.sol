...
//		PROPOSE_BLOCK_SLOT == bytes32(uint256(keccak256('eip1984.proxy.proposeBlock')) - 1));
		_setAdmin(msg.sender);
	}

	modifier ifAdmin() {if (msg.sender == _admin()) {_;} else {_fallback();}}
	function getSettings() external ifAdmin returns(address logic, uint pgrdBlck) {return (_logic(), _proposeBlock());}
	...
	function changeAdmin(address newAdm) external ifAdmin {emit AdminChanged(newAdm);_setAdmin(newAdm);}
