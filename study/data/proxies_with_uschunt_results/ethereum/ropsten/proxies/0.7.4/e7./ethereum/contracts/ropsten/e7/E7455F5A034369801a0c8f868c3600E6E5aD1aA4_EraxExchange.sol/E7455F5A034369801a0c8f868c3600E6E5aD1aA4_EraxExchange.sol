...
contract ExchangeCore is ReentrancyGuarded, Ownable {
    ...
    uint public maximumOriginatorFee = 0x4e2;

    uint public maximumAgentFee = 0x4e2;
        ...
        returns (uint)
        {
            ...
            if (sell.paymentToken == address(0)) {
                ...
                if(sell.agent!= address(0) && sell.agentFee > 0){
                    ...
                    uint agentFee = SafeMath.div(SafeMath.mul(sell.agentFee, price), INVERSE_BASIS_POINT);
                ...
                if (sharedProxyAddresses[sell.target]) {
                    uint originatorFee = getOriginatorFee(sell.callData);

                    if(originatorFee > 0){
                        require(originatorFee <= maximumOriginatorFee, "41");

                        address originator = getOriginator(sell.callData);
                ...
                if(sell.agent!= address(0) && sell.agentFee > 0){
                    require(sell.agentFee <= maximumAgentFee, "44");
                ...
                if (sharedProxyAddresses[sell.target]) {
                    uint originatorFee = getOriginatorFee(sell.callData);
                    if(originatorFee > 0){
                        require(originatorFee <= maximumOriginatorFee, "42");
