...
// File: contracts/UNVFactory.sol
pragma solidity =0.7.4;
pragma experimental ABIEncoderV2;

contract UNVFactory is Ownable {
  address private tokenImplementation;
  address public merkleRootSigner;
  address public feeCollector;
  uint256 public fee; // is fee must be apply  for all unv contracts or for created after changing, add 50000
...
// File: contracts/UNV.sol
pragma solidity =0.7.4;

contract UNV is ERC20Upgradeable, OwnableUpgradeable {
  using SafeMathUpgradeable for uint256;
  ...
  function addMerkleRoot(
    ...
    require(
      recoveredAddress != address(0) &&
        recoveredAddress == UNVFactory(factory).merkleRootSigner(),
      "Invalid signature"
    );
  ...
  function claimProjectTokensByFeeCollector() external {
    ...
    require(
      ...
      "Cammot claim project tokens before allowed date"
    );
    require(
      _msgSender() == UNVFactory(factory).feeCollector(),
      "Cammot claim project tokens if caller is not fee collector"
    );

    uint256 amount = redeemToken.balanceOf(address(this));
    redeemToken.transfer(UNVFactory(factory).feeCollector(), amount);
  }
    ...
    returns (bool)
  {
    ...
    if (overridenFee == uint256(-1)) {
      feeFromAmount = amount.mul(UNVFactory(factory).fee()).div(100000);
    } else {
      feeFromAmount = amount.mul(overridenFee).div(100000);
    }

    super.transfer(UNVFactory(factory).feeCollector(), feeFromAmount);
