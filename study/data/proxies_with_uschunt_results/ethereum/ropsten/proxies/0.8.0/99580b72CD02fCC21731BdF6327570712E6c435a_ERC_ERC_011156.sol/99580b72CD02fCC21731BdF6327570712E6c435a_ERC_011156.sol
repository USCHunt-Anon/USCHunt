...
// main contract []
contract ERC_011156 is Context, IERC20, Ownable, ReentrancyGuard {
    // public properties
        // extend
        ...
        // addresses
            ...
            // taxes
                ...
                uint256 public TAX_LIQUIDITY_TOTAL = 0;
    ...
    // private properties
        // tokenomics
            string private _name = "ERC_011156";
            string private _symbol = "ERC011156";
    ...
    // events
        ...
        event EventBurn(address indexed from, uint256 burn);
        event EventSellForTaxesReward(uint256 value);
        event EventSwapAndLiquifyReward(uint256 tokens, uint256 reward);
        event EventSwapAndLiquifyAddLiquidity(uint256 token0, uint256 token1);
        event EventContractNativeBalance(uint256 value);
    ...
    function decreaseAllowance(address spender, uint256 subtractedValue) public virtual returns (bool){
        _approve(_msgSender(), spender, _allowances[_msgSender()][spender].sub(subtractedValue, "ERC_011156 decreased allowance below zero"));
        return(true);
    }
    ...
    function transferFrom(address sender, address recipient, uint256 amount) public override returns(bool) {
        ...
        _approve(sender, _msgSender(), _allowances[sender][_msgSender()].sub(amount, "ERC_011156 transfer amount exceeds allowance"));
        return(true);
    }
    ...
    // setter
        ...
        function setTaxesEnabled() public onlyOwner returns(bool liquidity){
            // enable the collection of taxes
            _switchTaxes = true;
            return(_switchTaxes);
        }

        function setTaxesDisabled() public onlyOwner returns(bool liquidity){
    ...
    function _approve(address owner, address spender, uint256 amount) private {
        require(owner != address(0), "ERC_011156 approve from the zero address");
        require(spender != address(0), "ERC_011156 approve to the zero address");
    ...
    function _transfer(address from, address to, uint256 amount) private{
        ...
        if(!_switchTaxes || (_accountNoTaxes[from] || _accountNoTaxes[to])){
            ...
            if(_switchTaxLiquidity){
                ...
                TAX_LIQUIDITY_TOTAL = TAX_LIQUIDITY_TOTAL.add(liquidity);
            ...
            emit EventTransferWithTaxes(from, to, amount, taxes);
        }
        ...
        if(_switchTaxes && !_swapLocked && tokens > _minTokensToSell){
    ...
    function _transactionSwapAndLiquify(uint256 amount) private swapLocking returns(bool){
        ...
        if(_switchTaxLiquidity){
            ...
            fullTokenSwap = TAX_LIQUIDITY_TOTAL;
            halfToken0 = TAX_LIQUIDITY_TOTAL.div(2);
            halfToken1 = TAX_LIQUIDITY_TOTAL.sub(halfToken0);
        ...
        emit EventSwapAndLiquifyReward(tokensToSell, nativeDeltaAfterSwap);
        ...
        if(_switchTaxLiquidity){
            ...
            emit EventSwapAndLiquifyAddLiquidity(halfToken1, nativeAddLiquidity);
            TAX_LIQUIDITY_TOTAL = TAX_LIQUIDITY_TOTAL.sub(fullTokenSwap);
        }

        emit EventContractNativeBalance(address(this).balance);
