...
// flattened :  Wednesday, 28-Apr-21 19:43:47 UTC
library SafeMath {
    /**
...
interface IVErc20 is IVBase, IErc20 {
    ...
    function setVInstitutionAddress(
        address _address,
        string memory nonce,
        uint8[2] calldata V,
        bytes32[2] calldata R,
...
abstract contract AVBase is IVBase {
    ...
    mapping(bytes32 => mapping(bytes32 => mapping(address => bool))) internal bytesToAddressToBoolStorage;
    ...
    function isEthereum(
        ...
        return (getVStoreChainId() == ETHEREUM_MAINNET || getVStoreChainId() == ETHEREUM_KOVAN || getVStoreChainId() == ETHEREUM_GOERLI);
    }

    function isMatic(
    ) internal view returns (bool) {
        return (getVStoreChainId() == MATIC_MUMBAI || getVStoreChainId() == MATIC_MAINNET);
    }
    ...
    function vStore(
        ...
        return IVStore(addressStorage[key("vStoreAddress")]);
    }

    function setVStoreAddress(
        address vStoreAddress
    ) internal {
        addressStorage[key("vStoreAddress")] = vStoreAddress;
    }

    function getVStoreChainId(
    ) internal view returns (uint256) {
        return vStore().chainId();
    }

    function vStoreAddress(
        string memory _key
    ) internal view returns (address) {
        return vStore().getAddress(_key);
    }
...
abstract contract AVProxy is IVProxy, AVBase {
    ...
    constructor (
        ...
        setVStoreAddress(vStoreAddress);
    }
    ...
    function implementationAddress(
        ...
        return vStoreAddress(implementationAddressKey());
    }
...
contract VErc20Proxy is AVProxy {
    constructor (
        ...
        address recoveryAddress,
        address vStoreAddress,
        address vFinanceAddress,
        string memory name,
        string memory symbol,
        address underlying,
        string memory institutionId
    ) AVProxy(vAddress, userAddress, recoveryAddress, "vErc20ImplementationAddress", vStoreAddress) {
        addressStorage[key("vFinanceAddress")] = vFinanceAddress;
