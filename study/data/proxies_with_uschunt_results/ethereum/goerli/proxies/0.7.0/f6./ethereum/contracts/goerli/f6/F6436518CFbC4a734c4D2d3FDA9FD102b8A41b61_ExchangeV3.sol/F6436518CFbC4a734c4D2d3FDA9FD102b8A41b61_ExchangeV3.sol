...
library ExchangeBalances
{
        ...
        returns (bool)
    {
        ...
        if (merkleProof.balanceLeaf.tokenID.isNFT()) {
            // Verify the NFT data
            uint minter = uint(merkleProof.nftData.minter);
            uint nftType = uint(merkleProof.nftData.nftType);
            uint token = uint(merkleProof.nftData.token);
            uint nftIDLo = merkleProof.nftData.nftID & 0xffffffffffffffffffffffffffffffff;
            uint nftIDHi = merkleProof.nftData.nftID >> 128;
            uint creatorFeeBips = merkleProof.nftData.creatorFeeBips;
...
library ExchangeWithdrawals
{
        ...
        public
    {
        ...
        if (!tokenID.isNFT()) {
            require(
                merkleProof.nftData.nftID == 0 && merkleProof.nftData.minter == address(0),
                "NOT_AN_NFT"
            );
            ...
            transferNFTs(
                ...
                owner,
                owner,
                merkleProof.nftData.nftID,
                balance,
                merkleProof.nftData.minter,
                merkleProof.nftData.nftType,
                merkleProof.nftData.token,
                new bytes(0),
                gasleft(),
                false
            );
        ...
        public
    {
        if (!tokenID.isNFT()) {
            ...
            if (!transferNFTs(
                ...
                S.amountWithdrawableNFT[to][nftMinter][nftType][nftToken][nftID] =
                    S.amountWithdrawableNFT[to][nftMinter][nftType][nftToken][nftID].add(amount);
            }
        }
    }
        ...
        returns (bool success)
    {
        if (token == minter) {
            ...
            success = ExchangeNFT.withdraw(from, to, nftType, token, nftID, amount, extraData, gasLimit);
        } else {
            // This is an inhouse NFT contract with L2 minting support
            try IL2MintableNFT(token).mintFromL2{gas: gasLimit}(to, nftID, amount, minter, extraData) {
                success = true;
            } catch {
                success = false;
            }
...
library NftDataTransaction
{
        ...
        pure
    {
        ...
        _offset += 2;
        nftData.nftID = data.toUintUnsafe(_offset);
        _offset += 32;
        nftData.creatorFeeBips = data.toUint8Unsafe(_offset);
        _offset += 1;
        nftData.nftType = ExchangeData.NftType(data.toUint8Unsafe(_offset));
        _offset += 1;

        if (nftData.scheme == uint8(NftDataScheme.SCHEME_WITH_MINTER_ADDRESS)) {
            nftData.minter = data.toAddressUnsafe(_offset);
        } else if (nftData.scheme == uint8(NftDataScheme.SCHEME_WITH_TOKEN_ADDRESS)) {
            nftData.tokenAddress = data.toAddressUnsafe(_offset);
        } else {
...
library NftMintTransaction
{
    ...
    bytes32 constant public NFTMINT_TYPEHASH = keccak256(
        "NftMint(address minter,address to,uint8 nftType,address token,uint256 nftID,uint8 creatorFeeBips,uint96 amount,uint16 feeTokenID,uint96 maxFee,uint32 validUntil,uint32 nonce)"
    );
        ...
        internal
    {
        ...
        if (auxiliaryData.length == 0) {
            ...
            require(mint.creatorFeeBips == 0, "CREATORFEEBIPS_NONZERO");

            // The minter should be the NFT token contract for deposits
            require(mint.minter == mint.tokenAddress, "MINTER_NOT_TOKEN_CONTRACT");

            // Process the deposit
            ExchangeData.Deposit memory pendingDeposit = S.pendingNFTDeposits[mint.to][mint.nftType][mint.tokenAddress][mint.nftID];
            ...
            // and the owner receives a gas refund.
            if (pendingDeposit.amount == 0) {
                delete S.pendingNFTDeposits[mint.to][mint.nftType][mint.tokenAddress][mint.nftID];
            } else {
                S.pendingNFTDeposits[mint.to][mint.nftType][mint.tokenAddress][mint.nftID] = pendingDeposit;
            }
        } else {
            // The minter should NOT be the NFT token contract for L2 mints
            require(mint.minter != mint.tokenAddress, "MINTER_EQUALS_TOKEN_CONTRACT");
            ...
            // Check the on-chain authorization
            S.requireAuthorizedTx(mint.minter, auxData.signature, txHash);
        }
    }
        ...
        pure
    {
        ...
        require(data.toUint8Unsafe(_offset) == 1, "INVALID_AUXILIARY_DATA");
        _offset += 1;
        ...
        _offset += 12;
        mint.nonce = data.toUint32Unsafe(_offset);
        _offset += 4;
    }
...
contract ExchangeV3 is IExchangeV3, ReentrancyGuard, ERC1155Holder, ERC721Holder
{
    ...
    function getPendingDepositAmountNFT(
        address               owner,
        address               token,
        ExchangeData.NftType  nftType,
        uint256               nftID
