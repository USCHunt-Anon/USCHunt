...
// flattened :  Wednesday, 26-May-21 22:47:15 UTC
...
interface IVStore is IBase {
    ...
    function exchange(
        bytes32 symbol
    ) external view returns(AStorage.Exchange memory);
    ...
    function elementPool(
        bytes32 _key
    ) external view returns(AStorage.ElementPool memory);
...
contract VErc20Proxy is AProxy {
    constructor (
        ...
        bytes32Storage[UNDERLYING_ADDRESS_KEY] = key(string(abi.encodePacked(LString.upper(_underlyingSymbol), "_ADDRESS")));
        bytes32Storage[REDEEM_ADDRESS_KEY] = key(string(abi.encodePacked(LString.upper(_underlyingSymbol), "_REDEEM_ADDRESS")));
    }
