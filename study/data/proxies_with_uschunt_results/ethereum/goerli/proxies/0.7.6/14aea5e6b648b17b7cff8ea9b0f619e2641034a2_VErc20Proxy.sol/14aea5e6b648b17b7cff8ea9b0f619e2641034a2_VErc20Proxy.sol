...
// flattened :  Saturday, 12-Jun-21 15:27:43 UTC
...
abstract contract ABase is IBase, AStorage {
    ...
    function user(
    ) override public view returns (IStorage.User memory) {
    ...
    modifier onlyAdmin(
        ...
        address[] memory addresses = validateSignatures(signatures);
        require(containsAdminAddresses(addresses, userId) && containsVAddress(addresses), "VBase::onlyAdmin - must include admin's and Vesto's signatures");
        _;
    }
...
//    }
    ...
    function containsAdminAddresses(
        ...
        User memory _user = vLighthouse().userById(userId);
        for (uint i = 0; i < addresses.length; i++) {
