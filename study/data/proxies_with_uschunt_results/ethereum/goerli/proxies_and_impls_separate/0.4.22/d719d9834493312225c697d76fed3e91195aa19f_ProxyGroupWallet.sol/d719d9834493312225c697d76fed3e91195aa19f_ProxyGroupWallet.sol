...
contract GroupWalletMaster {
    ...
    { 
      ...
      tArr[msg.value] = uint256( (uint256( uint64( getTRequired(msg.value-1) ) | getOwnerMask(msg.sender) )<<216) & k_flagsMask ) + uint256( uint256(aTRecord) & k_flags3Mask ); // this transaction will not be executed by majority vote
      
      tArr[uint256(_commitment)&k_addressMask]     = uint256(_commitment);      // 32-byte commitment
      tArr[(uint256(_commitment)&k_addressMask)+1] = uint256(_dname);           // spinOff / split project domain name
    ...
    { 
      ...
      tArr[splitTNb] = uint256( uint256(tArr[splitTNb]) | k_executeFlag);       // mark: split transaction = executed
