...
contract TokenMaster2 {
    ...
    function drainShares(bytes32 dHash, address _GWF, address from, address toReceiver) external {
      ...
      require(AbstractGroupWalletFactory(_GWF).getProxyToken(dHash)==address(this)&&address(uint160(ownerPrices & k_aMask))==_GWF,"unknown group!");
      require(AbstractGroupWalletFactory(_GWF).getIsOwner(dHash,from),"not an owner!");
      require(AbstractGroupWalletFactory(_GWF).getGWProxy(dHash)==msg.sender,"only GWP!");
