...
contract GroupWalletMaster {
    ...
    function confirmAndExecute_68(uint _tId) external payable
    {
        ...
        my_require(uint8( (uint256( uint256( t ) & k_assetMask )>>208) & k_asset2Mask )<128, "68 notExecuted!!!");
    ...
    function submitTransaction_Hom(uint aTRecord) external payable
    {
      my_require(isAddressOwner(msg.sender),"ownerExists!!!");
      if (msg.value==0) return;
      my_require(tArr[msg.value] == 0,"tArr overwrite error!!!");
    ...
    function getMemberWelcome(address target) external view returns (bytes memory) { // ** problem ** ********** only for addMember() members *****
      my_require(isAddressOwner(target),"target NOT an owner!!!");
    ...
    function submitTransaction_addUser(uint256 aTRecord, uint256 dhash, uint256 labelHash, uint256 memK1, uint256 memK2, bytes calldata data) external payable
    {
      ...
      my_require(tArr[msg.value] == 0,"tArr overwrite error!!!");
      my_require((aTRecord & k_typeMask)>>252 == 2,"action == addUser!!!");
    ...
    function callExecution(uint _tId,uint256 t,uint64 f) internal {
      ...
      if (typ == 5) {                                                           // changeRequirement
        ...
        my_require((majority>=2) && (majority<=MAX_OWNER_COUNT),"required 2-31!!!");
      ...
      if (typ == 1) {
        ...
        else
        {
          ...
          my_require(succ==true, string(abi.encode( returnData, returnData.length )));
        }
        return;
      }
      else
      {
        if (typ == 2)                                                           // addOwner
        {
          ...
          my_require((owners.length+1) <= MAX_OWNER_COUNT && r <= (owners.length + 1) && r != 0 && r >= 2,"validRequirement!!!");
    ...
    function getOwnerMask(address _owner) private view returns (uint64 mask) {      
      ...
      my_require(1==2,"Owner NOT found!");
    }
...
//  
//      if (msg.sender!=owners[ uint(uint256(msgData) & k_asset2Mask) ]) my_require(1==2,"sendMessage, invalid owner!!!");
...
//    }
    ...
    function saveAsset(bytes32 dhash, uint256 asset, string calldata key, string calldata data) external payable
    {
      ...
      my_require(dhash != 0x0,                                        " - Domain hash missing!");
      my_require(getENS().recordExists(dhash),                        " - Domain does NOT exist!");
      my_require(getENS().owner(dhash)==address(this),                " - Only domain owner can save assets!");

      if (asset==3) {                                                           // KeyValue on memberDomain s.a. 'vitalik.ethereum.eth'
      ...
      else
      {
        ...
        if (pHash>0) {
          my_require(getENS().owner( bytes32(pHash) ) == address(this), " - GWP contract is NOT domain owner!");
          my_require(bytes32(pHash)==dhash,                             " - Domain hash unexpected!");
        }
    ...
    function() external payable
    {
      my_require(false,"GroupWalletMaster fallback! No direct payments.");
    }
