...
contract ERC20Upgradeable is Initializable, ContextUpgradeable, IERC20Upgradeable, IERC20MetadataUpgradeable {
     ...
     *通话条件：
     *-当'from'和'to'均为非零时，'from'的代币'amount'将被转移到'to'。
     *-当'from'为零时，将为'to'铸造'amount'代币。
     *-当'to'为零时，``来自``的代币的'amount`将被烧掉。
     *-“from”和“to”永远都不是零。
     * Calling conditions:
     *
...
contract AS_Token is ERC20Upgradeable,OwnableUpgradeable {
    ...
    bool public contractStatus = false;  //upgrade add 合约正常才能转

	function initialize(string calldata name,string calldata symbol)public initializer{
        __ERC20_init_unchained(name, symbol);  //代币的名称、符号
        __Context_init_unchained();
        __Ownable_init_unchained();
    ...
    // add upgrade
    // function setContactStatus(bool _b) external {
    //     contractStatus = _b;
    // }
}