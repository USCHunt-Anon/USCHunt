...
//STATE
    ...
    function initialize(address _dev1Address, address _dev2Address, uint _maxSupply, uint _initsupply, bool _init, bool _gameactive ) external {
       ...
       INIT_SUPPLY          = _initsupply;
       initialized          = _init;
       isGameActive         = _gameactive;
     
    }

    function setAddresses(address _whitelist, address _ren, address _inventory, address _campaigns, address _terminus)  public {
       onlyOwner();
...
//MINT
    ...
    function presale(uint256 _reserveAmount, address _whitelister) external payable {      
        isPlayer();
        
        require(msg.sender == whitelist);
...
//INTERNALS
        ...
        function _mintElf(address _to) private returns (uint16 id) {
            ...
            {        
                ...
                elf.race = rand % 100 > 95 ? 3 : uint16(_randomize(rand, "Race", id)) % 3;//////Make 3:woodborne more rare. 
                elf.hair = elf.race == 3 ? 0 : uint16(_randomize(rand, "Hair", id)) % 3;/// 0:brown white dark
                elf.accessories = uint16(_randomize(rand, "Accessories", id)) % 4; //4accessories
        ...
        private {
            ...
            uint256 rand = _rand();
                ...
                if(action == 0){//Unstake if currently staked
                    ...
                    // require(msg.value >= .001 ether);  CHANGE                      
...
//Bridge and Tunnel Stuff
    ...
    function modifyElfDNA(uint256 id, uint256 sentinel) external {
       // require (msg.sender == terminus, "not terminus"); CHANGE THIS
