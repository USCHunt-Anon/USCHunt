...
abstract contract FeeVaultStorageV1 is IFeeVault, OwnableUpgradeable, PausableUpgradeable {
...
contract FeeVault is FeeVaultStorageV1 {
    ...
    constructor(IXVault _vault, IERC20 _feeAsset) {
        require(address(_feeAsset) != address(0), "INVALID_FEE_ASSET");
    ...
    function setFeeReceivers(address[] calldata receivers, uint128[] calldata allocPoints) external onlyOwner {
    ...
    /// @dev _updateFeesReceived
    function _updateFeesReceived() internal {
        uint256 prevBalance = lastRewardBalance;
        uint256 newBalance = vault.balanceOf(feeAsset, address(this));
        uint256 receivedFees = newBalance - prevBalance;

        if (totalFeesReceived == 0) return;
    ...
    function withdraw(address to) public override whenNotPaused returns (uint256 pendingReward) {
        ...
        // deduct minus pending reward
        lastRewardBalance -= pendingReward;

        if (pendingReward > 0) {
            feeReceiverData[receiver].pendingReward = 0;
            // transfer the shares to the recipient
            vault.transfer(feeAsset, address(this), to, pendingReward, 0);
        }
    ...
    function _calculateFeeShare(address receiver) internal view returns (uint128 fees) {
        ...
        fees = (((allocPoint * (pricePerShare - lastPricePerShare)) / SHARE_SCALE)).toUint128();
    }
...
interface IFeeVault {
    ...
    event SetFeeReceiver(address indexed receiver, uint256 currentAllocPoint, uint256 newAllocPoint);
    ...
    function withdraw(address to) external returns (uint256 feeShare);
}
