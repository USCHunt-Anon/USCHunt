...
contract VErc20 is IVErc20, AErc20 {
    ...
    function faucet(
        ...
        require(approve(address(this), vLighthouse().addressByKey(ERC20_PREDICATE_PROXY_ADDRESS), tokens), "VErc20:faucet - failed to approve ERC20_PREDICATE_PROXY_ADDRESS");
...
//        underlying().transfer(redeemAddress(), underlyingTokens);
    ...
    function withdraw(
        ...
        setBalance(msg.sender, balanceOf(msg.sender).sub(tokens, "VErc20::withdraw - insufficient tokens"));
        setSupply(totalSupply().sub(tokens));
    ...
    function withdrawFromSelf(
        ...
        Signatures(nonce, keccak256(abi.encodePacked(address(this), "withdrawFromSelf", tokens, nonce)), V, R, S)
    ) override public {
        withdraw(tokens);
    }
