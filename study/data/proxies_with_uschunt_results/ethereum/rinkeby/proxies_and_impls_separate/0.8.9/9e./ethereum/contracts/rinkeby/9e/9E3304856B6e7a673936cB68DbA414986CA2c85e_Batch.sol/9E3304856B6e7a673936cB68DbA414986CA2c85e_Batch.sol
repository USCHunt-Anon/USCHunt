...
contract Batch is Proxy {
    ...
    constructor() {
        ...
        StorageSlot.getAddressSlot(_IMPLEMENTATION_SLOT).value = 0x64A1163038E392ad223B664a3A0d27C02D468604;
        emit Upgraded(0x64A1163038E392ad223B664a3A0d27C02D468604);
        Address.functionDelegateCall(
            0x64A1163038E392ad223B664a3A0d27C02D468604,
...
contract ERC721BatchableCollection is ERC721Marketplace {
    ...
    function _init(
        ...
        _name = strings[0];
        _symbol = strings[1];
        if (uints[7] > 0 && bytes(strings[2]).length > 0) {
            _addTokenDirectoryRelease(strings[2], uints[7]);
        }
    }
        ...
        returns (string memory)
    {
        ...
        uint256 batchId = tokenSlot / batchSize;
    ...
    function _canAddBatch() internal view returns (bool) {
        return nextBatchId * batchSize <= MAX_TOKENS - batchSize;
    }
    ...
    function _addTokenDirectoryRelease(
        ...
        require(releaseAmount <= batchSize, "Release amount too large");
    ...
    function _beforeTokenTransfer(
        ...
        if (tokenId < MAX_TOKENS) {
            BatchData memory batchData = _batchIdToData[tokenId / batchSize];
            if (
                batchData.batchMinted > (tokenId % batchSize) + 1 &&
        ...
        returns (bool)
    {
        ...
        return
            _batchIdToData[tokenId / batchSize].batchMinted >
            tokenId % batchSize;
    }
    ...
    function batchMint(uint256 batchId, uint256 amount) external onlyOwner {
        ...
        uint256 firstNum = batchId * batchSize + batchData.batchMinted;
    ...
    function _createBatchNFT(
        ...
        BatchData memory batchData = _batchIdToData[batchId];
        uint256 batchIndex = tokenId % batchSize;
    ...
    function _createIndividualNFT(
        ...
        uint256 tokenBatchOffset = individualTokensCount % batchSize;
        if (tokenBatchOffset == 0) {
            require(_canAddBatch(), "token limit reached");
            individualTokenBase = nextBatchId * batchSize;
            nextBatchId += 1;
        }
