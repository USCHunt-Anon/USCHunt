...
contract BLACK is MappableToken, Configurable {
    bytes32 internal constant _rebaseTime_      = "rebaseTime";
    bytes32 internal constant _rebasePeriod_    = "rebasePeriod";
    bytes32 internal constant _rebaseInflation_ = "rebaseInflation";
    ...
    function rebase() public {
        ...
        uint old = _totalSupply;
        _mint(address(config[_mine_]), old.mul(config[_rebaseInflation_]));

        emit Rebase(old, _totalSupply);
    }
    event Rebase(uint oldTotalSupply, uint newTotalSupply);
}
