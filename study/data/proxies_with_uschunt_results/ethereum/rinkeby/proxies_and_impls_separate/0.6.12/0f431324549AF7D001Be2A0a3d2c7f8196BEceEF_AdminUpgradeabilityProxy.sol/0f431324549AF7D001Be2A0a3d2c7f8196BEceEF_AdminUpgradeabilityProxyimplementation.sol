...
//import"openzeppelin-for-marketplace/introspection/IERC165.sol";
//import"openzeppelin-for-marketplace/token/ERC1155/IERC1155.sol";
//import"openzeppelin-for-marketplace/token/ERC721/IERC721.sol";
//import"openzeppelin-for-marketplace/token/ERC20/IERC20.sol";
//import"openzeppelin-for-marketplace/token/ERC20/SafeERC20.sol";
//import"openzeppelin-for-marketplace/math/SafeMath.sol";
...
interface ITokenRegistry {
    ...
    function allowedNFTs(address) external view returns (uint256);
}
...
contract Marketplace is OwnableUpgradeable, ReentrancyGuardUpgradeable, PausableUpgradeable {
    ...
    event UpdateTokenRegistry(address tokenRegistry);
...
//    bytes4 private constant _ERC721_RECEIVED = 0x150b7a02;
    ...
    /// @notice Listing owner -> Array of Listing items
    mapping(address => Listing[]) addressListings;

    Listing[] listings;

    uint32[] internal emptyIndexesInListings;

    /// @notice NftAddress -> Token ID -> Offers
    ...
    ITokenRegistry public tokenRegistryAddress;
    ...
    modifier offerNotExists(
        ...
        for(uint i=0; i < offers[_nftAddress][_tokenId].length; i++){
            if(offers[_nftAddress][_tokenId][i].offerer == _creator){
                Offer memory offer = offers[_nftAddress][_tokenId][i];        
                require(
    ...
    function initialize(
        ...
        // !!! Check if others needed for example address registry, tokenRegistry
        __Ownable_init();
    ...
    /// @param _startingTime scheduling for a future sale, time in seconds
    function listItem(
        address _nftAddress,
        uint256 _tokenId,
        uint256 _quantity,
        ...
        uint256 nftType = tokenRegistryAddress.allowedNFTs(_nftAddress);
    ...
    function cancelListing(
        ...
        (, bool found) = _removeListing(_nftAddress, _tokenId, _msgSender());
    ...
    /// @param _newQuantity New quantity for sale
    function updateListing(
        ...
        _validPayToken(_newPayToken);

        Listing[] storage _ownersListings = addressListings[_msgSender()];
        require(_ownersListings.length > 0, "item not found");
        bool found = false;
        for(uint256 i =0; i < _ownersListings.length; i++) {

            if (_ownersListings[i].tokenId == _tokenId && _ownersListings[i].contractAddress == _nftAddress) {
                
                _ownersListings[i].payToken = _newPayToken;
                _ownersListings[i].pricePerItem = _newPrice;
                _ownersListings[i].quantity = _newQuantity;
                uint internalIndex = _ownersListings[i].internalIndex;
                // Updating an array
                listings[internalIndex].payToken = _newPayToken;
                listings[internalIndex].pricePerItem = _newPrice;
                listings[internalIndex].quantity = _newQuantity;
                found = true;
                break;
            }
        }
    ...
    /// @param _owner owner of the listed NFT        
    function buyItem(
        address _nftAddress,
        uint256 _tokenId,
        address _payToken,
        ...
        whenNotPaused
    {
        (Listing memory _listing, bool found) = _removeListing(_nftAddress, _tokenId, _owner);
        require(found == true, "item not found");        
        require(_listing.payToken == _payToken, "invalid pay token");
        require(block.timestamp >= _listing.startingTime, "item not buyable");

        _validOwner(_listing.contractAddress, _listing.tokenId, _listing.owner, _listing.quantity);
    ...
    /// @param _deadline Offer expiration time, in seconds
    function createOffer(
        ...
        uint256 _pricePerItem,
        uint256 _deadline
    ) external nonReentrant whenNotPaused offerNotExists(_nftAddress, _tokenId, _msgSender()) {

        require(tokenRegistryAddress.allowedNFTs(_nftAddress) != 0, "invalid nft address");
        require(_deadline > block.timestamp, "invalid expiration");

        _validPayToken(address(_payToken));

        offers[_nftAddress][_tokenId].push(Offer(
            _msgSender(),
            _payToken,
            _quantity,
            _pricePerItem,
    ...
    /// @param _nftAddress NFT contract address
    /// @param _tokenId TokenId
    function cancelOffer(address _nftAddress, uint256 _tokenId) external whenNotPaused {
        bool exists = false;        
        for(uint i=0; i < offers[_nftAddress][_tokenId].length; i++){
            if(offers[_nftAddress][_tokenId][i].offerer == _msgSender()){
                delete (offers[_nftAddress][_tokenId][i]);
                exists = true;
            }
        }
        require(exists == true, "no such offer");
    ...
    function acceptOffer(
        ...
        for(uint i=0; i < offers[_nftAddress][_tokenId].length; i++){
            if(offers[_nftAddress][_tokenId][i].offerer == _creator){
                offer = offers[_nftAddress][_tokenId][i];        
                require(
                    offer.quantity > 0 && offer.deadline > block.timestamp,
                    "no such offer or expired"
                );            
            }            
        }
        require(offer.quantity > 0 && offer.deadline > block.timestamp, "no such offer");

        _validOwner(_nftAddress, _tokenId, _msgSender(), offer.quantity);

        uint256 price = offer.pricePerItem.mul(offer.quantity);
        uint256 feeAmount = price.mul(platformFee).div(100000);

        require(IERC20(offer.payToken).transferFrom(_creator, address(this), price));
        IERC20(offer.payToken).approve(treasuryAddress, feeAmount);
        ITokenPaymentSplitter(treasuryAddress).split(address(offer.payToken), _msgSender(), feeAmount);
        
        offer.payToken.safeTransfer(
            _msgSender(),
            price.sub(feeAmount)
        );

        // Transfer NFT to buyer and delete all offers, if it is NFT
        ...
        _removeListing(_nftAddress, _tokenId, _msgSender());

    }

    function getOffers(address _nft, uint256 _tokenID) external view returns (Offer[] memory) {
    ...
    function getListingByOwner(address _owner) public view returns(Listing[] memory){
        return addressListings[_owner];
    }

    /**
     @notice Method for updating platform fee
     @dev Only admin
     @param _platformFee uint24 the platform fee to set  !!! (MUST be saved in the % * 1000 form, e.g. 2.755% = 2755)
     */
    function updatePlatformFee(uint24 _platformFee) external onlyOwner {
        platformFee = _platformFee;
    ...
    function updateTokenRegistry(address _registry) external onlyOwner {
        tokenRegistryAddress = ITokenRegistry(_registry);
        emit UpdateTokenRegistry(_registry);
    }
...
*/
    ...
    function _validPayToken(address _payToken) internal {
        require( tokenRegistryAddress.enabled(_payToken) == true, "invalid pay token");
    }
    ...
    function _validOwner(
        ...
        if (nftType == 721) {
            IERC721 nft = IERC721(_nftAddress);
            require(nft.ownerOf(_tokenId) == _owner, "not owner");
        } else if ( nftType == 1155) {
            IERC1155 nft = IERC1155(_nftAddress);
            require(nft.balanceOf(_owner, _tokenId) >= quantity, "not owner");
        } else {
            revert("invalid nft address");
        }
    }

    function _addListing(Listing memory _listing) private {
        Listing[] memory _ownerListings = addressListings[_msgSender()];

        for(uint256 i = 0; i < _ownerListings.length; i++) {
            Listing memory _l = _ownerListings[i];
            if(_l.contractAddress == _listing.contractAddress){
                require (_l.tokenId != _listing.tokenId ,"already listed"); 
            }
        }
        if(emptyIndexesInListings.length > 0){
            uint emptyIndex = uint(emptyIndexesInListings[emptyIndexesInListings.length-1]);
            listings[emptyIndex] = _listing;
            emptyIndexesInListings.pop();
            _listing.internalIndex = emptyIndex;
        } else {
            listings.push(_listing);
            _listing.internalIndex = listings.length-1;
        }
        addressListings[_msgSender()].push(_listing);
    }
    ...
    function _removeListing(
        ...
        uint256 _tokenId, 
        address _owner
    ) private returns(Listing memory, bool found) {

        Listing[] storage _ownersListings = addressListings[_owner];
        found = false;
        Listing memory _listing;
        if(_ownersListings.length <= 0){
            return (_listing,found);
        }
        for(uint256 i =0; i < _ownersListings.length; i++) {
            _listing = _ownersListings[i];

            if (_listing.tokenId == _tokenId && _listing.contractAddress == _contract) {
                // Removing from mapping
                uint mapLength = _ownersListings.length;
                if(mapLength > 1){
                    _ownersListings[i] = _ownersListings[mapLength-1];
                }
                _ownersListings.pop();

                // Removing from an array
                delete(listings[_listing.internalIndex]);
                emptyIndexesInListings.push(uint32(_listing.internalIndex));
                found = true;
                break;
            }
        }
    ...
    function _buyItem(Listing memory _listing) private {
       
        uint256 price = _listing.pricePerItem.mul(_listing.quantity);
        require(IERC20(_listing.payToken).balanceOf(_msgSender()) >= price, "Not enough amount to buy");

        uint256 feeAmount = price.mul(platformFee).div(100000);

        require(IERC20(_listing.payToken).transferFrom(_msgSender(), address(this), price));
        IERC20(_listing.payToken).approve(treasuryAddress, feeAmount);
        ITokenPaymentSplitter(treasuryAddress).split(_listing.payToken, _listing.owner, feeAmount);
        
        IERC20(_listing.payToken).safeTransfer(
            _listing.owner,
            price.sub(feeAmount)
        );

        // Transfer NFT to buyer
        if (IERC165(_listing.contractAddress).supportsInterface(INTERFACE_ID_ERC721)) {
            IERC721(_listing.contractAddress).safeTransferFrom(
                _listing.owner,
                _msgSender(),
                _listing.tokenId
            );
        } else {
            IERC1155(_listing.contractAddress).safeTransferFrom(
                _listing.owner,
                _msgSender(),
                _listing.tokenId,
                _listing.quantity,
...
//import"openzeppelin-for-marketplace/utils/Address.sol";
...
//import"openzeppelin-for-marketplace/token/ERC721/ERC721.sol";
//import"openzeppelin-for-marketplace/GSN/Context.sol";
//import"openzeppelin-for-marketplace/access/Ownable.sol";
...
//import"openzeppelin-for-marketplace/token/ERC20/ERC20.sol";
...
//!!! Check all these parameters
    ...
    /// @notice global bid withdrawal lock time //!!! Not working
    ...
    /// @notice Marketplace
    IMarketplace public marketplaceAddress;    
    ...
    modifier onlyMarketplace() {
        require(
            address(marketplaceAddress) == _msgSender(),
    ...
    function createAuction(
        ...
        require(
            _payToken == address(0) ||
                tokenRegistryAddress.enabled(_payToken),
            "invalid pay token"
        );
    ...
    /**
     ...
     @param _nftAddress ERC 721 Ad dress
     @param _tokenId Token ID of the item being auctioned
    ...
    /**
     @notice Allows the highest bidder to withdraw the bid (after 12 hours post auction's end) 
...
//            ),
    ...
    function updateMarketplace(address _marketplace) external onlyOwner {
        marketplaceAddress = IMarketplace(_marketplace);
    ...
    /**
     @notice Private method of creating an auction
     @param _nftAddress ERC 721 Address
     @param _tokenId Token ID of the NFT being auctioned
     @param _payToken Paying token
     @param _reservePrice Item cannot be sold for less than this or minBidIncrement, whichever is higher
