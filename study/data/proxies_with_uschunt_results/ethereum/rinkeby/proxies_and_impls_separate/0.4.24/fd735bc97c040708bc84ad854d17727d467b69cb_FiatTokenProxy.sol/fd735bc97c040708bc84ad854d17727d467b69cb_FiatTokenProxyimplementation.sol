...
contract Admin is Ownable {
    ...
    function setDailyDistribution(address[] memory stakedToken, address[] memory rewardToken, uint[] memory dailyDistribution) public onlyOwner {
        ...
        for(uint8 i=0; i< stakedToken.length; i++) {
             require(tokenDetails[stakedToken[i]].isExist == true && tokenDetails[rewardToken[i]].isExist == true,
            "Token not exist" );
...
contract Unifarm is Admin {
    ...
    uint256 public constant DAYS = 1 minutes;
    ...
    function viewAvailableRewards(address user, uint256 stakeId, address rewardTokenAddress) public view returns(uint availStake, uint availReward) {
        ...
        if (stakingDetails[user].endTime[stakeId] > block.timestamp) {
            ...
            interval = endOfProfit.sub(
                stakingDetails[user].startTime[stakeId]
            );
        } else if (
            stakingDetails[user].endTime[stakeId] <= block.timestamp
        ) {
        ...
        if (interval >= DAYS) {
            ...
            while (i < intervalDays.length) {
                if (noOfDays >= intervalDays[i]) {
                    ...
                    address rewardToken = tokensSequenceList[stakingDetails[user].tokenAddress[stakeId]][i];

                    if (
                        rewardToken != stakingDetails[user].tokenAddress[stakeId] &&
                        tokenBlockedStatus[stakingDetails[user].tokenAddress[stakeId]][rewardToken] == false
                    ) {
                        
                        if(rewardToken == rewardTokenAddress) {
                            
                            rewardsEarned = balDays.mul(getOneDayReward (
                                stakingDetails[user].stakedAmount[
                                    stakeId
                                ],
                                stakingDetails[user].tokenAddress[
                                    stakeId
                                ],
                                rewardToken
                                )
