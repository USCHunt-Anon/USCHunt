...
contract Dogewood is ERC721 {
    ...
    // level 1-9
    uint256[] public upgradeGearPrice;
    ...
    function setUpgradePrice(
        uint256[] calldata _upgradeLevelPrice,
        uint256[] calldata _upgradeGearPrice
    ) external onlyOwner {
        upgradeLevelPrice = _upgradeLevelPrice;
        upgradeGearPrice = _upgradeGearPrice;
    }
    ...
    function mintOG(address to, uint8 amount) public genesisMinting(amount) onlyOwner {
    ...
    function upgradeWithTreat(uint256 id, UpgradeTypes type_)
        public
        ownerOfDoge(id)
        ...
        noCheaters
    {
        ...
        } else if (type_ == UpgradeTypes.LEVEL_MAINHAND) {
            uint8 curVal = doges[id].lvlMainhand;
            require(curVal < 9, "already max level");
            treat.burn(msg.sender, upgradeGearPrice[curVal]);
            doges[id].lvlMainhand = curVal + 1;
        } else if (type_ == UpgradeTypes.LEVEL_OFFHAND) {
            uint8 curVal = doges[id].lvlOffhand;
            require(curVal < 9, "already max level");
            treat.burn(msg.sender, upgradeGearPrice[curVal]);
            doges[id].lvlOffhand = curVal + 1;
        } else if (type_ == UpgradeTypes.LEVEL_ARMOR) {
            uint8 curVal = doges[id].lvlArmor;
            require(curVal < 9, "already max level");
            treat.burn(msg.sender, upgradeGearPrice[curVal]);
            doges[id].lvlArmor = curVal + 1;
        }
    }
