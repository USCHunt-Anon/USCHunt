...
contract EtherOrcs is ERC721 {
    ...
    function setAddresses(address mig, address meta) external {
    ...
    function mint() public noCheaters returns (uint256 id) {
        require(openForMint, "not open for minting");
    ...
    function migrationAction(uint256 id, address owner_, Actions action_) public {
        ...
        //Add a time bonus
        activities[id].timestamp = uint88(block.timestamp - 60 minutes);
        activities[id].owner = owner_;
    }
...
contract ERC721 {
    ...
    string public name = "Ether Orcs";
    
    string public symbol = "Orcs";
...
contract Migrator {
    ...
    function initialize(address oldOrcs_,address newOrcs_, address zug_) public {
    ...
    function justMigrate(uint256 tokenId) external {
        _migrate(tokenId);
    }

    function _migrate(uint256 tokenId) internal {
        require(oldOrcs.ownerOf(tokenId) == msg.sender, "Not owner");
        ...
        oldOrcs.transferFrom(msg.sender, address(this), tokenId);
        ...
        // trasfer to burner account
        oldOrcs.transfer(burningAddress, tokenId);

        // if (block.timestamp - 25 hours < startingTime) zug.mint(msg.sender,1 ether);
    } 
