...
contract SpaceStation is ReentrancyGuard, EIP712 {
    ...
    function initialize(address eoa, address multisig) external {
        require(!initialized, "Contract already initialized");
        galaxy_signer = eoa;
        // use eoa in prd, to reduce activate_campaign tx gas cost
        campaign_setter = eoa;
        // access control, pause/unpause
        manager = multisig;
        treasury_manager = multisig;
    ...
    function claimDrop(IMerkleDistributor _merkleDistributor, uint256 index, address account, uint256 amount, bytes32[] calldata merkleProof) external {
        IMerkleDistributor(_merkleDistributor).claim(index, account, amount, merkleProof);
    }
