...
contract VirgoInstantLiquidity is Pausable, ReentrancyGuard {
    ...
    uint256 public constant signedCount = 2;    
    ...
    event CloseTransactions(uint256 transacionId);
    ...
    function initialize(address[3] memory _managers)  public initializer {        
    ...
    function updateOperator(address _operatorAddress) external onlyOwner {
    require(_operatorAddress!= address(0) && _operatorAddress != operator, "You can not change operator to zero address or same  operator address");
        operator = _operatorAddress;
        emit UpdateOperator(_operatorAddress);
    }

     //update nft owner address
    function updateNFTOwner(address _nftOwnerAddress) external onlyOwner {
    require(_nftOwnerAddress!= address(0) && !_nftOwnerAddress.isContract(), "You can not change nft ownerAddress to zero address or contract based address");
   ...
   //Get the balance of contract
    ...
    function getWithdrawInfo(uint256 _transactionId) external isManager view returns(
        address _from,
        address _to,
        uint256 _amount,
    ...
    function signTransaction(uint256 _transactionId) external isManager{
        ...
        if(transaction.signatureCount >= signedCount ){       
            ...
            payable(transaction.to).sendValue(transaction.amount); 
    ...
    function closeTransactions(uint256 _transacionId) public isManager{        
        ...
        emit CloseTransactions(_transacionId);
    }
