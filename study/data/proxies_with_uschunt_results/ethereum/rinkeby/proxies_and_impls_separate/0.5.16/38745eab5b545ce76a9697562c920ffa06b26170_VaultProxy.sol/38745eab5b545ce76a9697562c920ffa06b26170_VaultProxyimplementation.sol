...
// File @openzeppelin/contracts-ethereum-package/contracts/utils/ReentrancyGuard.sol@v2.5.0
...
contract VaultStorage is Initializable {
    ...
    bytes32 internal constant _TOTAL_SUPPLY_CAP_SLOT =
        0x138459372b9025a8c1e12a6475fdc7d2ce9c8afb75d5c05b2f0767a663aab39b;
    bytes32 internal constant _REQUIRED_XFORCE_TO_DEPOSIT_SLOT =
        0x8c4d03a0411d2979c24c85f28ab76c3e8fe536b0c4df132ab0774b8e894930e1;
    bytes32 internal constant _XFORCE_SLOT =
        0x748b19309761ca66457487fc6b7a404296135a82c410ce24e4fee447754a9256;
    ...
    constructor() public {
        ...
        assert(
            _TOTAL_SUPPLY_CAP_SLOT ==
                bytes32(
                    uint256(keccak256("eip1967.vaultStorage.totalSupplyCap")) -
                        1
                )
        );
        assert(
            _REQUIRED_XFORCE_TO_DEPOSIT_SLOT ==
                bytes32(
                    uint256(
                        keccak256(
                            "eip1967.vaultStorage.requiredXForceToDeposit"
                        )
                    ) - 1
                )
        );
        assert(
            _XFORCE_SLOT ==
                bytes32(
                    uint256(
                        keccak256("eip1967.vaultStorage.profitSharingPool")
                    ) - 1
                )
        );
    }
    ...
    function initialize(
        ...
        uint256 _totalSupplyCap_,
        uint256 _requiredXForceToDeposit_,
        address _xForce_
    ) public initializer {
        _setUnderlying(_underlying);
        _setVaultFractionToInvestNumerator(_toInvestNumerator);
        _setVaultFractionToInvestDenominator(_toInvestDenominator);
        _setTotalSupplyCap(_totalSupplyCap_);
        _setRequiredXForceToDeposit(_requiredXForceToDeposit_);
    ...
    function _setXForce(address _address) internal {
        setAddress(_XFORCE_SLOT, _address);
    }

    function _xForce() internal view returns (address) {
        return getAddress(_XFORCE_SLOT);
    }
    ...
    function _setTotalSupplyCap(uint256 _value) internal {
        setUint256(_TOTAL_SUPPLY_CAP_SLOT, _value);
    }

    function _totalSupplyCap() internal view returns (uint256) {
        return getUint256(_TOTAL_SUPPLY_CAP_SLOT);
    }

    function _setRequiredXForceToDeposit(uint256 _value) internal {
        setUint256(_REQUIRED_XFORCE_TO_DEPOSIT_SLOT, _value);
    }

    function _requiredXForceToDeposit() internal view returns (uint256) {
        return getUint256(_REQUIRED_XFORCE_TO_DEPOSIT_SLOT);
    }
    ...
    ReentrancyGuard
{
    ...
    event XForceChanged(address newXForce, address oldXForce);
    ...
    modifier vipGuard() {
        require(
            _requiredXForceToDeposit() == 0 ||
                (_xForce() != address(0) &&
                    IERC20(_xForce()).balanceOf(msg.sender) >=
                    _requiredXForceToDeposit()),
            "Must hold required xFORCE amount"
        );
        _;
    }
    ...
    function initializeVault(
        ...
        uint256 _toInvestNumerator,
        uint256 _toInvestDenominator,
        uint256 _totalSupplyCap, // Zero = unlimit
        uint256 _requiredXForceToDeposit, // Zero = unlimit
        address _xForce_
    ) public initializer {
        require(
    ...
    function xForce() external view returns (address) {
        return _xForce();
    }
    ...
    function requiredXForceToDeposit() public view returns (uint256) {
        return _requiredXForceToDeposit();
    }
    ...
    function setXForce(address _newXForce) public onlyGovernance {
        require(_newXForce != address(0), "new _xForce cannot be empty");
        emit XForceChanged(_newXForce, _xForce());
        _setXForce(_newXForce);
    }
    ...
    function setRequiredXForceToDeposit(uint256 value) external onlyGovernance {
        return _setRequiredXForceToDeposit(value);
    }
     ...
     * Approval is assumed.
     */
    function deposit(uint256 amount) external defense vipGuard {
    ...
    function _deposit(
        ...
        require(
            totalSupplyCap() == 0 ||
                totalSupply().add(toMint) <= totalSupplyCap(),
