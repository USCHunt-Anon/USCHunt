    ...
    ReentrancyGuardUpgradeable
{
    ...
    uint16 private constant HUNDRED_PERCENT = 100;
    ...
    mapping(uint256 => mapping(address => FarmingPendingEarlyWithdrawal))
        public farmingPendingEarlyWithdrawals;
    ...
    event StableFarmingEarlyPendingWithdraw(
        uint256 indexed poolId,
        address indexed account,
        uint256 amount,
        uint256 reward,
    ...
    event FarmingSetEarlyWithdrawTier(
        uint256 poolId,
        uint16 percentageKey,
        uint16 percentageValue
    );
    event FarmingSetDelayDuration(uint256 poolId, uint256 duration);

    event AddWhiteList(uint256 poolId, address[] addresses);
    event RemoveWhiteList(uint256 poolId, address[] addresses);
    event FarmingAddEarlyWhitelist(uint256 poolId, address account);
    event FarmingRemoveEarlyWhitelist(uint256 poolId, address account);
    event FarmingEarlyWithdraw(
        uint256 poolId,
        address account,
        uint256 amount,
        uint256 reward,
    ...
    struct FarmingPendingEarlyWithdrawal {
        uint256 reward;
        uint256 amount;
        uint256 applicableAt;
    }
    ...
    function getFarmingPendingEarlyWithdrawals(uint256 _poolId, address _user)
        external
        view
        returns (FarmingPendingEarlyWithdrawal memory)
    {
        return farmingPendingEarlyWithdrawals[_poolId][_user];
    }

    function isEarlyWithdrawWhitelisted(uint256 _poolId, address _user)
        external
        view
        returns (bool)
    {
    ...
    function farmingSetEarlyDelayDuration(uint256 _poolId, uint256 _duration)
        external
        onlyAdmin
        stableFarmingValidatePoolById(_poolId)
    {
        additionalPoolInfo[_poolId].delayDuration = _duration;
        emit FarmingSetDelayDuration(_poolId, _duration);
    }
    ...
    function farmingSetListEarlyWithdrawTier(
        uint256 _poolId,
        uint16[] calldata _percentageKeys,
        uint16[] calldata _percentageValues
    ) external onlyAdmin stableFarmingValidatePoolById(_poolId) {
        uint256 length = _percentageKeys.length;
        require(
            length == _percentageValues.length,
            "stableFarmingStakingPool: length not match"
        );
        ...
        for (uint256 i = 0; i < length; i++) {
            ...
            emit FarmingSetEarlyWithdrawTier(
                _poolId,
                _percentageKeys[i],
                _percentageValues[i]
            );
    ...
    function farmingAddEarlyWhitelist(
        uint256 _poolId,
        ...
        for (uint256 i = 0; i < length; i++) {
            ...
            emit FarmingAddEarlyWhitelist(_poolId, _accounts[i]);
        }
    }

    function farmingRemoveEarlyWhitelist(
        uint256 _poolId,
        address[] calldata _accounts
    ) external onlyAdmin stableFarmingValidatePoolById(_poolId) {
        uint256 length = _accounts.length;
        ...
        for (uint256 i = 0; i < length; i++) {
            ...
            emit FarmingRemoveEarlyWhitelist(_poolId, _accounts[i]);
        }
    }

    function farmingEarlyWithdraw(uint256 _poolId, uint16 _percentage)
        external
        nonReentrant
        whenNotPaused
        ...
        stableFarmingValidatePoolById(_poolId)
    {
        ...
        StableFarmingStakingData storage stakingData = stableFarmingStakingData[_poolId][
            account
        ];
        ...
        require(
            ...
            "stableFarmingStakingPool: invalid percentage"
        );
        require(
            poolInfo.isWhitelisted[account],
            "stableFarmingStakingPool: not whitelisted"
        );

        FarmingPendingEarlyWithdrawal
            storage pending = farmingPendingEarlyWithdrawals[_poolId][account];
        ...
        pool.stableFarmingAcceptedToken.safeTransfer(
            treasury,
            penaltyFee
        );

        // emit event
        emit FarmingEarlyWithdraw(_poolId, account, amount, reward, penaltyFee);
     ...
     */
    function stableFarmingUpdatePoolAfterStarted(
        uint256 _poolId,
        uint256 endJoinTime
    ) external whenNotPaused onlyAdmin {
        StableFarmingPoolInfo storage pool = stableFarmingPoolInfo[_poolId];
        ...
        emit StableFarmingPoolUpdated(
            _poolId,
            pool.stableFarmingAcceptedToken,
            pool.stableFarmingRewardToken,
            pool.cap,
            pool.APR,
            pool.startJoinTime,
            pool.endJoinTime,
     ...
     * @param _poolId id of the pool
     */
    function stableFarmingClaimPendingEarlyWithdraw(uint256 _poolId)
        external
        whenNotPaused
        nonReentrant
        stableFarmingValidatePoolById(_poolId)
        notInBlackList(msg.sender)
    {
        StableFarmingPoolInfo storage pool = stableFarmingPoolInfo[_poolId];
        address account = msg.sender;
        FarmingPendingEarlyWithdrawal storage pending = farmingPendingEarlyWithdrawals[_poolId][account];
        uint256 amount = pending.amount;
        ...
        delete farmingPendingEarlyWithdrawals[_poolId][account];
        pool.stableFarmingAcceptedToken.safeTransfer(account, amount);
        pool.stableFarmingRewardToken.safeTransfer(account, reward);

        emit StableFarmingEarlyPendingWithdraw(_poolId, account, amount, reward, block.timestamp);
    }
