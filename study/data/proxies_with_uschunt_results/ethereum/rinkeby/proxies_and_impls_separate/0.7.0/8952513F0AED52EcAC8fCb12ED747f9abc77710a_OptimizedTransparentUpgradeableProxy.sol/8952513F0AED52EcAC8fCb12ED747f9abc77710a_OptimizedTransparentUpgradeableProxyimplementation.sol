...
contract FeesAndRulePool is LinearPool {
    ...
    function deposit(
        ...
        bytes32 msgHash = getMessageHash(_poolId, msg.sender, _maxAmount);

        require(
            _verifySignature(msgHash, _signature),
            "FeePool: invalid signature"
        );

        uint256 _amountLeft = _calcAmountAndDeposit(_amount);

        uint256 userStaked = linearBalanceOf(_poolId, msg.sender);

        require(
            userStaked + _amountLeft <= _maxAmount,
            "FeePool: stake higher than maximum"
        );
