    ...
    AccessControlUpgradeable
{
    ...
    function stableFinanceSetPool(
        ...
        uint256 _delayDuration,
        uint256 _APR
    ) external onlyRole(ADMIN) stableFinanceValidatePoolById(_poolId) {
        StableFinancePoolInfo storage pool = stableFinancePoolInfo[_poolId];

        pool.delayDuration = _delayDuration;
        pool.APR = _APR;

        emit StableFinancePoolUpdated(_poolId, _delayDuration, _APR);
    }
        ...
        stableFinanceValidatePoolById(_poolId)
    {
        ...
        stakingData.pendingReward += _stableFinancePendingReward(
            _poolId,
            _stakedId,
            account
        );
        ...
        stableFinanceValidatePoolById(_poolId)
    {
        ...
        uint256 pendingReward = ((record.balance *
            stakedTimeInSeconds *
            record.APR) / ONE_YEAR_IN_SECONDS) / 1e20;

        totalReward = pendingReward + record.pendingReward;
        ...
        require(
            totalReward > 0.01e18,
            "StableFinanceStakingPool: At least 0.01 reward token to claim"
        );
        ...
        stableFinanceRewardDistributorToken.safeTransferFrom(
     ...
     * @param _poolId id of the pool
     * @return totalReward earned reward of a user
     * @param _stakedId id of staking record
     */
    function stableFinancePendingReward(
        ...
        uint256 _stakedId,
        address _account
    ) public view stableFinanceValidatePoolById(_poolId) returns (uint256) {
        StableFinanceStakingData storage user = stableFinanceStakingData[
            _poolId
        ][_account];
        return
            _stableFinancePendingReward(_poolId, _stakedId, _account) +
            user.stakingDatas[_stakedId].pendingReward;
    }
    ...
    function transferRoleSuperAdmin(address account) external onlyRole(SUPER_ADMIN) {
