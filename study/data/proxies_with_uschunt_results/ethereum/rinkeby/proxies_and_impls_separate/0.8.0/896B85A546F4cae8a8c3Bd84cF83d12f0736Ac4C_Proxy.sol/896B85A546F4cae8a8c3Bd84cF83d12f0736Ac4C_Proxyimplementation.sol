...
/// @dev This contract exists only to move logic out of the MarketController so its deployed bytecode size
/// may be reduced
    ...
    ERC1155PresetMinterPauserUpgradeable
{
    ...
    event ERC1155ControllerInitialized(address owner, address controller);

    /// @dev The address of the MarketController contract which will be allowed to call
    ...
    // TODO we might not actually need this, since it looks like we never actually
    // use the option tokens' total supply values
    ...
    function __ERC1155Controller_init(
        ...
        _setupRole(MINTER_ROLE, _marketController);

        controller = _marketController;

        emit ERC1155ControllerInitialized(msg.sender, _marketController);
    }
    ...
    /// @dev This function is overriden only in order to add the concept of totalSupply to our ERC1155
    /// token
    /// @param to the address which will receive the minted token
