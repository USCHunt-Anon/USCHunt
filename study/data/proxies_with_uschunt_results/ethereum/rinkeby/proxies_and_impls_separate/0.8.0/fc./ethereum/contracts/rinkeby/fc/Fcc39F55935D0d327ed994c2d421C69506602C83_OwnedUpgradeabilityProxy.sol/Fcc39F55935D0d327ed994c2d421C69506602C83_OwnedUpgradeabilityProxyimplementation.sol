...
contract CAMA is Context, CustomOwnable, ERC165, IERC721, IERC721Metadata {//change the name at the end
     ...
     */
    function ownerOf(uint256 tokenId) public view virtual override returns (address) {
        require(_exists(tokenId), "ERC2309: Invalid TokenId");
    ...
    */
     ...
     */
    function approve(address to, uint256 tokenId) public virtual override {
        address owner = CAMA.ownerOf(tokenId);
        require(to != owner, "ERC2309: approval to current owner");

        require(_msgSender() == owner || CAMA.isApprovedForAll(owner, _msgSender()),
    ...
    function transferFrom(address from, address to, uint256 tokenId) public virtual override {
        //solhint-disable-next-line max-line-length
        require(_isApprovedOrOwner(_msgSender(), tokenId), "ERC2309: transfer caller is not owner nor approved");

        _transfer(from, to, tokenId);
    }

    function batchTransfer(address to, uint96 size) public virtual {
        require(_msgSender() == owner() || CAMA.isApprovedForAll(owner(), _msgSender()), "ERC2309: transfer caller is not owner nor approved");
        require(to != owner(), "ERC2309: Can't transfer batch to own address");
        _batchTransfer(owner(), to, size);
    }

    function batchTransferFrom(address from, address to, uint[] memory ids) public virtual {
        require(from == _msgSender() || CAMA.isApprovedForAll(owner(), _msgSender()), "ERC2309: transfer caller is not owner nor approved");
        require(to != owner(), "ERC2309: Can't transfer batch to own address");
    ...
    function _safeTransfer(address from, address to, uint256 tokenId, bytes memory _data) internal virtual {
        _transfer(from, to, tokenId);
        require(_checkOnERC721Received(from, to, tokenId, _data), "ERC2309: transfer to non ERC721Receiver implementer");
    }
    ...
    function _isApprovedOrOwner(address spender, uint256 tokenId) internal view virtual returns (bool) {
        ...
        address owner = CAMA.ownerOf(tokenId);
        return (owner == spender || getApproved(tokenId) == spender || CAMA.isApprovedForAll(owner, spender));
    }
     ...
     */
    function _transfer(address from, address to, uint256 tokenId) internal virtual {
        require(CAMA.ownerOf(tokenId) == from, "ERC721: transfer of token that is not own");
        require(to != address(0), "ERC2309: transfer to the zero address");
        // require(tokenId < nextBatch, "ERC2309: Token Not in Supply");//need to discuss
     ...
     function _batchTransfer(address from, address to, uint96 size) internal returns (uint) {
        require(to != address(0), "ERC2309: must not be null");
        require(size > 0 && size <= batchSize, "ERC2309: size must be within limits");

        uint256 start = nextBatch;
        require(_exists(start.add(size)), "ERC2309: Max token limit reached");
    ...
    function _batchTransferFrom(address from, address to, uint[] memory ids) internal virtual {
        uint len = ids.length;
        require(to != address(0), "ERC2309: must not be null");
        require(ids.length > 1, "ERC2309: Min 2 ids");

        for (uint i; i < ids.length; i++) {
            // require(ids[i] < nextBatch, "ERC2309: Token Not in Supply");//need to discuss
            require(_exists(ids[i]), "ERC2309: Invalid TokenId");
    ...
    function _approve(address to, uint256 tokenId) private {
        _tokenApprovals[tokenId] = to;
        emit Approval(CAMA.ownerOf(tokenId), to, tokenId); // internal owner
    }
    ...
    //     owners.init(to, number_of_tokens, 1000);//change the arguments after consulting the client
