    ...
    OwnableUpgradeable
{
    ...
    /// @dev The addresses of the MarketController contract which will be approved
    /// with an allowance of MAX_UINT for all tokens held in the MarketVault
    address internal controller;

    ///////////////////// EVENTS /////////////////////

    /// @notice Emitted when the MarketVault is initialized
    event MarketVaultInitialized(address owner, address controller);

    /// @notice Emitted when a privileged MarketController contract is set on the MarketVault
    event MarketControllerSet(address marketController);

    ///////////////////// MODIFIER FUNCTIONS /////////////////////

    /// @notice Check if the msg.sender is the privileged MarketController contract address
    modifier onlyMarketController() {
        require(
            msg.sender == controller,
            "MarketVault: Sender must be the marketController"
        );
    ...
    function __MarketVault_init(address _marketController)
        external
        initializer
    {
        ...
        require(
            _marketController != address(0x0),
            "MarketVault: _marketController cannot be the 0x0 address"
        );

        controller = _marketController;

        emit MarketControllerSet(_marketController);

        emit MarketVaultInitialized(msg.sender, _marketController);
    }
    ...
    /// @param _newImplementation the address of the new MarketVault implementation
