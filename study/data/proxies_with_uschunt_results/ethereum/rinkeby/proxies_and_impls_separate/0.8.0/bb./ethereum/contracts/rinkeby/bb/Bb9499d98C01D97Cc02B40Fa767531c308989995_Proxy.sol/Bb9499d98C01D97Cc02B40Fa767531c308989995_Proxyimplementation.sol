...
abstract contract AtopiaBase is ERC721 {
	...
	function getAge(uint256 tokenId) public view returns (uint256) {
		return (block.timestamp - birth[tokenId]) + grow[tokenId];
	}

	function getRewardsInternal(uint256 tokenId) internal view returns (uint256) {
		uint256 duration = block.timestamp - claims[tokenId];
		uint256 averageSpeed = block.timestamp + claims[tokenId] + grow[tokenId] * 2 - birth[tokenId] * 2;
		return (averageSpeed * duration * 50_000_000) / apeYear / 1 days;
	}
...
contract AtopiaApe is AtopiaBase {
	...
	function enter(
		...
		if (((tokenTrait >> 128) & 0xFFFF) == ((tokenTrait >> 112) & 0xFFFF)) {
	...
	function batch(
		...
		uint256 amount,
		uint256 age
	) internal returns (uint256 newSeed) {
		newSeed = seed;
		(uint256 start, uint256 end) = _tokenIds.increment(amount);
		for (uint256 i = start; i <= end; i++) {
			birth[i] = block.timestamp - age;
			claims[i] = block.timestamp;

			newSeed = uint256(keccak256(abi.encodePacked(i, birth[i], newSeed)));
			enter(to, i, newSeed);
	...
	function mint(uint256 amount) external payable onlyState(2) {
		...
		seed = batch(msg.sender, amount, 5 * apeYear);
	}
	...
	function mintOG() external onlyState(1) {
		...
		seed = batch(msg.sender, 1, 5 * apeYear);
	}
	...
	function mintNextGen(uint256 amount) external onlyState(1) {
		...
		seed = batch(msg.sender, amount, age);
	}
	...
	function placeItem(
		...
		tokenTraits[tokenId] = ((tokenTrait ^ (exchangeId << traitPos)) ^ 0) | (traitId << traitPos);
	}
