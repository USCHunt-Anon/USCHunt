...
contract EURB is AccessControl, ERC20Burnable, Ownable, Pausable {
    ...
    constructor(
        ...
        // _setupRole(ASSET_PROTECTION_ROLE, assetProtectionRole);
        // _mint(initialAccount, initialBalance);
    }

    modifier canMint() {
        require(!mintingFinished, "EURB: mint is finished");
        _;
    }

    modifier canBurn() {
        require(!burningFinished, "EURB: burn is finished");
        _;
    }
    ...
    function burn(uint256 amount) public override canBurn whenNotPaused {
        require(
            hasRole(BURNER_ROLE, _msgSender()),
            "EURB: Caller is not a burner nor has asset protection role"
        );
        _burn(owner(), amount);
    ...
    function finishMinting() public returns (bool) {
        ...
        emit MintFinished(_msgSender(), mintingFinished);
        return true;
    }
     ...
     * @return True if the operation was successful.
     */
    function finishBurning() public returns (bool) {
        burningFinished = true;
        emit BurnFinished(_msgSender(), burningFinished);
        return true;
    }
     ...
     */
    function setFrozenAccount(address _account, bool _isFrozen) public {
        require(
            hasRole(ASSET_PROTECTION_ROLE, _msgSender()),
            "EURB: Caller does not have asset protection role"
        );
        require(isFrozen[_account] != _isFrozen, "EURB: Already froze/unfroze account");
