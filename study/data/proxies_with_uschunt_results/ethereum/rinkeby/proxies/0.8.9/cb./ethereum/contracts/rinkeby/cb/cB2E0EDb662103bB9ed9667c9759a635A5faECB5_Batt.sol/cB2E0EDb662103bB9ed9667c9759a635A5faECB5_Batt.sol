...
contract ERC721BatchableCollection is ERC721Marketplace {
    ...
    function _init(
        ...
        _name = strings[0];
        _symbol = strings[1];
        if (uints[6] > 0 && bytes(strings[2]).length > 0) {
            _addTokenDirectoryRelease(strings[2], uints[6]);
        }
    }
    ...
    function batchMint(uint256 batchId, uint256 amount) external onlyOwner {
        BatchData memory batchData = _batchIdToData[batchId];
        require(amount <= MAX_TRANSACTION && amount > 0, "Amount invalid");
        require(
