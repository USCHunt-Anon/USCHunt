...
contract MechaPenguins is ERC721Delegated {
    ...
    uint256 public constant MAX_SUPPLY = 17775;
    uint256 public constant MAX_PURCHASE_PER_TX = 20;

    address public proxyRegistryAddress;
    uint256 public reserveTokenId = 8887;
    ...
    function hasBeenClaimed(uint256[] memory nftTokenIds) external view returns (bool[] memory )
    {
            bool[] memory claimed = new bool[](nftTokenIds.length);
            uint256 index;
            for (index = 0; index < nftTokenIds.length; index++) {
                if (seen[nftTokenIds[index]]) {
                    claimed[nftTokenIds[index]] = true;
                } else {
                    claimed[nftTokenIds[index]] = false;
                }
            }
    ...
    function isApprovedForAll(address owner, address operator) internal view returns (bool) {
        ...
        return super._isApprovedForAll(owner, operator);
    }
}
