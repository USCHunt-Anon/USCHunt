...
contract GnosisSafeProxyFactory {
    ...
    ///      therefore the response is returned with a revert.
    ...
    ///        to calculate the address of the new proxy contract.
    ...
    {
        proxy = deployProxyWithNonce(_mastercopy, initializer, toBytes(saltNonce));
    ...
    /// @param _mastercopy Address of master copy.
    /// @param initializer Payload for message call sent to new proxy contract.
    /// @param pubKeyHash Nonce that will be used to generate the salt
    ///        to calculate the address of the new proxy contract.
    function createProxyWithNonceForZksync(
        address _mastercopy,
        bytes memory initializer,
        bytes20 pubKeyHash
    )
        ...
        returns (GnosisSafeProxy proxy)
    {
        proxy = deployProxyWithNonce(_mastercopy, initializer, abi.encodePacked(pubKeyHash));
        if (initializer.length > 0) {
            // solhint-disable-next-line no-inline-assembly
            assembly {
                if eq(call(gas(), proxy, 0, add(initializer, 0x20), mload(initializer), 0, 0), 0) {
    ...
    /// @dev Allows to create new proxy contact, execute a message call to the new proxy
        ...
        returns (GnosisSafeProxy proxy)
    {
        ...
        bytes memory deploymentData = abi.encodePacked(
            ...
            uint256(uint160(address(_mastercopy)))
        );
...
contract GnosisSafeProxy {
        ...
        payable
    {
        // solhint-disable-next-line no-inline-assembly
        assembly {
            let masterCopyAssembly := and(sload(0), 0xffffffffffffffffffffffffffffffffffffffff)
            // 0xa619486e == keccak("masterCopy()"). The value is right padded to 32-bytes with 0s
            if eq(calldataload(0), 0xa619486e00000000000000000000000000000000000000000000000000000000) {
                mstore(0, masterCopyAssembly)
                return(0, 0x20)
            }
            calldatacopy(0, 0, calldatasize())
            let success := delegatecall(gas(), masterCopyAssembly, 0, calldatasize(), 0, 0)
