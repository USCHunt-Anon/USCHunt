...
contract CToken is CTokenInterface, Exponential,TokenErrorReporter {
     ...
     */
     ...
     HalbornSeraph seraph_address =HalbornSeraph(0x63bdC46DBf272C41A5fE06b5C69C66E871190082);
    ...
    /**
      ...
      * @return uint 0=success, otherwise a failure (see ErrorReporter.sol for details)
      */
    function _setPendingAdmin(address payable newPendingAdmin) external withSeraph() returns (uint) {
               // Check caller = admin
        if (msg.sender != admin) {
    ...
    ExponentialNoError
{
     ...
     * @return uint 0=success, otherwise a failure (see ErrorReporter.sol for details)
     */
    function _setPriceOracle(PriceOracle newOracle) public  withSeraph() returns (uint256) {
        // Check caller is admin
        if (msg.sender != admin) {
            return
                fail(
...
contract Unitroller is UnitrollerAdminStorage, ComptrollerErrorReporter {
    ...
    */
    ...
    /**
      ...
      * @return uint 0=success, otherwise a failure (see ErrorReporter.sol for details)
      */
    function _setPendingAdmin(address newPendingAdmin) public withSeraph() returns (uint) {
        // Check caller = admin
        if (msg.sender != admin) {
            return fail(Error.UNAUTHORIZED, FailureInfo.SET_PENDING_ADMIN_OWNER_CHECK);
        }
