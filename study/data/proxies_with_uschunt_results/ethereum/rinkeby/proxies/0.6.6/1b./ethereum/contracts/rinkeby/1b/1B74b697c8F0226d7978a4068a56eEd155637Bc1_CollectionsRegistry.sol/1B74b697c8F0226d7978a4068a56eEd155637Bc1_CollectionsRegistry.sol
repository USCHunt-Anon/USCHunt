...
contract CollectionsRegistry is Ownable {
    ...
    mapping(address => address[]) public collectionsRegistry;

    event CollectionCreated(address indexed creator, address indexed collectionAddress);
    event CollectionData(address indexed _logic, address indexed _admin, bytes _data);

    constructor(address _tokenImplementation) public {
        require(_tokenImplementation != address(0), "Wrong token implementation");
        tokenImplementation = _tokenImplementation;
    }

    function setTokenImplementation(address _newTokenImplementation) external onlyOwner {
        require(_newTokenImplementation != address(0), "Wrong token implementation");
    ...
    */
        ...
        returns (address proxy)
    {
        ...
        collectionsRegistry[msg.sender].push(proxy);

        emit CollectionCreated(msg.sender, address(proxy));
        emit CollectionData(tokenImplementation, msg.sender, bytesData);
    }
