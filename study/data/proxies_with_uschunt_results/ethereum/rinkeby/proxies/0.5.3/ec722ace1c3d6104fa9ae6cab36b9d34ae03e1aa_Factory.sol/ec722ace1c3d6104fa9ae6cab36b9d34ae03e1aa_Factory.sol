...
contract Factory {
  ...
  function deploy(address _engine) external returns (address token) {
    bytes20 targetBytes = bytes20(_engine);
    assembly {
    ...
    emit ProxyCreated(address(token));
    
    bytes memory _data;
    
    if(_data.length > 0) {
      (bool success,) = token.call(_data);
      require(success);
    }    
  }
}