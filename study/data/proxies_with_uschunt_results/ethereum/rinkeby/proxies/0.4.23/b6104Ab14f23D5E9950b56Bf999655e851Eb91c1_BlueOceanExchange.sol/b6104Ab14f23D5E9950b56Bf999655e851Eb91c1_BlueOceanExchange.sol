...
/*
  ...
  - A small fee can be charged in BOT for order settlement in an amount configurable by the frontend hosting the orderbook.
  ...
  Future protocol versions may improve upon this structure in capability or usability according to protocol user feedback demand, with upgrades enacted by the BlueOcean DAO.
...
contract ProxyRegistry is Ownable {
    ...
    /* Delay period for adding an authenticated contract.
       This mitigates a particular class of potential attack on the BlueOcean DAO (which owns this registry) - if at any point the value of assets held by proxy contracts exceeded the value of half the BOT supply (votes in the DAO),
       a malicious but rational attacker could buy half the BlueOcean and grant themselves access to all the proxy contracts. A delay period renders this attack nonthreatening - given two weeks, if that happened, users would have
