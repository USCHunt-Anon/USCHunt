...
contract SloppyBees {
    ...
    address public constant implementation = 0x8A6F4251d79AD8322Deb425EF7BC5Da80624FC70;
...
# @version 0.3.0
"""
@title SloppyBees NFT
@license MIT
@notice Do not use this contract! It is implementation only. Use proxy to interact.
"""

from vyper.interfaces //importERC721

# implements: ERC721
# Impossible to implement ERC165 in vyper
# Implemented in proxy instead
...
baseURI: public(String[60])  # baseURI 60 + tokenURI 4 = 64
...
MAX_MINT: constant(uint256) = 10  # Max amount of tokens minted per tx
...
def initialize():
    ...
    self.admin = 0x070d874CaE6FE6dE0f3e72AA1f40e7Be765848E0

    self.presale_price = 59 * 10 ** 15  # 0.059 ETH
    self.presale_limit = 3

    self.mint_price = 90 * 10 ** 15  # 0.09 ETH
    self.mint_limit = 10

    self.official_site = "https://sloppybees.com"
...
@internal
@pure
def _to_string(_num: uint256) -> String[4]:
    """
    @dev Vyper does not support 'to_string' fot integers.
         Assumed to receive _num < 10^4
    """
    num: uint256 = _num
    result: String[4] = "////"  # Random symbols, so slice does not fail
    for i in range(4):
        digit: String[1] = slice("0123456789", num % 10, 1)
        result = slice(concat(digit, result), 0, 4)
        num /= 10
        if num == 0:
...
@external
@view
def tokenURI(_token_id: uint256) -> String[64]:
    """
    ...
    if len(self.baseURI) == 0:  # Preview URI
        if _token_id % 2 == 0:
            return "ipfs://QmUCHwFWA5SnwpaVvWkVJta8CiUtkoBKkP9cfzUzGfMwPj"
        else:
            return "ipfs://Qme6VdjYE2BLMnUKfPaHxUdu1Xj9SDMm6wzdC2JDjpaL78"
    id: String[4] = self._to_string(_token_id)
...
@view
@external
def presaled_by(_by: address) -> uint256:
    return self.minted[_by] % MAX_BEES


@view
@external
def minted_by(_by: address) -> uint256:
    return self.minted[_by] / MAX_BEES
...
def mint(_number_of_bees: uint256 = 1, _to: address = msg.sender) -> bool:
    ...
    """
    assert self.mint_is_active, "Mint is not active"
    assert _to != ZERO_ADDRESS, "ERC721: mint to the zero address"

    assert self.minted[msg.sender] / MAX_BEES + _number_of_bees <= self.mint_limit, "Too many tokens for you"
    assert self.totalSupply + _number_of_bees <= MAX_BEES, "Purchase would exceed max supply of Bees"

    assert msg.value == _number_of_bees * self.mint_price, "Incorrect ETH amount"
...
def presale(_number_of_bees: uint256, _signature: Bytes[65], _to: address = msg.sender) -> bool:
    ...
    @return A boolean that indicates if the operation was successful.
    """
    assert self.presale_is_active, "Presale is not active"
    assert _to != ZERO_ADDRESS, "ERC721: mint to the zero address"

    assert self._signature_is_valid(msg.sender, _signature), "You are not whitelisted for presale"

    assert self.minted[msg.sender] % MAX_BEES + _number_of_bees <= self.presale_limit, "Too many tokens for you"
    assert self.totalSupply + _number_of_bees <= MAX_BEES, "Purchase would exceed max supply of Bees"
    assert msg.value == _number_of_bees * self.presale_price, "Incorrect ETH amount"
...
@external
def set_base_uri(_base_uri: String[60]):
    """
...
def set_pollen(_pollen: address):
    ...
    """
    assert msg.sender == self.admin, "admin only"
    assert self.pollen == ZERO_ADDRESS, "pollen already set"
    self.pollen = _pollen
...
def start_presale():
    ...
    """
    assert msg.sender == self.admin, "admin only"
    assert not self.mint_is_active, "mint is active"
    assert self.presale_signer != ZERO_ADDRESS, "presale signer is not set"
    assert self.pollen != ZERO_ADDRESS, "pollen is not set"
...
def start_mint():
    ...
    """
    assert msg.sender == self.admin, "admin only"
    assert not self.presale_is_active, "presale is active"
    assert self.pollen != ZERO_ADDRESS, "pollen is not set"
...
def set_new_presale(_price: uint256, _limit: uint256):
    ...
    """
    assert msg.sender == self.admin, "admin only"
    assert _limit <= MAX_BEES, "too big limit"
...
def apply_new_admin():
    ...
    """
    assert msg.sender == self.future_admin, "new admin only"
