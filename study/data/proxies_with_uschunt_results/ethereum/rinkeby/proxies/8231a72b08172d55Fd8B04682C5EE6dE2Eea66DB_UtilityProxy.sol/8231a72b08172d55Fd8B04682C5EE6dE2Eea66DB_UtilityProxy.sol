...
contract UtilityProxy {
    bytes32 private constant _IMPL_POS =
        keccak256("dev.tur461.delegate.address.position");

    bytes32 private constant _OWNR_POS =
        keccak256("dev.tur461.owner.address.position");
    ...
    modifier onlyOwner() {
        ...
        assembly {
            ...
            if or(gt(own, sender), lt(own, sender)) {
                revert(0, 0)
            }
        }
        _;
