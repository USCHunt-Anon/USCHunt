...
contract DeathWish is ReentrancyGuard {
    ...
    function getPaginatedBenefactorsForSwitch(uint256 id, uint256 offset, uint256 len) external view returns (address[] memory) {
        ...
        for(uint256 i = offset; i < min(offset + len, benefactors[id].length); i++) {
            result[i - offset] = benefactors[id][i];
        }
        return result;
    }
    ...
    function getPaginatedOwnedSwitches(address _user, uint256 offset, uint256 len) external view returns (uint256[] memory) {
        ...
        uint256 arraylen =  min(offset + len, userSwitches[_user].length);
        uint256[] memory result = new uint256[](arraylen);
    
        for(uint256 i = offset; i < arraylen; i++) {
            result[i - offset] = userSwitches[_user][i];
        }
        return result;
    }
    ...
    function getPaginatedBenefactorSwitches(address _user, uint256 offset, uint256 len) external view returns (uint256[] memory) {
        ...
        uint256 arraylen = min(offset + len, userBenefactor[_user].length);
        uint256[] memory result = new uint256[](arraylen);
    
        for(uint256 i = offset; i < arraylen; i++) {
            result[i - offset] = userBenefactor[_user][i];
        }
        return result;
    }
