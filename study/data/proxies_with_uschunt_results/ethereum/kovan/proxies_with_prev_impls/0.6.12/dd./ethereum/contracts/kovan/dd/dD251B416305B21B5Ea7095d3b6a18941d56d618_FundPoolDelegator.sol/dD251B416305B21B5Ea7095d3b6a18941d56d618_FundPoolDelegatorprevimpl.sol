...
contract FundPoolDelegate is FundPoolStorgeV1, FundPoolEvents {
    ...
    function withdraw(uint256 _shareAmountIn) external {
        ...
        cumulativeProfitProfit = cumulativeProfitProfit.add(principal).sub(
            tokenAmountOut
        );
    ...
    //         managementFeePerShareLast.mul(share).div(1e18).add(fee);
    ...
    {
        //赎回费
        uint256 feeRate = getWithdrawFeeRate();
    ...
    function getWithdrawFeeRate() private view returns (uint256 _feeRate) {
    ...
    function setProfitRatePerBlock(uint256 _profitRatePerBlock) external {
        require(msg.sender == admin, "unauthorized");
        require(
            _profitRatePerBlock.mul(BLOCK_PER_YEAR) >= minProfitRate,
            "Invalid input"
        );
        require(
            _profitRatePerBlock.mul(BLOCK_PER_YEAR) <= maxProfitRate,
            "Invalid input"
        );
        profitRatePerBlock = _profitRatePerBlock;
    }
        ...
        )
    {
        ...
        uint256 totalTokens =
            totalTokenSupply.add(cumulativeProfitProfit.add(profit));
    ...
    function getTotalTokens() internal returns (uint256) {
        ...
        if (profitRatePerBlock != 0) {
            ...
            cumulativeProfitProfit = cumulativeProfitProfit.add(profit);
            uint256 totalTokens = totalTokenSupply.add(cumulativeProfitProfit);
            return totalTokens;
        } else {
    ...
    //             return 0;
    //         }
    //         uint256 blockOffset = now.sub(blockHeightLast);
    //         uint256 cumulativeProfitRate = blockOffset.mul(profitRatePerBlock);
