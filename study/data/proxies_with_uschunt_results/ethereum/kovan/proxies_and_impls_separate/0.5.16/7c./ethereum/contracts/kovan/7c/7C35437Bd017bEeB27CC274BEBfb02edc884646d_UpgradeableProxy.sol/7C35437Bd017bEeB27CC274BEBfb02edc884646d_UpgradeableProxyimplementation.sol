...
contract Vault is ERC20, ERC20Detailed, ControllableInit, VaultStorage, IVault {
    ...
    function doHardWork() public onlyControllerOrGovernance {
        ...
        if (!allowSharePriceDecrease) {
            require(sharePriceCheckpoint <= sharePriceAfterHardWork, "V19");
        }
        sharePriceCheckpoint = sharePriceAfterHardWork;
    ...
    function withdraw(uint256 numberOfShares) external {
        ...
        uint256 underlyingAmountToWithdraw = numberOfShares.mul(Math.min(storedSharePrice, calculatedSharePrice))
        .div(underlyingUnit);
    ...
    function _deposit(uint256 amount, address sender, address beneficiary) internal {
        ...
        uint256 toMint = amount.mul(underlyingUnit).div(
            Math.max(sharePriceCheckpoint, getPricePerFullShare())
        );
