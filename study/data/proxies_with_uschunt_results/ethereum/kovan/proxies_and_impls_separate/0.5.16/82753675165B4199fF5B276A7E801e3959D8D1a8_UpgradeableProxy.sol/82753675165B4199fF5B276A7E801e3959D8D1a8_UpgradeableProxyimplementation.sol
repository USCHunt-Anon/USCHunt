...
contract VaultStorage {
    ...
    uint256 public sharePriceCheckpoint;
    ...
    function initialize(
        ...
        sharePriceCheckpoint = _underlyingUnit;
...
contract Vault is ERC20, ERC20Detailed, ControllableInit, VaultStorage, IVault {
    ...
    function underlyingBalanceWithInvestmentForHolder(address holder) view external returns (uint256) {
        ...
        return balanceOf(holder).mul(Math.min(sharePriceCheckpoint, getPricePerFullShare())).div(underlyingUnit);
    }
    ...
    event WD(uint256 underlyingBalanceWithInvestment, uint256 numberOfShares, uint256 totalSupply, uint256 underlyingBalanceInVault, uint256 underlyingAmountToWithdraw);
    ...
    function withdraw(uint256 numberOfShares) external {
        ...
        uint256 storedSharePrice = sharePriceCheckpoint;
        uint256 calculatedSharePrice = getPricePerFullShare();

        uint256 underlyingAmountToWithdraw = numberOfShares.mul(calculatedSharePrice)
        .div(underlyingUnit);
        ...
        if (underlyingAmountToWithdraw > underlyingBalanceInVault()) {
            ...
            emit WD(underlyingBalanceWithInvestment(), numberOfShares, totalSupply, underlyingBalanceInVault(), underlyingAmountToWithdraw);
