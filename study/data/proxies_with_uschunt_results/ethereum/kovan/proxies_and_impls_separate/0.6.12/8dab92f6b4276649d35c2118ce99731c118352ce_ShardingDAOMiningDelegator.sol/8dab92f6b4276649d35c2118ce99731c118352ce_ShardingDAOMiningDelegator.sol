...
contract ShardingDAOMining is IInvitation, ActivityBase {
    ...
    function getLastDividendHeight(uint256 _pid)
        public
        view
        virtual
        returns(
    ...
    {
        PoolInfo storage pool = poolInfo[_pid];
        return pool.lastDividendHeight;
    }
    ...
    function updateTokenOracle(TokenPairInfo storage _pairInfo) internal returns (FixedPoint.uq112x112 memory _price) {
        ...
        uint32 timeElapsed;
        if(_pairInfo.blockTimestampLast == 0){
            ...
            _pairInfo.priceCumulativeLast = uint(_price._x) * timeElapsed;
        ...
        timeElapsed = blockTimestamp - _pairInfo.blockTimestampLast; // overflow is desired
...
contract ShardingDAOMiningDelegator is DelegatorInterface, ShardingDAOMining {
        ...
        )
    {
        bytes memory data =
            delegateToViewImplementation(
                abi.encodeWithSignature("getLastDividendHeight(uint256)", _pid)
            );
        return abi.decode(data, (uint256));
    }
