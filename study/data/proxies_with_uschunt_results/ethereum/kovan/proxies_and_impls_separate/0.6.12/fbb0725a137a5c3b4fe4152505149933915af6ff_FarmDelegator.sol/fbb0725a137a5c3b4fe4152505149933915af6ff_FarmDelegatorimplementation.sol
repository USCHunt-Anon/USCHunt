...
contract FarmDelegate is FarmStorageV1 {
    ...
    function getEndBlock(uint256 restReward) internal returns(uint256 blockHeightEnd){
        uint256 blockHeightBegin = block.number;
        while(restReward != 0){
    ...
    function _getEndBlock(uint256 blockHeightBegin, uint256 rewardPending)internal returns(uint256 blockHeightEnd,uint256 rewardRest){
        uint256 halvingPeriod = halvingPeriod0.add(halvingPeriod1);
        uint256 _phase = phase(blockHeightBegin);
        uint256 switchBlock = _phase.mul(halvingPeriod).add(startBlockOfGan).add(halvingPeriod0);
        uint256 reward;
        if(switchBlock > blockHeightBegin){ 
           reward = switchBlock.sub(blockHeightBegin).mul(ganPerBlockConcentratedMining.div(2 ** _phase));      
           if(reward >= rewardPending)
               return(switchBlock,reward.sub(rewardPending));
           else
            ...
            reward = endRewardBlock.sub(blockHeightBegin).mul(ganPerBlockContinuousMining.div(2 ** _phase));
            if(reward >= rewardPending)
                return(endRewardBlock,reward.sub(rewardPending));
            else
