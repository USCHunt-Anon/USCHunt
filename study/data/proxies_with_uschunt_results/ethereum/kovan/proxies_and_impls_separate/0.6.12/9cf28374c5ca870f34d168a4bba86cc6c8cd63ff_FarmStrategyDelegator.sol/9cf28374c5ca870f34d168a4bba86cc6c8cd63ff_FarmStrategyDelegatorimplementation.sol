...
contract FarmStrategy is FarmStrategyStorage{
    ...
    function withdraw(uint256 amount) external override returns (address[] memory tokens, uint256[] memory amounts){
        ...
        if(token0 == profit){
            uint256 token0Balance = IERC20(token0).balanceOf(address(this)).sub(profitAmount.sub(reinvestmentAmount));
            amounts[0] = token0Balance < amounts[0] ? token0Balance : amounts[0];
        }else if (token1 == profit){
            uint256 token1Balance = IERC20(token1).balanceOf(address(this)).sub(profitAmount.sub(reinvestmentAmount));
            amounts[1] = token1Balance < amounts[1] ? token1Balance : amounts[1];
        }
    ...
    function withdraw(address[] memory tokens, uint256 amount) external override returns (uint256, address[] memory, uint256[] memory){
        ...
        if(balance >= tokenAmount){
            ...
            (uint256 _caculatedLpAmount,uint256 _lpTokenAmount) = caculateLPTokenAmount(token, tokenAmount.sub(balance));
            if(_lpTokenAmount == 0){
                withdrawTokens[0] = token;
                amounts[0] = balance;
                tokenAmount = balance;
