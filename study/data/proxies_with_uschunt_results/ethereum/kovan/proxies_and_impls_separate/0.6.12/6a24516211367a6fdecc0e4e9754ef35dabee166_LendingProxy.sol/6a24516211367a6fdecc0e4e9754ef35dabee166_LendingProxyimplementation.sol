...
contract LendingData is Ownable {
    ...
    function _getAccountTotalValue(address _account, bool _testFlag)
        internal
        returns (
            uint256,
            uint256,
        ...
        )
    {
        ...
        _var.assetPrice = _testFlag ? _var.priceOracle.getUnderlyingPrice(priceToken) : getAssetUSDPrice(priceToken);
    ...
    function getAccountTotalValueForTest(address _account)
        external
        returns (
            uint256,
            uint256,
    ...
    function getAccountAssetStatus(IiToken _asset, address _account, uint256 _type) internal returns (bool, uint256) {
        ...
        if (_type == 0) {
            ...
            _isiToken = _balance == 0 ? false : _isiToken;
        }
        ...
        )
    {
        ...
        if (_var.maxSeizediToken < _var.borrowerCollateralBalance)
            return (
                _var.maxRepay,
                _var.maxRepay > _var.liquidatorBalance
        ...
        return (
            _var.maxRepay,
            _var.maxRepayByCollateral > _var.liquidatorBalance
                ? _var.liquidatorBalance
    ...
    function getAccountRewardAmount(address _account) external returns (uint256) {
        ...
        IiToken[] memory _iTokens = controller.getAlliTokens();
        for (uint256 i = 0; i < _iTokens.length; i++) {
            _rewardDistributor.updateDistributionState(_iTokens[i], false);
            _rewardDistributor.updateDistributionState(_iTokens[i], true);
            _rewardDistributor.updateReward(_iTokens[i], _account, false);
            _rewardDistributor.updateReward(_iTokens[i], _account, true);
        }
