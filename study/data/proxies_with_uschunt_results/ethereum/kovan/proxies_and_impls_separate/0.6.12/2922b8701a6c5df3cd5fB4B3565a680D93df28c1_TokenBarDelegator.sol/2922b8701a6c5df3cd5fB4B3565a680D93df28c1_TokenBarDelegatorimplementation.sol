...
contract TokenBarDelegate is
    ITokenBarStorge,
    ERC20("SHDTokenBar", "xSHDToken")
{
    using SafeMath for uint256;
    ...
    function swapAllForSHD() public {
        ...
        for (uint256 i = 0; i < wantTokenWhiteList.length; i++) {
            ...
            if (wantToken != weth) {
                ...
                address router = routerMap[wantToken][weth];
                if (balance > 0) {
                    address[] memory path;
                    path[0] = wantToken;
                    path[1] = weth;
                    IERC20(wantToken).approve(router, balance);
        ...
        address router = routerMap[weth][SHDToken];
        if (balanceOfWeth > 0) {
            address[] memory path;
            path[0] = weth;
            path[1] = SHDToken;
            IERC20(weth).approve(router, balanceOfWeth);
            IUniswapV2Router02(router).swapExactTokensForTokens(
                balanceOfWeth,
                0,
                path,
        ...
        returns (uint256 SHDAmountOut)
    {
            ...
            if (wantToken != weth) {
                ...
                if (balance > 0) {
                    ...
                    uint256[] memory amounts =
                        IUniswapV2Router02(router).getAmountsOut(balance, path);
                    balanceOfWeth = balanceOfWeth.add(amounts[1]);
                }
            }
        }
        ...
        if (balanceOfWeth > 0) {
            ...
            path[1] = SHDToken;
            uint256[] memory amounts =
                IUniswapV2Router02(router).getAmountsOut(balanceOfWeth, path);
            SHDBalance = SHDBalance.add(amounts[1]);
        }
        SHDAmountOut = xSHDAmountIn.mul(SHDBalance).div(totalShares);
    }
}