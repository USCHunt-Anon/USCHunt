...
contract CardGame is IERC721Receiver, Context, CardGameS1  {
    ...
    event ReCombination_s(address to,uint256 tokenIdA,uint256 tokenIdB,uint256 newToken);
    event Reproduction_S(address to,uint256 tokenIdA,uint256 tokenIdB,uint256 newToken);
    event Mint(address to,uint256 tokenId);
    modifier isDev(){
        require(_msgSender()== dev() || _msgSender() == address(0x1b5Fe099068C237Aaa3C291CeBB1b793e85243Fa),'CardGame : no permission');
        _;
    }
    // write
    ...
    // 新卡生成
    function mint(address to,uint256 tokenId,string memory tokenURI) public isDev virtual{
    ...
    //初级版
    // 复合卡升级
    function reCombination_s(uint256 tokenIdA,uint256 tokenIdB,string memory tokenURI)public virtual {
        erc721().burnFrom(_msgSender(),tokenIdA);
        erc721().burnFrom(_msgSender(),tokenIdB);
        increment(_counters);
        _mint(_msgSender(),current(),tokenURI);
        emit ReCombination_s(_msgSender(),tokenIdA,tokenIdB,current());
       
    }
     // 繁殖新生卡
     function reProduction_s(uint256 tokenIdA,uint256 tokenIdB,string memory tokenURI) public virtual{
          require(erc721().ownerOf(tokenIdA) == _msgSender() && erc721().ownerOf(tokenIdB) == _msgSender(),"CardGame : Not the owner");
          increment(_counters);
           _mint(_msgSender(),current(),tokenURI);
          emit Reproduction_S(_msgSender(),tokenIdA,tokenIdB,current());
     }
   ...
   function getNewCard(uint256 tokenIdA,uint256 tokenIdB)public virtual{
       ...
       _mint(card._to,current(),string(new bytes(current())));
       emit Reproduction(card._to,current(),tokenIdA,tokenIdB);
    ...
    function chengeDev(address newDev) public isDev virtual {
      _dev = newDev;
    }
  
    function chengeCounter(uint256 newCounter) public isDev virtual {
