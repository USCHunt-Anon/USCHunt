...
contract MintableToken is ERC20Upgradeable, PausableUpgradeable {
    ...
    modifier onlyOwner() {
        require(msg.sender == owner, "GOL: ONLY OWNER");
        _;
    }

    modifier onlyMinter() {
        require(msg.sender == minter, "GOL: ONLY MINTER");
        _;
    }
    ...
    function setCapacity(uint256 _cap) external onlyOwner {
        require(_cap >= totalSupply(), "GOL: INVALID CAPACITY");
        cap = _cap;
    }

    function mint(address user, uint256 amount) external whenNotPaused onlyMinter {
        require(totalSupply() + amount <= cap, "GOL: FULL CAPACITY");
        _mint(user, amount);
    }
    ...
    function burn(address account, uint256 amount) external whenNotPaused {
        require(
            msg.sender == account,
            "GOL: SENDER ADDRESS IS NOT MATCH ACCOUNT ADDRESS"
        );
        _burn(account, amount);
    }

    function changeMinter(address newMinter) external whenNotPaused onlyMinter {
        require(newMinter != address(0), "GOL: ZERO ADDRESS");
        emit MinterChanged(minter, newMinter);
        minter = newMinter;
    }

    function changeOwner(address newOwner) external onlyOwner {
        require(newOwner != address(0), "GOL: ZERO ADDRESS");
