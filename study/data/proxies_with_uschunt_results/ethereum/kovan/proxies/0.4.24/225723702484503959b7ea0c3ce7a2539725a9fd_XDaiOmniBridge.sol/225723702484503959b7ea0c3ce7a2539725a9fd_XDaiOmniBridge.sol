...
contract XDaiOmniBridge is Ownable {
    /// @dev xDAI TokenBridge smart contract.
    address public xDaiOmniBridgeAddress;

    constructor(address _bridgeAddress) public {
        xDaiOmniBridgeAddress = _bridgeAddress;
    }

    function setBridgeAddress(address _newBridgeAddress) public onlyOwner validAddress(_newBridgeAddress) {
        xDaiOmniBridgeAddress = _newBridgeAddress;
    }

    function approveTransferERC20ToXDAI(address _tokenAddress, uint256 amount)
        public
        validAddress(_tokenAddress)
        validTokenAmount(amount)
    {
        ERC20 contractToken = ERC20(_tokenAddress);
        contractToken.approve(xDaiOmniBridgeAddress, amount);
    }

    function transferERC20ToXDAI(address _tokenAddress, uint256 amount) public validAddress(_tokenAddress) validTokenAmount(amount) {
        ForeignMultiAMBErc20ToErc677(xDaiOmniBridgeAddress).relayTokens(ERC677(_tokenAddress), amount);
    }

    function claimTokens(address _tokenAddress) public {
        ForeignMultiAMBErc20ToErc677(xDaiOmniBridgeAddress).claimTokens(_tokenAddress, msg.sender);
    }
