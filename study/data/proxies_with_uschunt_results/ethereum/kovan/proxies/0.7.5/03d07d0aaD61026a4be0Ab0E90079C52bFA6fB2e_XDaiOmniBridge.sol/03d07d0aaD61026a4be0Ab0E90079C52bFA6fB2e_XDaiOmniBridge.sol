...
contract XDaiOmniBridge is Ownable {
    ...
    function setBridgeAddress(address _newBridgeAddress)
        public
        onlyOwner
        validAddress(_newBridgeAddress)
    {
    ...
    {
        IERC20 contractToken = IERC20(_tokenAddress);
        contractToken.approve(xDaiOmniBridgeAddress, amount);
    }

    function transferERC20ToXDAI(address _tokenAddress, uint256 amount)
        public
        validAddress(_tokenAddress)
        validTokenAmount(amount)
    {
        IERC677 _token = IERC677(_tokenAddress);
        // ForeignOmnibridge(xDaiOmniBridgeAddress).relayTokens(token, amount);
        xDaiOmniBridgeAddress.delegatecall(
            abi.encodeWithSignature(
                "relayTokens(IERC677,uint256)",
                _token,
                amount
            )
        );
    ...
    function claimTokens(address _tokenAddress) public {
        ForeignOmnibridge(xDaiOmniBridgeAddress).claimTokens(
            _tokenAddress,
            msg.sender
        );
    }
