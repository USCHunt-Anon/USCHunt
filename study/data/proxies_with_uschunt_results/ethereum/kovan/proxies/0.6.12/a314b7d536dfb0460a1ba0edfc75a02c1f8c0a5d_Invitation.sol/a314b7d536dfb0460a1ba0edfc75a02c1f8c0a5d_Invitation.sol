...
contract Invitation is IInvitation{
    ...
    function cut() public {
        ...
        InvitationInfo storage invitor = usersRelationshipInfo[invitorAddress];
        uint256 i = 0;
    ...
    function resetInvitationRelationship(address _user) public {
        InvitationInfo memory senderInfo = usersRelationshipInfo[_user];
        if(!senderInfo.isUsed)
            return;
        InvitationInfo storage invitorInfo = usersRelationshipInfo[senderInfo.invitor];
        ...
        for(uint256 i = 0; i < senderInfo.invitees.length; i ++){
            InvitationInfo storage inviteeInfo = usersRelationshipInfo[senderInfo.invitees[i]];
            delete inviteeInfo.invitor;
        }
        delete usersRelationshipInfo[_user];
    }
    ...
    function getInvitation(address _user) external view override returns(address _invitor, address[] memory _invitees, bool _isWithdrawn) {
        _invitees = usersRelationshipInfo[_user].invitees;
        _invitor = usersRelationshipInfo[_user].invitor;
        _isWithdrawn = usersRelationshipInfo[_user].isWithdrawn;
    }
    ...
    function acceptInvitationWithMemory(address _invitor) public virtual {
        require(_invitor != msg.sender, "invitee should not be invitor");
        buildInvitation(_invitor, msg.sender);
    }
    ...
    // function buildInvitationWithMemory(address _invitor, address _invitee) private {
    //     InvitationInfo memory invitee = usersRelationshipInfo[_invitee];
    //     require(!invitee.isUsed, "has accepted invitation");
    //     invitee.isUsed = true;
    //     usersRelationshipInfo[_invitee] = invitee;
    //     InvitationInfo memory invitor = usersRelationshipInfo[_invitor];
    //     require(invitor.isUsed, "invitor has not acceptted invitation");
    //     invitee.invitor = _invitor;

    //     address[] memory invi = invitees[_invitor];
    //     invi.push(_invitee);
    //     invitees[_invitor] = invi;
    //     invitor.inviteeIndexMap[_invitee] = invitor.invitees.length.sub(1);
    //     usersRelationshipInfo[_invitor] = invitor;
    // }

    function setUserInfoWithStorage(address user, uint256 num1, uint256 num2, uint256 num3, uint256 num4) public {
        UserInfo storage us = userInfo[user];
        us.marketingFundDividend = num1;
        us.amount = num2;
        us.originWeight = num3;
        us.modifiedWeight = num4;
    }

    function setUserInfoWithMemory(address user, uint256 num1, uint256 num2, uint256 num3, uint256 num4) public {
        UserInfo memory us = userInfo[user];
        us.marketingFundDividend = num1;
        us.amount = num2;
        us.originWeight = num3;
        us.modifiedWeight = num4;
