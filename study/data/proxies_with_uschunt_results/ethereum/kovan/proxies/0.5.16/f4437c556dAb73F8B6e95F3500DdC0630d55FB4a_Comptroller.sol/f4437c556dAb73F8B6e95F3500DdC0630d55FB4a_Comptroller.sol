...
contract ComptrollerStorage is UnitrollerAdminStorage {
    ...
    uint internal constant dflHalvePeriod = 21600000;

    // Current speed (per block)
    uint public dflCurrentSpeed;

    // The speed (per block) when mining starts
    uint internal constant dflInitialSpeed = 5.787037037037037e18;

    // The speed (per block) below what the mining process ends
    uint internal constant dflMinSpeed = 3.0e10;
...
contract Comptroller is ComptrollerStorage, ComptrollerInterface, ComptrollerErrorReporter, ExponentialNoError, Distributor {
    ...
    function accrueDFL() internal {
        ...
        if (currentBlock < getDflMiningStartBlock()) {
            return;
        }

        if (dflAccrualBlock == 0) {
            dflCurrentSpeed = dflInitialSpeed; // initial speed
            dflAccrualBlock = getDflMiningStartBlock();
            dflNextHalveBlock = getDflMiningStartBlock() + dflHalvePeriod;
        }

        uint startBlock;
        uint endBlock = dflAccrualBlock;
        while (endBlock < currentBlock) {
            if (dflCurrentSpeed < dflMinSpeed) {
                break;
            }
    ...
    function accrueDflFresh(uint startBlock, uint endBlock) internal {
        ...
        if (deltaBlocks > 0) {
            ...
            DFL(getDflAddress()).mint(address(this), deltaDFLs);
    ...
    /**
     * @notice Return the address of the DFL token
     * @return The address of DFL
     */
    function getDflAddress() public view returns (address) {
        return 0x26f77b6D11F49e239214Df7228F8081f188548d2;
    }

    /**
     * Return the block number at when the DFL mining starts
     */
    function getDflMiningStartBlock() public view returns (uint) {
        return 10000000;  // TODO: fix this before deploy
    }
}