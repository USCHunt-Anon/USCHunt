...
interface IBridgeState {
  ...
  function accountOutboundings(address account, uint periodId, uint index) external view returns(uint);
  ...
  external view
  returns(uint, uint, uint[] memory, bool);

  function outboundIdsToProcess(uint index)
  external view
  returns(uint);
  ...
  function accountOutboundingsInPeriod(address _account, uint _period)
  external view
  returns(uint[] memory);

  function applicableInboundIds(address account) external view returns(uint[] memory);

  function outboundRequestIdsInPeriod(address account, uint periodId)
  external view
  returns(uint[] memory);
  ...
  function appendOutboundingRequest(address account, uint amount, uint destChainIds) external;
  ...
  function appendInboundingRequest(address account, uint amount, uint srcChainId, uint srcOutboundingId) external;
