...
contract CTokenStorage {
     ...
     */
    uint public constant protocolSeizeShareMantissa = 0.1e18; // 10%
...
contract CToken is CTokenInterface, Exponential, TokenErrorReporter {
    ...
    function _setPendingAdmin(address payable newPendingAdmin) external returns (uint) {
        ...
        require(newPendingAdmin != address(0), 'pendingAdmin can not be zero address');
...
contract ComptrollerV8Storage is ComptrollerV7Storage {
    /// @notice The address of COMP
    address public compAddress;

    /// @notice Whether or not COMP can claim
    bool public compClaimPaused = true;

    /// @notice Multiplier used to calculate the received COMP amount
    uint public compGainFactorMantissa;
}
...
contract Unitroller is UnitrollerAdminStorage, ComptrollerErrorReporter {
    ...
    function _setPendingImplementation(address newPendingImplementation) public returns (uint) {
        ...
        require(newPendingImplementation != address(0), 'pendingComptrollerImplementation can not be zero address');
...
contract Comptroller is ComptrollerV8Storage, ComptrollerInterface, ComptrollerErrorReporter, ExponentialNoError {
    ...
    /// @notice Emitted when a gain factor is changed by admin
    event NewCompGainFactor(uint oldCompGainFactorMantissa, uint newCompGainFactorMantissa);
    ...
    function mintAllowed(address cToken, address minter, uint mintAmount) external returns (uint) {
        ...
        Error err = addToMarketInternal(CToken(cToken), minter);
    ...
    function _setBorrowCapGuardian(address newBorrowCapGuardian) external {
        ...
        require(newBorrowCapGuardian != address(0), 'borrowCapGuardian can not be zero address');
    ...
    function _setPauseGuardian(address newPauseGuardian) public returns (uint) {
        ...
        require(newPauseGuardian != address(0), 'pauseGuardian can not be zero address');
    ...
    function grantCompInternal(address user, uint amount) internal returns (uint) {
        if(getCompAddress() == address(0)){
            return amount;
        }
        CompInterface comp = CompInterface(getCompAddress());
        uint compRemaining = comp.balanceOf(address(this));
        if (amount > 0 && amount <= compRemaining && !compClaimPaused && compGainFactorMantissa > 0) {
            Exp memory compGainFactorExp = Exp({mantissa: compGainFactorMantissa});
            comp.transfer(user, mul_ScalarTruncate(compGainFactorExp, amount));
            return 0;
        }
        return amount;
    }
    ...
    function _setCompAddress(address newCompAddress) public returns (uint) {
        require(msg.sender == admin, "only admin can set comp address");

        require(newCompAddress != address(0), 'compAddress can not be zero address');
        address oldCompAddress = compAddress;
        compAddress = newCompAddress;

        emit NewCompAddress(oldCompAddress, compAddress);

        return uint(Error.NO_ERROR);
    }

    function _setCompClaimPaused(bool state) public returns (uint) {
        require(msg.sender == admin, "only admin can set comp claimable");

        compClaimPaused = state;

        emit ActionPaused("Claim COMP", state);

        return uint(Error.NO_ERROR);
    }

    function _setCompGainFactor(uint newCompGainFactorMantissa) public returns (uint) {
        require(msg.sender == admin, "only admin can set comp gain factor");

        uint oldCompGainFactorMantissa = compGainFactorMantissa;
        compGainFactorMantissa = newCompGainFactorMantissa;

        emit NewCompGainFactor(oldCompGainFactorMantissa, newCompGainFactorMantissa);
